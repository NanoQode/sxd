...
import { creditNoteIssued, planCreditNoteApplication, type OriginalRecognition } from '@simplexd/domain/ledger';
...
export async function issueCreditNote(
  ...
  return withActor(rt.db, fa.ctx, async (tx) => {
    const [visible] = await tx.select({ id: schema.invoices.id, organizationId: schema.invoices.organizationId }).from(schema.invoices).where(eq(schema.invoices.id, invoiceId));
    if (!visible) throw new ApiError('not_found', 'invoice not found');
    assertStaff(fa, 'finance.invoices.manage', { type: 'invoice', id: visible.id, organizationId: visible.organizationId });
    ...
    return elevated(tx, fa, async () => {
      ...
      const [note] = await tx
        ...
        .values({ organizationId: invoice.organizationId, invoiceId: invoice.id, number, amountKobo: amount, currency: invoice.currency, reason: input.reason, status: 'issued', issuedBy: userId, issuedAt: now })
        .returning();
      const source: OriginalRecognition = invoice.isRentOnBehalfOfOwner ? 'rent_payable' : invoice.kind === 'deposit' ? 'unearned' : 'revenue';
      const journal = await postJournal(
        tx,
        creditNoteIssued({
          creditNote: { id: note!.id, number, invoiceId: invoice.id, organizationId: invoice.organizationId, currency: invoice.currency, amountKobo: amount },
          source,
      ...
      try {
        ...
        if (isUniqueViolation(err)) throw new ApiError('conflict', 'this credit note was already applied');
        throw err;
      }
      const [updatedInvoice] = await tx
        .update(schema.invoices)
        .set({ amountCreditedKobo: plan.newAmountCreditedKobo, status: plan.newStatus, paidAt: plan.newStatus === 'paid' ? now : invoice.paidAt, version: invoice.version + 1 })
        .where(and(eq(schema.invoices.id, invoice.id), eq(schema.invoices.version, invoice.version)))
        .returning();
      if (!updatedInvoice) throw new ApiError('version_conflict', 'invoice changed while crediting');
      const [applied] = await tx.update(schema.creditNotes).set({ status: 'applied', journalId: journal.id }).where(eq(schema.creditNotes.id, note!.id)).returning();
      await emitEvent(tx, fa, {
        eventType: 'credit_note.issued',
        aggregateType: 'credit_note',
        aggregateId: note!.id,
        organizationId: invoice.organizationId,
        payload: { creditNoteId: note!.id, number, invoiceId: invoice.id, amountKobo: amount, invoiceStatus: plan.newStatus },
      });
...
export async function getCreditNote(rt: FinanceRuntime, fa: FinanceActor, id: string): Promise<CreditNoteDto> {
  return withActor(rt.db, fa.ctx, async (tx) => {

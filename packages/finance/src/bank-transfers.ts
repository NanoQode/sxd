...
import {
  ApiError,
  type BankTransferReceiptCreate,
  type BankTransferReceiptDto,
...
async function loadReceiptForUpdate(tx: Transaction, id: string): Promise<BankReceiptRow> {
  const [row] = await tx
    ...
    .where(eq(schema.bankTransferReceipts.id, id))
    .for('update');
...
export async function submitBankTransferReceipt(
  ...
  return withActor(rt.db, fa.ctx, async (tx) => {
    ...
    if (!['issued', 'partially_paid', 'overdue'].includes(invoice.status)) {
      throw new ApiError(
        'invalid_transition',
        `invoice ${invoice.number} is ${invoice.status} and cannot receive a transfer`,
      );
    }
    ...
    if (input.uploadedFileId) {
      const [file] = await tx
        .select({ id: schema.fileObjects.id, status: schema.fileObjects.status })
        .from(schema.fileObjects)
        .where(eq(schema.fileObjects.id, input.uploadedFileId));
...
export async function confirmBankTransferReceipt(
  ...
  receipt: BankTransferReceiptDto;
  ...
  return withActor(rt.db, fa.ctx, async (tx) => {
    ...
    assertStaff(fa, 'finance.allocations.manage', {
      type: 'invoice',
      id: receipt.invoiceId,
      organizationId: receipt.organizationId,
    });
    if (receipt.status === 'confirmed')
      throw new ApiError('conflict', 'this transfer was already confirmed');
    if (receipt.status === 'rejected')
      throw new ApiError(
        'invalid_transition',
        'a rejected receipt cannot be confirmed; ask the customer to resubmit',
      );
    const confirmed = BigInt(input.confirmedAmountKobo);
    if (confirmed <= 0n)
      throw new ApiError('validation_failed', 'confirmed amount must be positive');
    const now = rt.now();
    return elevated(tx, fa, async () => {
      ...
      const [existing] = await tx
        ...
        .where(eq(schema.allocations.dedupeKey, bankReceiptDedupeKey(receipt.id)));
      ...
      try {
        ...
        if (isUniqueViolation(err))
          throw new ApiError('conflict', 'this transfer was already allocated');
        throw err;
      }
      const [updatedInvoice] = await tx
        ...
        .returning();
      if (!updatedInvoice)
        throw new ApiError('version_conflict', 'invoice changed during confirmation');
        ...
        if (sr && sr.status === 'awaiting_payment') {
          await transitionEngagement(tx, fa, {
            ...
            metadata: {
              ...
              bankReceiptId: receipt.id,
              receiptNumber,
              billingConsequence: 'bank_transfer_confirmed',
            },
          });
        }
      }
...
export async function rejectBankTransferReceipt(
  rt: FinanceRuntime,
  fa: FinanceActor,
  receiptId: string,
  input: { note: string },
  ...
  return withActor(rt.db, fa.ctx, async (tx) => {
    ...
    if (receipt.status === 'confirmed')
      throw new ApiError(
        'invalid_transition',
        'a confirmed transfer cannot be rejected; issue a credit note or refund instead',
      );
    const [updated] = await tx
      .update(schema.bankTransferReceipts)
...
export async function getBankTransferReceipt(
  rt: FinanceRuntime,
  fa: FinanceActor,
  receiptId: string,
): Promise<BankTransferReceiptDto> {
  return withActor(rt.db, fa.ctx, async (tx) => {
    const [row] = await tx
      .select()
      .from(schema.bankTransferReceipts)
      .where(eq(schema.bankTransferReceipts.id, receiptId));
    if (!row) throw new ApiError('not_found', 'bank transfer receipt not found');

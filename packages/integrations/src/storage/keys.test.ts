...
describe('storage keys', () => {
  it('accepts only the narrow safe character set', () => {
    expect(isValidStorageKey('org/acme/evidence/0b6f3c1a-1111-4222-8333-444455556666.jpg')).toBe(
      true,
    );
  ...
  it('builds organisation-scoped keys and refuses blocked extensions', () => {
    expect(
      buildStorageKey({
        ...
        purpose: 'Site Photos',
        fileId: '0B6F3C1A-1111-4222-8333-444455556666',
        ext: 'JPG',
      }),
    ...
    expect(
      buildStorageKey({
        organizationId: null,
        purpose: 'cms-media',
        fileId: 'abcdef12-3456',
        ext: null,
      }),
    ).toBe('shared/cms-media/abcdef12-3456');
    expect(() =>
      buildStorageKey({ organizationId: 'o', purpose: 'p', fileId: 'abcdef12-3456', ext: '.svg' }),
    ).toThrow(/not accepted/);
    expect(() =>
      buildStorageKey({ organizationId: 'o', purpose: 'p', fileId: 'abcdef12-3456', ext: '.html' }),
    ).toThrow(/not accepted/);
    expect(() =>
      buildStorageKey({ organizationId: 'o', purpose: 'p', fileId: '../x', ext: '.jpg' }),
    ).toThrow(/fileId/);
  ...
  it('derives variant keys next to the original', () => {
    expect(deriveVariantKey('org/o/evidence/abc.jpg', 'thumb', 'webp')).toBe(
      'org/o/evidence/abc.thumb.webp',
    );
    expect(deriveVariantKey('shared/doc/abc', 'Redacted Copy', '.pdf')).toBe(
      'shared/doc/abc.redacted-copy.pdf',
    );

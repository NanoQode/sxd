...
describe('content disposition policy', () => {
  it('never serves markup or scripts inline, even when requested', () => {
    ...
    expect(
      resolveContentDisposition('image/svg+xml; charset=utf-8', 'attachment').contentType,
    ).toBe('application/octet-stream');
  });

  it('allows inline only for images, video and PDF', () => {
    expect(resolveContentDisposition('image/png', 'inline')).toEqual({
      disposition: 'inline',
      contentType: 'image/png',
    });
    expect(resolveContentDisposition('video/mp4', 'inline')).toEqual({
      disposition: 'inline',
      contentType: 'video/mp4',
    });
    expect(resolveContentDisposition('application/pdf', 'inline')).toEqual({
      disposition: 'inline',
      contentType: 'application/pdf',
    });
    expect(resolveContentDisposition('image/JPEG; charset=binary', 'inline')).toEqual({
      disposition: 'inline',
    ...
    expect(resolveContentDisposition('application/pdf', 'attachment').disposition).toBe(
      'attachment',
    );
    expect(resolveContentDisposition('', 'inline')).toEqual({
      disposition: 'attachment',
      contentType: 'application/octet-stream',
    });
  });

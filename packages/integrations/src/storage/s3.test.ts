...
describe('S3StorageProvider presigning', () => {
  ...
  it('presigns downloads with disposition/content-type overrides and never inline for markup', async () => {
    ...
    expect(svgUrl.searchParams.get('response-content-disposition')).toBe("attachment; filename=\"logo.svg\"; filename*=UTF-8''logo.svg");
    ...
    expect(pngUrl.searchParams.get('response-content-disposition')).toMatch(/^inline; filename="thumb\.webp"/);
    expect(pngUrl.searchParams.get('response-content-type')).toBe('image/webp');

    await expect(
      provider.createSignedDownloadUrl({ bucket: 'quarantine', key: 'org/o/x/abcdef12-3456.png', fileName: 'x', contentType: 'image/png', contentDisposition: 'inline' }),
    ).rejects.toMatchObject({ code: 'quarantined' });
  });

  it('validates keys, bucket configuration and upload targets', async () => {
    const provider = new S3StorageProvider(options());
    await expect(provider.headObject({ bucket: 'private', key: '../x' })).rejects.toMatchObject({ code: 'invalid_key' });
    await expect(
      provider.createUploadIntent({ bucket: 'private', key: 'org/o/x/abcdef12-3456.png', contentType: 'image/png', sizeBytes: 1, multipart: false }),
    ).rejects.toMatchObject({ code: 'invalid_request' });
    expect(() => new S3StorageProvider({ ...options(), buckets: { private: 'same', quarantine: 'same' } })).toThrow(/different/);
...
describe('S3StorageProvider commands', () => {
  it('creates multipart uploads, presigns parts and completes with sorted parts', async () => {
    ...
    expect(sent[0]).toEqual({
      ...
      input: { Bucket: 'sxd-quarantine', Key: 'org/o/video/abcdef12-3456.mp4', ContentType: 'video/mp4' },
    });

    const done = await provider.completeMultipart({
      key: 'org/o/video/abcdef12-3456.mp4',
      uploadId: 'upload-1',
      parts: [{ partNumber: 3, etag: '"c"' }, { partNumber: 1, etag: '"a"' }, { partNumber: 2, etag: '"b"' }],
    });
    ...
    expect(sent[1]).toEqual({
      ...
      input: {
        ...
        Key: 'org/o/video/abcdef12-3456.mp4',
        UploadId: 'upload-1',
        MultipartUpload: { Parts: [{ PartNumber: 1, ETag: '"a"' }, { PartNumber: 2, ETag: '"b"' }, { PartNumber: 3, ETag: '"c"' }] },
      },
    });
    await expect(provider.createUploadIntent({ key: 'org/o/v/abcdef12-3456.mp4', contentType: 'video/mp4', sizeBytes: 1, multipart: true })).rejects.toThrow(
      /partCount/,
    );
  });
  ...
  it('heads, copies, puts and streams objects through the client', async () => {
    ...
    const { transport, sent } = fakeTransport({
      HeadObjectCommand: { ContentLength: 10, ContentType: 'image/png', ETag: '"e"', LastModified: new Date('2026-09-23T00:00:00Z') },
      PutObjectCommand: { ETag: '"put"' },
      GetObjectCommand: { Body: Readable.from([Buffer.from('data')]) },
    });
    const provider = new S3StorageProvider(options(transport));
    expect(await provider.headObject({ bucket: 'private', key: 'org/o/x/abcdef12-3456.png' })).toEqual({
      sizeBytes: 10,
      contentType: 'image/png',
      etag: '"e"',
      lastModified: new Date('2026-09-23T00:00:00Z'),
    });
    expect(sent[0]).toEqual({ name: HeadObjectCommand.name, input: { Bucket: 'sxd-private', Key: 'org/o/x/abcdef12-3456.png' } });

    await provider.copyObject({ bucket: 'quarantine', key: 'org/o/x/abcdef12-3456.png' }, { bucket: 'private', key: 'org/o/x/abcdef12-3456.png' });
    expect(sent[1]).toEqual({
    ...
    expect(await provider.putObject({ bucket: 'derivatives', key: 'org/o/x/abcdef12-3456.thumb.webp' }, Buffer.from('abc'), 'image/webp')).toEqual({ etag: '"put"' });
    expect(sent[2]).toMatchObject({ name: PutObjectCommand.name, input: { Bucket: 'sxd-private', ContentType: 'image/webp', ContentLength: 3 } });
    await expect(provider.putObject({ bucket: 'private', key: 'org/o/x/abcdef12-3456.bin' }, Readable.from([]), 'application/octet-stream')).rejects.toThrow(
      /contentLength/,
    );

    const stream = await provider.getObjectStream({ bucket: 'private', key: 'org/o/x/abcdef12-3456.png' });
    const chunks: Buffer[] = [];
    for await (const chunk of stream) chunks.push(Buffer.from(chunk as Uint8Array));
    expect(Buffer.concat(chunks).toString()).toBe('data');

    const missing = new S3StorageProvider(options(fakeTransport({ HeadObjectCommand: notFound, GetObjectCommand: notFound }).transport));
    expect(await missing.headObject({ bucket: 'private', key: 'org/o/x/abcdef12-0000.png' })).toBeNull();
    await expect(missing.getObjectStream({ bucket: 'private', key: 'org/o/x/abcdef12-0000.png' })).rejects.toMatchObject({ code: 'not_found' });
  });
});
...
describe('storage factory', () => {
  it('maps environment variables and refuses local-dev outside development/test', () => {
    ...
    expect(s3.s3).toMatchObject({ region: 'eu-west-1', forcePathStyle: false, credentials: null, buckets: { private: 'p', quarantine: 'q' } });
    expect(createStorageProvider(s3).id).toBe('s3');

    const dev = storageConfigFromEnv({ APP_ENV: 'test', STORAGE_PROVIDER: 'local-dev', APP_URL: 'http://localhost:3000', AUTH_SECRET: 'a-long-enough-development-secret' });

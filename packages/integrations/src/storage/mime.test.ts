...
describe('mime detection', () => {
  ...
  it('sniffs markup and scripts that file-type does not classify', () => {
    ...
    expect(sniffActiveContent(Buffer.from('#!/bin/sh\nrm -rf /'))).toEqual({ active: true, kind: 'script' });
    expect(sniffActiveContent(PNG)).toEqual({ active: false, kind: null });
    expect(sniffActiveContent(Buffer.from('name,value\n1,2\n'))).toEqual({ active: false, kind: null });
  });

  it('applies the domain mime-mismatch rules', () => {
    expect(agreesWithDeclared('image/jpg', { mime: 'image/jpeg', ext: 'jpg' })).toMatchObject({ agrees: true, effectiveMime: 'image/jpeg' });
    expect(agreesWithDeclared('application/vnd.openxmlformats-officedocument.wordprocessingml.document', { mime: 'application/zip', ext: 'zip' }).agrees).toBe(true);
    expect(agreesWithDeclared('image/png', { mime: 'application/pdf', ext: 'pdf' })).toMatchObject({ agrees: false });
    expect(agreesWithDeclared('text/csv', null)).toMatchObject({ agrees: true, effectiveMime: 'text/csv' });
  });

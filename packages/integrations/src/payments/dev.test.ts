...
describe('DevPaymentProvider', () => {
  it('refuses live and production', () => {
    expect(
      () => new DevPaymentProvider({ appUrl: 'http://localhost:3000', environment: 'live' }),
    ).toThrow(/live/);
    expect(
      () => new DevPaymentProvider({ appUrl: 'http://localhost:3000', appEnv: 'production' }),
    ).toThrow(/production/);
  ...
  it('initializes to the local checkout page and stays pending until simulated', async () => {
    ...
    expect(result.authorizationUrl).toBe(
      'http://localhost:3000/dev/paystack-checkout?reference=sxd-inv-7-a1',
    );
    expect(result.accessCode).toMatch(/^dev_/);
    expect(result.providerReference).toBe(init.reference);
    await expect(dev.initialize(init)).rejects.toMatchObject({
    ...
    expect(
      matchVerification({ attempt: { ...init, status: 'pending' }, verification: success })
        .decision,
    ).toBe('settle');
    ...
    expect(
      matchVerification({ attempt: { ...init, status: 'pending' }, verification: tampered })
        .decision,
    ).toBe('mismatch');
  ...
  it('runs the refund lifecycle without settling on submission', async () => {
    ...
    dev.simulate(init.reference, 'success');
    const refund = await dev.createRefund({
    ...
    const again = await dev.createRefund({
      reference: init.reference,
      amountKobo: 100_000n,
      reason: 'partial',
      idempotencyKey: 'r1',
    ...
    const delivery = dev.buildWebhookEvent('refund.processed', init.reference, {
  ...
  it('signs simulated webhooks that the same adapter verifies and dedupes', async () => {
    ...
    expect(event.reference).toBe(init.reference);
    expect(deriveDedupeKey(event)).toBe(
      deriveDedupeKey(
        dev.parseWebhookEvent(dev.buildWebhookEvent('charge.success', init.reference).rawBody),
      ),
    );
    expect(
      planWebhookActions(event, { attemptStatus: 'pending', alreadyAllocated: false }),
    ).toEqual([{ kind: 'verify_and_settle', reference: init.reference }]);
    const dispute = dev.parseWebhookEvent(
      dev.buildWebhookEvent('charge.dispute.create', init.reference).rawBody,
    );
    expect(dispute.amountKobo).toBe(250_000n);
    expect(
      planWebhookActions(dispute, { attemptStatus: 'successful', alreadyAllocated: true })[0]!.kind,
    ).toBe('open_chargeback');
  });

  it('reports itself honestly on connection test', async () => {
    const dev = new DevPaymentProvider({ appUrl: 'http://localhost:3000', appEnv: 'development' });
    expect(await dev.testConnection()).toEqual({
      ok: true,
      message: DEV_ADAPTER_LABEL,
...
describe('createPaymentProvider', () => {
  it('refuses the dev adapter in production and builds it elsewhere', () => {
    ...
    const dev = createPaymentProvider({
      provider: 'dev',
      environment: 'test',
      appUrl: 'http://localhost:3000',
      appEnv: 'development',

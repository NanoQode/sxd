...
describe('DevMalwareScanner', () => {
  ...
  it('flags the EICAR string and passes everything else', async () => {
    ...
    expect(
      await scanner.scan(Buffer.from(EICAR_TEST_STRING), { fileName: 'eicar.com', sizeBytes: 68 }),
    ).toMatchObject({
      verdict: 'infected',
    ...
    expect(
      await scanner.scan(Readable.from([Buffer.from('hello '), Buffer.from('world')]), {
        fileName: 'a.txt',
        sizeBytes: 11,
      }),
    ).toMatchObject({
    ...
    expect(
      await scanner.scan(Buffer.from('anything'), {
  ...
  it('is refused outside development/test and selected by the factory', () => {
    ...
    expect(() => createMalwareScanner({ appEnv: 'production', scanner: 'dev' })).toThrow(
      /development adapter/,
    );
  });
});

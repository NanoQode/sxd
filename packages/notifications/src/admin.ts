...
function validateContent(input: {
  ...
  if (!input.bodyText.trim())
    throw new NotificationAdminError('validation_failed', 'bodyText is required', [
      { path: 'bodyText' },
    ]);
  if (input.channel === 'email' && !input.subject?.trim())
    throw new NotificationAdminError('validation_failed', 'email templates need a subject', [
      { path: 'subject' },
    ]);
  if (input.channel === 'sms') {
    if (input.bodyHtml)
      throw new NotificationAdminError('validation_failed', 'SMS templates have no HTML body', [
        { path: 'bodyHtml' },
      ]);
    ...
    if (estimate.segments > 5)
      throw new NotificationAdminError(
        'validation_failed',
        `SMS body would need ${estimate.segments} segments; keep it to 5 or fewer`,
        [{ path: 'bodyText' }],
      );
  }
...
export async function createTemplate(
  ...
  return withActor(db, systemContext('templates'), async (tx) => {
    ...
    await audit(
      ...
      'template.created',
      row!.id,
      { key: row!.key, channel: row!.channel, version: row!.version },
      undefined,
      actor.correlationId,
    );
    return templateToDto(row!);
...
export async function updateTemplate(
  ...
  return withActor(db, systemContext('templates'), async (tx) => {
    ...
    if (current.status !== 'draft')
      throw new NotificationAdminError(
        'invalid_transition',
        `an ${current.status} template is immutable; create a new version`,
      );
    if (
      input.expectedUpdatedAt &&
      Math.abs(new Date(input.expectedUpdatedAt).getTime() - current.updatedAt.getTime()) > 999
    )
      throw new NotificationAdminError('conflict', 'template changed since you loaded it', {
...
export async function applyTemplateAction(
  ...
  return withActor(db, systemContext('templates'), async (tx) => {
    ...
    switch (action) {
      case 'approve':
        if (current.status !== 'draft')
          throw new NotificationAdminError('invalid_transition', 'only drafts can be approved');
      ...
      case 'retire':
        if (current.status === 'retired')
          throw new NotificationAdminError('invalid_transition', 'template is already retired');
      ...
      case 'reopen':
        if (current.status !== 'retired')
          throw new NotificationAdminError(
            'invalid_transition',
            'only retired templates can be reopened as drafts',
          );
...
export async function previewNotificationTemplate(
  ...
  if (!source)
    throw new NotificationAdminError('validation_failed', 'templateId or template is required');
...
export async function testSend(
  ...
  if (input.channel === 'sms') {
    ...
    if (!/^[^\s@]+@[^\s@]+$/.test(email))
      throw new NotificationAdminError('validation_failed', 'invalid email address', [
        { path: 'to' },
      ]);
    recipient = { email };
  }
  const providerInfo =
    input.channel === 'sms'
      ? await resolveSmsProvider(db, env)
      : await resolveMailProvider(db, env);
...
export async function listDeliveries(db: Db, query: DeliveryListQuery = {}): Promise<DeliveryPage> {
  ...
  if (query.cursor) {
    ...
    if (isoAt && id && !Number.isNaN(new Date(isoAt).getTime()))
      cursor = { createdAt: new Date(isoAt), id };
  }
    ...
    const rows = await tx
      ...
      .where(
        and(
          ...
          query.templateKey
            ? eq(schema.deliveryAttempts.templateKey, query.templateKey)
            : undefined,
          query.recipient
            ? ilike(
                schema.deliveryAttempts.recipient,
                `%${query.recipient.replace(/[\\%_]/g, (c) => `\\${c}`)}%`,
              )
            : undefined,
          ...
          cursor
            ? or(
                ...
                and(
                  eq(schema.deliveryAttempts.createdAt, cursor.createdAt),
                  lt(schema.deliveryAttempts.id, cursor.id),
                ),
              )
            : undefined,
        ),
...
export interface ProviderStatusDto {
  ...
  status:
    'disconnected' | 'configured_unverified' | 'connected' | 'degraded' | 'expired' | 'disabled';
  adapter: string;
  enabled: boolean;
  configured: boolean;
  ...
  return withActor(db, systemContext('provider-status'), async (tx) => {
    ...
    for (const provider of ['smtp', 'termii'] as const) {
      ...
      const counts = await tx
        ...
        .where(
          and(
            eq(schema.deliveryAttempts.channel, channel),
            gte(schema.deliveryAttempts.createdAt, since),
          ),
        )

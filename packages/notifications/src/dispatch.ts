...
export async function dispatchRequest(
  ...
  for (const recipient of recipients) {
    ...
    for (const channel of channels) {
      try {
        outcomes.push(
          await dispatchChannel(db, env, providers, request, recipient, channel, variables),
        );
      } catch (err) {
        env.log.error(
...
async function dispatchChannel(
  ...
  const prepared = await withActor(
    db,
    systemContext(request.correlationId ?? 'notifications'),
    async (tx) => {
      ...
      const attemptId =
        ...
        (await insertAttempt(tx, {
          ...
          dedupeKey,
          relatedEntityType:
            request.label === 'test' ? 'test_send' : (request.relatedEntity?.type ?? null),
      ...
      if (rendered?.channel === 'email') {
        ...
        message = {
          ...
          to: address!,
          body: rendered.body,
          category: smsCategoryFor(request.category),
        };
      } else {
        return {

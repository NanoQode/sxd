...
/**
 * Scheduled work: quiet-hour deferrals whose send time has passed and
 * daily/weekly digests that bundle in-app items for people who asked for a
 * summary instead of individual emails. Booking reminders are emitted as
 * `appointment.reminder_due` outbox events by the calendar scan and flow
 * through the event registry like any other event.
 */
...
export async function sendDueReminders(
  ...
  const result: ReminderSweepResult = { deferredSent: 0, deferredFailed: 0 };
  const now = env.now();

  // Deferred (quiet hours) sends whose send_at has passed.
  for (let round = 0; round < 20; round += 1) {
    const jobs = await withActor(db, systemContext('reminders'), (tx) =>
      claimJobs(tx, {
        workerId: 'notifications-reminders',
        queues: [DEFERRED_QUEUE],
        limit: 25,
        now,
      }),
    );
    ...
    for (const job of jobs) {
      try {
        const outcome = await executeDeferredSend(
          db,
          job.payload as unknown as DeferredJobPayload,
          env,
          providers,
        );
...
export async function sendDigests(db: Db, options: PipelineOptions = {}): Promise<DigestResult> {
  ...
  const byUser = new Map<
    string,
    { period: 'daily' | 'weekly'; categories: NotificationCategory[] }
  >();
  ...
  for (const [userId, { period, categories }] of byUser) {
    ...
    const { last, items, user } = await withActor(db, systemContext('digests'), async (tx) => {
      const [last] = await tx
        .select({
          sentAt: schema.deliveryAttempts.sentAt,
          queuedAt: schema.deliveryAttempts.queuedAt,
        })
    ...
    const bucket =
      period === 'daily'
        ...
        : `${now.getUTCFullYear()}-w${isoWeek(now)}`;
      ...
      db,
      {
        ...
        variables: {
          ...
          items: items
            .map(
              (i) =>
                `- ${i.title}${i.body ? `: ${i.body}` : ''}${i.linkPath ? ` (${env.appUrl}${i.linkPath})` : ''}`,
            )
            .join('\n'),
          appUrl: env.appUrl,
        },
    ...
    if (outcome.outcomes.some((o) => o.status === 'sent' || o.status === 'accepted'))

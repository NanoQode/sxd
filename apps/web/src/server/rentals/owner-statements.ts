 ...
 * (settled allocations mirrored into `rent_allocations`), the agreed
...
function assertRead(identity: RequestIdentity, ref: ResourceRef): void {
  assertAllowed(authorizeAny(identity.actor, [{ staff: 'rentals.manage' }, { staff: 'finance.read' }, { org: 'org.read' }], ref));
}
...
export function toStatementDto(row: StatementRow): OwnerStatementDto {
  ...
  return {
    ...
    status: row.status,
    totals: row.totals ?? { collectedKobo: '0', feesKobo: '0', expensesKobo: '0', netKobo: '0', arrearsKobo: '0', openObligations: [] },
    lines: s.lines,
...
async function leasesInScope(tx: DbExecutor, scope: { organizationId: string; propertyId: string | null; estateId: string | null }): Promise<LeaseRow[]> {
  const propertyIds = scope.estateId
    ? (await tx.select({ id: schema.properties.id }).from(schema.properties).where(eq(schema.properties.estateId, scope.estateId))).map((p) => p.id)
    : null;
...
async function collectedLines(tx: DbExecutor, leases: LeaseRow[], periodStart: string, periodEnd: string): Promise<StatementLine[]> {
  ...
  const rows = await tx
    ...
    .where(
      and(
        inArray(schema.rentAllocations.leaseId, leases.map((l) => l.id)),
...
async function feeLines(tx: DbExecutor, leases: LeaseRow[], collected: StatementLine[], periodStart: string, periodEnd: string): Promise<StatementLine[]> {
  const months = monthsInPeriod(periodStart, periodEnd);
  const out: StatementLine[] = [];
  for (const lease of leases) {
    const leaseCollected = collected.filter((c) => c.leaseId === lease.id && c.kind === 'rent_collected').reduce((s, c) => s + c.amountKobo, 0n);
    const fee = leaseManagementFee(
      { basis: lease.managementFeeBasis, feeBps: lease.managementFeeBps, fixedKobo: lease.managementFeeFixedKobo },
      leaseCollected,
    ...
    out.push({
      ...
      amountKobo: fee,
      leaseId: lease.id,
      feeBps: lease.managementFeeBasis === 'percentage_of_collected' ? lease.managementFeeBps : null,
    });
  }
  return out;
}
...
async function recoveryLines(tx: DbExecutor, scope: { organizationId: string; propertyId: string | null; estateId: string | null }, periodEnd: string, statementId: string | null): Promise<{ lines: StatementLine[]; workOrderIds: string[] }> {
  const rows = await tx
    .select()
  ...
  for (const wo of rows) {
    ...
    if (recovered) {
      const [j] = await tx.select({ sourceId: schema.journals.sourceId, description: schema.journals.description }).from(schema.journals).where(eq(schema.journals.id, recovered.id));
    ...
    lines.push({ kind: 'maintenance_recovery', description: `Maintenance: ${wo.title}`, amountKobo: amount, workOrderId: wo.id, leaseId: wo.leaseId });
    ids.push(wo.id);
  }
  return { lines, workOrderIds: ids };
}

async function arrearsLines(tx: DbExecutor, leases: LeaseRow[], periodEnd: string): Promise<StatementLine[]> {
  const out: StatementLine[] = [];
  for (const lease of leases) {
    const charges = (await loadCharges(tx, lease.id)).filter((c) => c.kind !== 'deposit' && c.chargedAt <= periodEnd);
    ...
    if (ageing.totalOutstandingKobo > 0n)
      out.push({ kind: 'arrears', description: `Outstanding on lease ${lease.id.slice(0, 8)} as of ${periodEnd}`, amountKobo: ageing.totalOutstandingKobo, leaseId: lease.id });
  }
  return out;
}

async function shortStayLines(tx: DbExecutor, scope: { organizationId: string; propertyId: string | null }, periodStart: string, periodEnd: string): Promise<StatementLine[]> {
  const rows = await tx
    .select()
  ...
  const out: StatementLine[] = [];
  for (const b of rows) {
    out.push({ kind: 'short_stay_income', description: `Stay ${b.guestName} ${b.checkIn} to ${b.checkOut} (${b.nights} nights)`, amountKobo: stayGross(b.nights, b.nightlyRateKobo), stayBookingId: b.id });
    const expense = b.platformFeeKobo + b.cleaningKobo;
    if (expense > 0n) out.push({ kind: 'short_stay_expense', description: `Stay costs (platform fee + cleaning) for ${b.guestName}`, amountKobo: expense, stayBookingId: b.id });
  }
  return out;
}
...
export async function generateOwnerStatement(
  ...
  if (input.periodEnd < input.periodStart) throw new ApiError('validation_failed', 'periodEnd is before periodStart');
  if (input.estateId && !isFeatureEnabled(identity, FEATURES.estateManagement))
    throw new ApiError('feature_disabled', 'estate statements need estate management', { details: { feature: FEATURES.estateManagement } });
  const organizationId = resolveOrganization(identity, input.organizationId);
  assertManage(identity, statementRef(organizationId));
  const scope = { organizationId, propertyId: input.propertyId ?? null, estateId: input.estateId ?? null };
  return withActor(getDb(), ctxFor(identity, options), async (tx) => {
    const leases = await leasesInScope(tx, scope);
    await syncRentAllocations(tx, leases.map((l) => l.id), input.periodEnd);
    const [existing] = await tx
      ...
      .where(
        and(
          ...
          scope.propertyId ? eq(schema.ownerStatements.propertyId, scope.propertyId) : sql`${schema.ownerStatements.propertyId} IS NULL`,
          scope.estateId ? eq(schema.ownerStatements.estateId, scope.estateId) : sql`${schema.ownerStatements.estateId} IS NULL`,
          eq(schema.ownerStatements.periodStart, input.periodStart),
          eq(schema.ownerStatements.periodEnd, input.periodEnd),
        ),
      );
    if (existing && existing.status !== 'draft')
      throw new ApiError('conflict', `a ${existing.status} statement already exists for this period`, { details: { statementId: existing.id } });
    ...
    const stays = isFeatureEnabled(identity, FEATURES.shortStay) ? await shortStayLines(tx, scope, input.periodStart, input.periodEnd) : [];
    ...
    const openWork = await tx
      .select({ id: schema.workOrders.id, title: schema.workOrders.title, estimate: schema.workOrders.estimateKobo, approved: schema.workOrders.approvedAmountKobo })
      .from(schema.workOrders)
      .where(
        and(
          eq(schema.workOrders.organizationId, organizationId),
          scope.propertyId ? eq(schema.workOrders.propertyId, scope.propertyId) : undefined,
          inArray(schema.workOrders.status, ['approved', 'in_progress', 'completed', 'awaiting_approval']),
        ),
      );
    const openObligations = [
      ...arrears.map((a) => ({ description: a.description, amountKobo: a.amountKobo.toString() })),
      ...openWork.map((w) => ({ description: `Open work: ${w.title}`, amountKobo: (w.approved ?? w.estimate ?? 0n).toString() })),
    ];
    const totalsJson = {
    ...
    const [row] = existing
      ? await tx.update(schema.ownerStatements).set({ totals: totalsJson, lines: linesJson, generatedAt: new Date() }).where(eq(schema.ownerStatements.id, existing.id)).returning()
      : await tx
          .insert(schema.ownerStatements)
          .values({ organizationId, propertyId: scope.propertyId, estateId: scope.estateId, periodStart: input.periodStart, periodEnd: input.periodEnd, status: 'draft', totals: totalsJson, lines: linesJson })
          .returning();
    await recordAudit(tx, identity, {
      ...
      entityId: row!.id,
      organizationId,
      after: { periodStart: input.periodStart, periodEnd: input.periodEnd, totals: totalsJson, leases: leases.length },
...
function feeDraft(statement: StatementRow, line: StatementLineDto, lease: LeaseRow, estateSegment: string | null): JournalDraft {
  ...
  if (line.feeBps) {
    const draft = managementFeeFromCollectedRent({
      statement: { id: statement.id, organizationId: statement.organizationId, ownerOrganizationId: statement.organizationId, period: `${statement.periodStart}..${statement.periodEnd}` },
  ...
  return {
    ...
    lines: [
      debit(ACCOUNTS.RENT_COLLECTED_PAYABLE_TO_OWNERS, amount, { ...entity, memo: 'Fixed management fee deducted from rent payable', organizationId: statement.organizationId }),
      credit(ACCOUNTS.MANAGEMENT_FEE_REVENUE, amount, { ...entity, memo: 'Fixed management fee earned' }),
    ],
  };
}
...
export async function reconcileOwnerStatement(
  ...
  return withActor(getDb(), ctx, async (tx) => {
    const [row] = await tx.select().from(schema.ownerStatements).where(eq(schema.ownerStatements.id, id)).for('update');
    ...
    const s = stored(row);
    const leases = s.leaseIds.length > 0 ? await tx.select().from(schema.leases).where(inArray(schema.leases.id, s.leaseIds)) : [];
    const estateSegment = row.estateId ? ((await tx.select({ s: schema.estates.ledgerSegment }).from(schema.estates).where(eq(schema.estates.id, row.estateId)))[0]?.s ?? null) : null;
    const workOrders = s.workOrderIds.length > 0 ? await tx.select().from(schema.workOrders).where(inArray(schema.workOrders.id, s.workOrderIds)) : [];
    ...
    const result = await elevated(tx, ctx, async () => {
      for (const line of s.lines.filter((l) => l.kind === 'management_fee')) {
        ...
        if (!(await findJournalByRef(tx, draft.businessEventRef))) await postJournal(tx, draft, { postedBy: userId });
      }
      for (const wo of workOrders) {
        const line = s.lines.find((l) => l.workOrderId === wo.id);
        if (!line) continue;
        const draft = maintenanceExpenseRecovered({
          workOrder: { id: wo.id, organizationId: wo.organizationId, currency: 'NGN', ownerOrganizationId: wo.organizationId },
          amountKobo: BigInt(line.amountKobo),
          estateSegment,
        });
        if (!(await findJournalByRef(tx, draft.businessEventRef))) {
          await postJournal(tx, { ...draft, description: `${draft.description} (statement ${row.id})` }, { postedBy: userId });
        }
      }
          ...
          : BigInt(
              (
                await tx.execute<{ total: string }>(sql`
                  ...
                  where i.lease_id in (${sql.join(leases.map((l) => sql`${l.id}`), sql`, `)})
              ...
              (
                await tx.execute<{ total: string }>(sql`
                  ...
                  where la.code = ${ACCOUNTS.RENT_COLLECTED_PAYABLE_TO_OWNERS}
                    and j.business_event_ref in (${sql.join(workOrders.map((w) => sql`${`work_order:${w.id}:recovered`}`), sql`, `)})
                `)
              ).rows[0]?.total ?? '0',
            );
    ...
    if (!matches) {
      await tx.update(schema.ownerStatements).set({ lines: { ...s, reconciliation } }).where(eq(schema.ownerStatements.id, id));
      throw new ApiError('conflict', 'statement totals do not match the ledger; regenerate the statement', { details: { totals, reconciliation } });
    }
    ...
    const [updated] = await tx
      ...
      .set({ status: 'reconciled', reconciledBy: userId, reconciledAt: new Date(), lines: { ...s, reconciliation, reconciliationId } })
...
export async function issueOwnerStatement(identity: RequestIdentity, id: string, options: ServiceOptions = {}): Promise<OwnerStatementDto> {
  const userId = requireUserId(identity);
  return withActor(getDb(), ctxFor(identity, options), async (tx) => {
    const [row] = await tx.select().from(schema.ownerStatements).where(eq(schema.ownerStatements.id, id)).for('update');
    if (!row) throw new ApiError('not_found', 'statement not found');
    assertManage(identity, statementRef(row.organizationId, row.id));
    if (row.status !== 'reconciled') throw new ApiError('invalid_transition', `only reconciled statements are issued (statement is ${row.status})`);
    const [updated] = await tx.update(schema.ownerStatements).set({ status: 'issued', issuedAt: new Date() }).where(eq(schema.ownerStatements.id, id)).returning();
    await recordAudit(tx, identity, { action: 'owner_statement.issued', entityType: 'owner_statement', entityId: id, organizationId: row.organizationId, correlationId: options.correlationId });
    const members = await tx.select({ userId: schema.member.userId }).from(schema.member).where(eq(schema.member.organizationId, row.organizationId));
    await appendOutbox(tx, {
      ...
      organizationId: row.organizationId,
      actorUserId: userId,
      payload: { statementId: id, periodStart: row.periodStart, periodEnd: row.periodEnd, netKobo: row.totals?.netKobo ?? '0', recipientUserIds: members.map((m) => m.userId) },
...
export async function getOwnerStatement(identity: RequestIdentity, id: string): Promise<OwnerStatementDto> {
  requireUserId(identity);
  return withActor(getDb(), identity.ctx, async (tx) => {
    const [row] = await tx.select().from(schema.ownerStatements).where(eq(schema.ownerStatements.id, id));
    if (!row) throw new ApiError('not_found', 'statement not found');
    ...
    if (!isStaffIdentity(identity) && row.status === 'draft') throw new ApiError('not_found', 'statement not found');
    return toStatementDto(row);
  });
}

export async function listOwnerStatements(identity: RequestIdentity, query: OwnerStatementListQuery): Promise<Page<OwnerStatementDto>> {
  requireUserId(identity);
  ...
  return withActor(getDb(), identity.ctx, async (tx) => {
    const rows = await tx
      ...
      .where(
        and(
          ...
          cursor ? or(lt(schema.ownerStatements.generatedAt, cursor.createdAt), and(eq(schema.ownerStatements.generatedAt, cursor.createdAt), lt(schema.ownerStatements.id, cursor.id))) : undefined,
        ),
      )
    ...
    return { items: page.map(toStatementDto), nextCursor: last ? encodeCursor(last.generatedAt, last.id) : null };
  });
}

/** Statement row loader for the payout module. */
export async function loadStatementForPayout(tx: DbExecutor, id: string): Promise<{ row: StatementRow; reconciliationId: string | null }> {
  const [row] = await tx.select().from(schema.ownerStatements).where(eq(schema.ownerStatements.id, id));
  if (!row) throw new ApiError('not_found', 'statement not found');

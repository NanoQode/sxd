...
function assertManage(identity: RequestIdentity, r: ResourceRef): void {
  assertAllowed(authorizeAny(identity.actor, [{ staff: 'estates.manage' }, { org: 'org.properties.manage' }], r));
}

function assertRead(identity: RequestIdentity, r: ResourceRef): void {
  assertAllowed(authorizeAny(identity.actor, [{ staff: 'estates.manage' }, { staff: 'rentals.manage' }, { staff: 'customers.read' }, { org: 'org.read' }], r));
}

async function toDto(tx: DbExecutor, row: EstateRow): Promise<EstateDto> {
  const props = await tx.select({ id: schema.properties.id }).from(schema.properties).where(eq(schema.properties.estateId, row.id)).orderBy(asc(schema.properties.name));
  return {
    id: row.id,
...
export async function createEstate(identity: RequestIdentity, input: EstateCreate, options: ServiceOptions = {}): Promise<EstateDto> {
...
async function requireEstate(tx: DbExecutor, identity: RequestIdentity, id: string, mode: 'read' | 'manage'): Promise<EstateRow> {
...
export async function getEstate(identity: RequestIdentity, id: string): Promise<EstateDto> {
  requireFlag(identity, FEATURES.estateManagement);
  return withActor(getDb(), identity.ctx, async (tx) => toDto(tx, await requireEstate(tx, identity, id, 'read')));
}

export async function listEstates(identity: RequestIdentity, query: { organizationId?: string }): Promise<EstateDto[]> {
  requireFlag(identity, FEATURES.estateManagement);
  requireUserId(identity);
  ...
  return withActor(getDb(), identity.ctx, async (tx) => {
    const rows = await tx.select().from(schema.estates).where(orgId ? eq(schema.estates.organizationId, orgId) : undefined).orderBy(asc(schema.estates.name));
    const out: EstateDto[] = [];
...
export async function attachProperty(identity: RequestIdentity, estateId: string, propertyId: string, options: ServiceOptions = {}): Promise<EstateDto> {
  requireFlag(identity, FEATURES.estateManagement);
  requireUserId(identity);
  return withActor(getDb(), ctxFor(identity, options), async (tx) => {
    const estate = await requireEstate(tx, identity, estateId, 'manage');
    const [property] = await tx.select({ id: schema.properties.id, organizationId: schema.properties.organizationId }).from(schema.properties).where(eq(schema.properties.id, propertyId));
    if (!property || property.organizationId !== estate.organizationId) throw new ApiError('not_found', 'property not found');
    await tx.update(schema.properties).set({ estateId }).where(eq(schema.properties.id, propertyId));
    await recordAudit(tx, identity, { action: 'estate.property_attached', entityType: 'estate', entityId: estateId, organizationId: estate.organizationId, after: { propertyId }, correlationId: options.correlationId });
...
export async function runServiceCharges(
  ...
  return withActor(getDb(), ctx, async (tx) => {
    const estate = await requireEstate(tx, identity, estateId, 'manage');
    assertAllowed(authorizeAny(identity.actor, [{ staff: 'estates.manage' }], ref(estate.organizationId, estate.id)));
    const policy = (estate.serviceChargePolicy as { amountKobo?: string; description?: string } | null) ?? null;
    const amountText = input.amountKobo ?? policy?.amountKobo;
    if (!amountText) throw new ApiError('validation_failed', 'the estate has no service charge policy; pass amountKobo');
    const amount = BigInt(amountText);
    if (amount <= 0n) throw new ApiError('validation_failed', 'service charge must be positive');
    const propertyIds = (await tx.select({ id: schema.properties.id }).from(schema.properties).where(eq(schema.properties.estateId, estateId))).map((p) => p.id);
    if (propertyIds.length === 0) return { invoiced: 0, skipped: 0, invoiceIds: [] };
    const leases = await tx.select().from(schema.leases).where(and(inArray(schema.leases.propertyId, propertyIds), inArray(schema.leases.status, ['active', 'expiring'])));
    ...
    for (const lease of leases) {
      const [dup] = await tx
        ...
        .where(and(eq(schema.rentCharges.leaseId, lease.id), eq(schema.rentCharges.kind, 'service_charge'), eq(schema.rentCharges.estateId, estateId), eq(schema.rentCharges.description, description)))
        .limit(1);
      if (dup) {
        result.skipped += 1;
        continue;
      ...
      const [charge] = await tx
        ...
        .values({ leaseId: lease.id, kind: 'service_charge', description, amountKobo: amount, chargedAt: dueDate, estateId, createdBy: userId })
        .returning();
      const [tenant] = await tx
        .select({ userId: schema.leaseParties.userId })
        .from(schema.leaseParties)
        .where(and(eq(schema.leaseParties.leaseId, lease.id), eq(schema.leaseParties.role, 'tenant'), eq(schema.leaseParties.accessStatus, 'active')))
        .limit(1);
      const invoiceId = await elevated(tx, ctx, async () => {
        ...
        const [linked] = await tx
          ...
          .set({ leaseId: lease.id, isRentOnBehalfOfOwner: true, ownerOrganizationId: null, estateSegment: estate.ledgerSegment })
          .where(eq(schema.invoices.id, draft.id))
          .returning();
        const issued = await issueInvoiceTx(tx, fa, linked!, { now: new Date(), dueDate });
        return issued.id;
      });
      await tx.update(schema.rentCharges).set({ invoiceId }).where(eq(schema.rentCharges.id, charge!.id));
      result.invoiced += 1;
    ...
    await recordAudit(tx, identity, {
      ...
      after: { periodStart: input.periodStart, periodEnd: input.periodEnd, amountKobo: amount, ...result },
      correlationId: options.correlationId,
    });
    return result;
  });

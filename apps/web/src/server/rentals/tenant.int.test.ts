...
import { createLease, getLease, inviteParty, listParties, postLeaseNotice, revokeParty, transitionLease } from './leases';
import { generateOwnerStatement, listOwnerStatements } from './owner-statements';
import { listPayouts } from './payouts';
import { getLeaseBalance, listCharges, runRentInvoicing } from './schedules';
import { createMyTicket, getMyBalance, getMyLease, listMyCharges, listMyLeases, listMyNotices, listMyReceipts, listMyTickets } from './tenant';
import { getWorkOrder } from '@/server/maintenance/work-orders';
import { createRentalFixture, errorCode, opsIdentity, ownerIdentity, payInvoice, tenantIdentity, type RentalFixture } from './testing/fixtures';
...
async function invite(owner: ReturnType<typeof ownerIdentity>, leaseId: string, tenant: 1 | 2) {
  ...
  const party = await inviteParty(owner, leaseId, { role: 'tenant', name: 'T', email, expiresInDays: 7 });
  // Accept directly at the data layer (the token flow is covered in rentals.int.test.ts).
  await f.dbs.owner
    .update(schema.leaseParties)
    .set({ userId: tenant === 1 ? f.tenant1 : f.tenant2, accessStatus: 'active', acceptedAt: new Date(), invitationTokenHash: null })
...
beforeAll(async () => {
  ...
  const base = { kind: 'residential_monthly' as const, startDate: '2026-01-01', endDate: '2026-12-31', rentAmountKobo: '10000000', rentPeriod: 'monthly' as const, currency: 'NGN', depositKobo: '0', managementFeeBasis: 'none' as const };
  const a = await createLease(ownerIdentity(f, 'A'), { ...base, propertyId: f.propertyA, unitId: f.unitA1 });
  const b = await createLease(ownerIdentity(f, 'B'), { ...base, propertyId: f.propertyB, unitId: f.unitB1 });
  leaseA = a.id;
  leaseB = b.id;
...
describe('tenant portal isolation', () => {
  it('lists and reads only the tenant\'s own lease', async () => {
    ...
    await expect(getLeaseBalance(t1, leaseB)).rejects.toSatisfy((e) => errorCode(e) === 'not_found');
    await expect(listCharges(tenantIdentity(f, 2), leaseA)).rejects.toSatisfy((e) => errorCode(e) === 'not_found');
    ...
    expect((await getLease(ownerIdentity(f, 'A'), leaseA)).parties.length).toBeGreaterThanOrEqual(1);
  });

  it('shows balances, charges, receipts and notices for the tenant only', async () => {
    const t1 = tenantIdentity(f, 1);
    const [invoice] = await f.dbs.owner.select().from(schema.invoices).where(eq(schema.invoices.leaseId, leaseA));
    ...
    await postLeaseNotice(ownerIdentity(f, 'A'), leaseA, { title: 'Inspection', body: 'Annual inspection next week.' });
  ...
  it('keeps maintenance tickets per tenant and never exposes the owner ledger', async () => {
    ...
    const ticket = await createMyTicket(t1, leaseA, { title: 'Leaking tap', description: 'Kitchen', category: 'plumbing', priority: 'normal' });
    expect(ticket).toMatchObject({ leaseId: leaseA, propertyId: f.propertyA, unitId: f.unitA1, reportedByUserId: f.tenant1, status: 'requested' });
    await expect(createMyTicket(t1, leaseB, { title: 'x', category: 'other', priority: 'low' })).rejects.toSatisfy((e) => errorCode(e) === 'not_found');
    expect((await listMyTickets(t1)).map((w) => w.id)).toEqual([ticket.id]);
    expect(await listMyTickets(t2)).toHaveLength(0);
    await expect(getWorkOrder(t2, ticket.id)).rejects.toSatisfy((e) => errorCode(e) === 'not_found');
    ...
    const statement = await generateOwnerStatement(opsIdentity(f), { organizationId: f.orgA, periodStart: '2026-01-01', periodEnd: '2026-12-31' });
  ...
  it('removes access the moment a party is revoked', async () => {
    const t1 = tenantIdentity(f, 1);
    const party = (await getLease(ownerIdentity(f, 'A'), leaseA)).parties.find((p) => p.userId === f.tenant1)!;

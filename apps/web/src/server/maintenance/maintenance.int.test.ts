...
describe('work orders', () => {
  it('runs request → triage → assign → evidence → approval → complete → verify (expense journal) → close', async () => {
    ...
    const wo = await createWorkOrder(owner, {
    ...
    await expect(getWorkOrder(ownerIdentity(f, 'B'), wo.id)).rejects.toSatisfy(
      (e) => errorCode(e) === 'not_found',
    );
    await expect(
      triageWorkOrder(supportIdentity(f), wo.id, { expectedVersion: 1 }),
    ).rejects.toSatisfy((e) => ['forbidden', 'not_found'].includes(errorCode(e)!));
    await expect(triageWorkOrder(owner, wo.id, { expectedVersion: 1 })).rejects.toSatisfy(
      (e) => errorCode(e) === 'forbidden',
    );

    const triaged = await triageWorkOrder(ops, wo.id, {
      priority: 'urgent',
      estimateKobo: '8000000',
      expectedVersion: 1,
    });
    expect(triaged.status).toBe('triaged');
    expect(triaged.priority).toBe('urgent');
    await expect(
      assignWorkOrder(ops, wo.id, { assigneeUserId: f.tenant1, expectedVersion: 2 }),
    ).rejects.toSatisfy((e) => errorCode(e) === 'validation_failed');
    const assigned = await assignWorkOrder(ops, wo.id, {
      assigneeUserId: f.partner,
      instructions: 'Replace the heater element',
      expectedVersion: 2,
    });
    expect(assigned).toMatchObject({
      status: 'assigned',
      assigneeUserId: f.partner,
      assigneeName: f.partner,
    });
    const [assignment] = await f.dbs.owner
      .select()
      .from(schema.assignments)
      .where(eq(schema.assignments.assigneeUserId, f.partner));
    ...
    await expect(
      startWorkOrder(tenantIdentity(f, 2), wo.id, { expectedVersion: 3 }),
    ).rejects.toSatisfy((e) => errorCode(e) === 'not_found');
    const started = await startWorkOrder(partner, wo.id, { expectedVersion: 3 });
    expect(started.status).toBe('in_progress');
    const awaiting = await requestApproval(partner, wo.id, {
      estimateKobo: '9500000',
      expectedVersion: 4,
    });
    expect(awaiting.status).toBe('awaiting_approval');
    // Only the owner organisation approves the cost.
    await expect(
      approveWorkOrder(partner, wo.id, { approvedAmountKobo: '9500000', expectedVersion: 5 }),
    ).rejects.toSatisfy((e) => errorCode(e) === 'forbidden');
    await expect(
      approveWorkOrder(ops, wo.id, { approvedAmountKobo: '9500000', expectedVersion: 5 }),
    ).rejects.toSatisfy((e) => errorCode(e) === 'forbidden');
    // An adviser reads the organisation's work but cannot decide its cost.
    await expect(
      approveWorkOrder(adviserIdentity(f), wo.id, {
        approvedAmountKobo: '9500000',
        expectedVersion: 5,
      }),
    ).rejects.toSatisfy((e) => errorCode(e) === 'forbidden');
    const approved = await approveWorkOrder(owner, wo.id, {
      approvedAmountKobo: '9500000',
      expectedVersion: 5,
    });
    ...
    await expect(
      completeWorkOrder(partner, wo.id, { actualCostKobo: '9200000', expectedVersion: 6 }),
    ).rejects.toSatisfy((e) => errorCode(e) === 'insufficient_evidence');
    const foreignFile = await insertFile(f.dbs.owner, f.orgB, f.ownerB);
    await expect(addEvidence(partner, wo.id, { fileIds: [foreignFile] })).rejects.toSatisfy((e) =>
      ['validation_failed', 'not_found'].includes(errorCode(e)!),
    );
    const photo = await insertFile(f.dbs.owner, f.orgA, f.partner);
    const withEvidence = await addEvidence(partner, wo.id, {
      fileIds: [photo],
      caption: 'New element fitted',
    });
    expect(withEvidence.evidence).toHaveLength(1);
    expect(withEvidence.evidence[0]).toMatchObject({
      fileId: photo,
      kind: 'photo',
      uploaderUserId: f.partner,
    });
    const completed = await completeWorkOrder(partner, wo.id, {
      actualCostKobo: '9200000',
      expectedVersion: 6,
    });
    ...
    await expect(verifyWorkOrder(partner, wo.id, { expectedVersion: 7 })).rejects.toSatisfy(
      (e) => errorCode(e) === 'forbidden',
    );
    // Read-only staff (support: customers.read) see the work order but cannot verify or post the expense.
    expect((await getWorkOrder(supportIdentity(f), wo.id)).status).toBe('completed');
    await expect(
      verifyWorkOrder(supportIdentity(f), wo.id, { expectedVersion: 7 }),
    ).rejects.toSatisfy((e) => errorCode(e) === 'forbidden');
    await expect(
      verifyWorkOrder(ops, wo.id, { costKobo: '9900000', expectedVersion: 7 }),
    ).rejects.toSatisfy((e) => errorCode(e) === 'validation_failed');
    ...
    await expect(closeWorkOrder(ops, wo.id, { expectedVersion: 9 })).rejects.toSatisfy(
    ...
    expect(
      (await journalLinesByRef(f.dbs.owner, `work_order:${wo.id}:recovered`)).map((l) => l.code),
    ).toEqual(['2100', '5200']);
    ...
    const audit = await f.dbs.owner
      ...
      .where(eq(schema.auditEvents.entityId, wo.id));
    ...
    const events = await f.dbs.owner
      ...
      .where(eq(schema.outboxEvents.aggregateId, wo.id));
  ...
  it('flags SLA breaches once, lists them and lets the owner cancel with a reason', async () => {
    ...
    const breached = await listWorkOrders(opsIdentity(f), {
      limit: 10,
      breachedOnly: true,
      organizationId: f.orgA,
    });
    ...
    expect(
      (await listWorkOrders(ownerIdentity(f, 'B'), { limit: 10 })).items.some(
        (w) => w.id === wo.id,
      ),
    ).toBe(false);
    await expect(
      cancelWorkOrder(owner, wo.id, { reason: '', expectedVersion: 1 }),
    ).rejects.toSatisfy((e) => errorCode(e) === 'invalid_transition');
    const cancelled = await cancelWorkOrder(owner, wo.id, {
      reason: 'Fixed by the estate',
      expectedVersion: 1,
    });
...
describe('assets and preventive maintenance', () => {
  it('is gated by the flag, keeps warranties and generates recurring work orders', async () => {
    ...
    await expect(listAssets(owner, { limit: 10 })).rejects.toSatisfy(
    ...
    const asset = await createAsset(flagged, {
      propertyId: f.propertyA,
    ...
    const withWarranty = await addWarranty(flagged, asset.id, {
      provider: 'Mikano',
      reference: 'W-1',
      startsAt: '2025-01-10',
    ...
    await expect(getAsset(ownerIdentity(f, 'B', ALL_FLAGS), asset.id)).rejects.toSatisfy(
      (e) => errorCode(e) === 'not_found',
    );
    expect(
      (await listAssets(flagged, { limit: 10, serviceDueBefore: '2026-09-22' })).items.map(
        (a) => a.id,
      ),
    ).toEqual([asset.id]);
    ...
    expect(await generateRecurringWorkOrders(f.dbs.app, '2026-09-22')).toBe(0);
    const [recurring] = await f.dbs.owner
      .select()
      .from(schema.workOrders)
      .where(eq(schema.workOrders.assetId, asset.id));

...
export async function requireWorkOrder(
  ...
  if (
    partyIds.includes(userId) ||
    (row.reportedByUserId === userId &&
      !identity.actor.memberships.some((m) => m.organizationId === row.organizationId))
  ) {
    assertAllowed(
      authorizeTenant(identity.actor, 'tenant.maintenance.request', {
        ...ref,
...
/**
 * Write rights on top of the read relationship: staff act with
 * `maintenance.manage` (read-only staff roles cannot move work); the owner
 * organisation decides costs and verifies with `org.change_orders.approve`
 * and cancels with `org.maintenance.request`; a tenant only cancels a ticket
 * they reported. Assignees are bounded by the machine rules.
 */
function assertWriteRight(
  identity: RequestIdentity,
  access: WorkOrderAccess,
  to: WorkOrderRow['status'] | 'evidence',
): void {
  const userId = requireUserId(identity);
  switch (access.viewer) {
    case 'staff':
      assertAllowed(authorizeAny(identity.actor, [{ staff: 'maintenance.manage' }], access.ref));
      return;
    case 'customer':
      if (to === 'cancelled')
        assertAllowed(
          authorizeAny(identity.actor, [{ org: 'org.maintenance.request' }], access.ref),
        );
      else
        assertAllowed(
          authorizeAny(identity.actor, [{ org: 'org.change_orders.approve' }], access.ref),
        );
      return;
    case 'tenant':
      if (to !== 'cancelled' || access.row.reportedByUserId !== userId)
        throw new ApiError('forbidden', 'tenants can only cancel tickets they reported');
      return;
    case 'assignee':
      return;
  }
  ...
  return withActor(getDb(), ctxFor(identity, options), async (tx) => {
    const [property] = await tx
      .select({
        ...
        estateId: schema.properties.estateId,
      })
    ...
    if (leaseId) {
      ...
      if (!lease || lease.propertyId !== property.id)
        throw new ApiError('not_found', 'lease not found');
    ...
    if (input.assetId) {
      if (!isFeatureEnabled(identity, FEATURES.preventiveMaintenance))
        throw new ApiError(
          'feature_disabled',
          'asset-linked work orders need preventive maintenance',
          { details: { feature: FEATURES.preventiveMaintenance } },
        );
      const [asset] = await tx
        .select({ id: schema.assets.id })
        .from(schema.assets)
        .where(and(eq(schema.assets.id, input.assetId), eq(schema.assets.propertyId, property.id)));
    ...
    if (input.estateId && !isFeatureEnabled(identity, FEATURES.estateManagement))
      throw new ApiError('feature_disabled', 'estate issues need estate management', {
...
async function transition(
  ...
  return withActor(getDb(), ctxFor(identity, options), async (tx) => {
    ...
    if (input.allow && !input.allow.includes(viewer))
      throw new ApiError('forbidden', `a ${viewer} cannot move a work order to ${input.to}`);
    assertWriteRight(identity, access, input.to);
    const decision = evaluateTransition(workOrderMachine, {
      from: row.status,
      to: input.to,
      actor: actorKind(identity, viewer),
    ...
    if (decision.rule.permission === 'maintenance.manage')
      assertAllowed(authorizeAny(identity.actor, [{ staff: 'maintenance.manage' }], access.ref));
    if (decision.rule.permission === 'org.change_orders.approve')
      assertAllowed(
        authorizeAny(identity.actor, [{ org: 'org.change_orders.approve' }], access.ref),
      );
...
export function triageWorkOrder(
  ...
  return transition(
    ...
    id,
    {
      ...
      patch: {
        ...(input.priority
          ? { priority: input.priority, slaDueAt: slaDueAt(input.priority, new Date()) }
          : {}),
        ...(input.category ? { category: input.category } : {}),
        ...(input.estimateKobo !== undefined
          ? { estimateKobo: input.estimateKobo ? BigInt(input.estimateKobo) : null }
          : {}),
      },
    },
    options,
      ...
      guard: async (tx, access) => {
        {
          const [staff] = await tx
            ...
            .where(
              and(
                eq(schema.staffRoles.userId, input.assigneeUserId),
          ...
          const [partner] = staff
            ...
            : await tx
                ...
                .where(eq(schema.partnerProfiles.userId, input.assigneeUserId));
          if (!staff && !partner)
            throw new ApiError(
              'validation_failed',
...
export function startWorkOrder(
  ...
  return transition(
    identity,
    id,
    { to: 'in_progress', expectedVersion: input.expectedVersion, allow: ['staff', 'assignee'] },
    options,
  );
}
...
export function approveWorkOrder(
  ...
  return transition(
    ...
    id,
    {
      ...
      allow: ['customer'],
      patch: {
        approvedAmountKobo: BigInt(input.approvedAmountKobo),
        approvedBy: userId,
        approvedAt: new Date(),
      },
    },
...
export async function addEvidence(
  ...
  return withActor(getDb(), ctxFor(identity, options), async (tx) => {
    ...
    const files = await tx
      ...
      .where(inArray(schema.fileObjects.id, input.fileIds));
    for (const fileId of input.fileIds) {
      const file = files.find((f) => f.id === fileId);
      if (!file)
        throw new ApiError('validation_failed', 'file not found', {
      ...
      guard: async (tx, access) => {
        ...
        if (!count)
          throw new ApiError(
            ...
            'attach at least one piece of evidence before completing the work',
          );
      },
    },
    options,
...
export async function verifyWorkOrder(
  ...
  return withActor(getDb(), ctx, async (tx) => {
    ...
    const { row, viewer } = access;
    if (row.version !== input.expectedVersion) throw versionConflict(row.version);
    if (!['staff', 'customer'].includes(viewer))
      throw new ApiError('forbidden', 'only staff or the owner verify completed work');
    assertWriteRight(identity, access, 'verified');
    const decision = evaluateTransition(workOrderMachine, {
      from: row.status,
      to: 'verified',
      actor: actorKind(identity, viewer),
    ...
    const cost = input.costKobo
      ? BigInt(input.costKobo)
      : (row.actualCostKobo ?? row.approvedAmountKobo ?? 0n);
    if (row.approvedAmountKobo !== null && cost > row.approvedAmountKobo && viewer !== 'customer')
      throw new ApiError(
        'validation_failed',
        'cost exceeds the owner-approved amount; the owner must approve the higher cost',
        {
          details: {
            approvedAmountKobo: row.approvedAmountKobo.toString(),
    ...
    if (cost > 0n) {
      ...
      const [supplier] = row.assigneeUserId
        ? await tx
            .select({ organizationId: schema.partnerProfiles.organizationId })
            .from(schema.partnerProfiles)
            .where(eq(schema.partnerProfiles.userId, row.assigneeUserId))
        : [];
        ...
        const posted = await postJournal(
          ...
          maintenanceExpenseRecoverable({
            workOrder: {
              ...
              currency: 'NGN',
              amountKobo: cost,
              supplierOrganizationId: supplier?.organizationId ?? null,
            },
            estateSegment,
          }),
          { postedBy: userId },
...
export function closeWorkOrder(
  ...
  return transition(
    identity,
    id,
    { to: 'closed', expectedVersion: input.expectedVersion, allow: ['staff'] },
    options,
  );
}
...
export async function getWorkOrder(identity: RequestIdentity, id: string): Promise<WorkOrderDto> {
  return withActor(getDb(), identity.ctx, async (tx) =>
    one(tx, (await requireWorkOrder(tx, identity, id)).row),
  );
}
...
export async function listWorkOrders(
  ...
  return withActor(getDb(), identity.ctx, async (tx) => {
    ...
    const personalScope = !staff
      ? or(
          eq(schema.workOrders.reportedByUserId, userId),
          eq(schema.workOrders.assigneeUserId, userId),
          sql`${schema.workOrders.leaseId} IN (SELECT lease_id FROM lease_parties WHERE user_id = ${userId} AND access_status = 'active')`,
        )
      : undefined;
    const rows = await tx
      .select()
      ...
      .where(
        and(
          ...
          !staff
            ? memberScope && identity.actor.memberships.some((m) => m.organizationId === orgId)
              ? or(memberScope, personalScope)
              : personalScope
            : undefined,
          ...
          query.breachedOnly
            ? and(
                lt(schema.workOrders.slaDueAt, now),
          ...
          cursor
            ? or(
                lt(schema.workOrders.createdAt, cursor.createdAt),
                and(
                  eq(schema.workOrders.createdAt, cursor.createdAt),
                  lt(schema.workOrders.id, cursor.id),
                ),
              )
            : undefined,
        ),
    ...
    return {
      items: await toDtos(tx, page, now),
...
export async function generateRecurringWorkOrders(
  ...
  return withActor(db, systemContext('work_orders.recurring'), async (tx) => {
    const [flag] = await tx
      ...
      .where(eq(schema.featureFlags.key, FEATURES.preventiveMaintenance));

...
import {
  ...
  createTender,
  getTender,
  inviteTenderPartners,
  listMyInvitedTenders,
  listTenders,
  publishTender,
  reviseTender,
} from './tenders';
...
describe('tender lifecycle and sealed bids (acceptance scenario 4)', () => {
  it('runs end to end with competitor isolation, sealing and award visibility', async () => {
    ...
    await expect(
      createTender(staffIdentity('tnd-admin', ['super_admin'], { flags: {} }), tenderInput()),
    ).rejects.toMatchObject({ code: 'feature_disabled' });
    await expect(
      createTender(staffIdentity('tnd-ops', ['inspector']), tenderInput()),
    ).rejects.toMatchObject({ name: 'AuthorizationError' });
    await expect(
      createTender(admin, tenderInput({ evaluationWeights: { price: 50, quality: 40 } })),
    ).rejects.toMatchObject({ code: 'validation_failed' });
    await expect(
      createTender(
        admin,
        tenderInput({
          timeline: { releaseAt: hoursFromNow(3), submissionDeadlineAt: hoursFromNow(2) },
        }),
      ),
    ).rejects.toMatchObject({ code: 'validation_failed' });
    ...
    await expect(
      inviteTenderPartners(admin, tender.id, { partnerUserIds: ['tnd-customer-a'] }),
    ).rejects.toMatchObject({ code: 'validation_failed' });
    const invitations = await inviteTenderPartners(admin, tender.id, {
      partnerUserIds: ['tnd-contractor-a', 'tnd-contractor-b'],
    });
    ...
    const outbox = await dbs.owner
      ...
      .where(eq(schema.outboxEvents.eventType, 'tender.published'));
    expect(outbox[0]?.payload).toMatchObject({
      tenderId: tender.id,
      recipientUserIds: ['tnd-contractor-a', 'tnd-contractor-b'],
    });
    ...
    await expect(getTender(partnerIdentity('tnd-contractor-c'), tender.id)).rejects.toMatchObject({
      code: 'not_found',
    });
    ...
    expect((await listMyInvitedTenders(contractorA, { limit: 10 })).items.map((t) => t.id)).toEqual(
      [tender.id],
    );
    expect((await listTenders(customerB, { limit: 10 })).items).toEqual([]);
    expect((await listTenders(customerA, { limit: 10 })).items.map((t) => t.id)).toEqual([
      tender.id,
    ]);

    // Questions: the asker sees their own; competitors only published, anonymised answers.
    const question = await askTenderQuestion(contractorA, tender.id, {
      question: 'Is the perimeter fence in scope?',
    });
    expect(await listTenderQuestions(contractorB, tender.id)).toEqual([]);
    const answered = await answerTenderQuestion(admin, tender.id, question.id, {
      answer: 'Yes, see drawing A-101.',
      publish: true,
    });
    ...
    expect((await listTenderQuestions(contractorA, tender.id))[0]).toMatchObject({
      askedByMe: true,
      askedByUserId: null,
    });
    expect((await listTenderQuestions(admin, tender.id))[0]?.askedByUserId).toBe(
      'tnd-contractor-a',
    );

    // Revisions: a deadline can only move later.
    await expect(
      reviseTender(admin, tender.id, {
        changes: {},
        deadlineExtendedTo: hoursFromNow(1),
        reason: 'shorten',
    ...
    const withContent = await addBidRevision(contractorA, bidA.id, {
      amountKobo: '150000000000',
      currency: 'NGN',
      lineItems: [],
      qualifications: { cac: 'RC123' },
      attachmentFileIds: [],
    });
    expect(withContent.latestRevision?.submittedAt).toBeNull();
    const submittedA = await submitBid(contractorA, bidA.id, {
      clientClaimedTime: '2000-01-01T00:00:00Z',
    });
    ...
    await expect(
      addBidRevision(contractorB, bidA.id, {
        amountKobo: '1',
        currency: 'NGN',
        lineItems: [],
        qualifications: {},
    ...
    expect((await listMyBids(contractorB, { limit: 10 })).items.map((b) => b.id)).toEqual([
      bidB.id,
    ]);
    ...
    await expect(openTenderBids(admin, tender.id, { reason: 'curiosity' })).rejects.toMatchObject({
      code: 'invalid_transition',
    });
    expect(await listTenderBids(customerA, tender.id)).toEqual([]);
    await expect(getTenderComparison(admin, tender.id)).rejects.toMatchObject({
      code: 'forbidden',
      details: { code: 'bids_sealed' },
    });

    // Closing early is refused; after the deadline nothing can be submitted, revised or withdrawn.
    await expect(closeTender(admin, tender.id)).rejects.toMatchObject({
      code: 'invalid_transition',
    });
    await setDeadline(tender.id, new Date(Date.now() - 60_000));
    await expect(
      submitBid(contractorA, bidA.id, {
        revision: {
          amountKobo: '1',
          currency: 'NGN',
          lineItems: [],
    ...
    await expect(
      addBidRevision(contractorA, bidA.id, {
        amountKobo: '1',
        currency: 'NGN',
        lineItems: [],
        qualifications: {},
    ...
    await expect(
      withdrawBid(contractorA, bidA.id, { reason: 'changed my mind' }),
    ).rejects.toMatchObject({ code: 'deadline_passed' });
    await expect(
      askTenderQuestion(contractorA, tender.id, { question: 'Too late question?' }),
    ).rejects.toMatchObject({ code: 'deadline_passed' });
    ...
    await expect(
      startEvaluation(opsManager, tender.id, { reason: 'evaluation start' }),
    ).rejects.toMatchObject({ name: 'AuthorizationError' });
    await expect(
      startEvaluation(adminNoMfa, tender.id, { reason: 'evaluation start' }),
    ).rejects.toMatchObject({ decision: { code: 'mfa_required' } });
    const evaluating = await startEvaluation(admin, tender.id, {
      reason: 'Deadline passed; opening for evaluation panel',
    });
    ...
    expect(
      log
        .filter((l) => l.action === 'open')
        .map((l) => l.bidId)
        .sort(),
    ).toEqual([bidA.id, bidB.id].sort());
    ...
    expect(
      (await dbs.owner.select().from(schema.bidAccessLog)).some(
        (l) => l.action === 'read' && l.bidId === bidA.id && l.userId === 'tnd-admin',
      ),
    ).toBe(true);
    await expect(scoreBid(admin, bidA.id, { scores: { price: 70 } })).rejects.toMatchObject({
      code: 'validation_failed',
    });
    await scoreBid(admin, bidA.id, { scores: { price: 70, quality: 90 } });
    await scoreBid(admin, bidB.id, { scores: { price: 90, quality: 60 } });
    await scoreBid(opsManager, bidB.id, {
      scores: { price: 80, quality: 60 },
      notes: 'strong price',
    });
    ...
    expect(
      (await dbs.owner.select().from(schema.bidAccessLog)).some(
        (l) => l.action === 'read' && l.userId === 'tnd-customer-a',
      ),
    ).toBe(true);
    ...
    const award = await decideAward(admin, tender.id, {
      bidId: bidB.id,
      notes: 'Best value',
    ...
    const awardedRows = await dbs.owner
    ...
    const publishedAward = await publishAward(admin, tender.id, {
      expectedVersion: afterDecide.version,
    });
    ...
    const jobs = await dbs.owner
      .select()
      .from(schema.jobs)
      .where(eq(schema.jobs.type, 'tenders.notify_award'));
    expect(jobs).toHaveLength(1);
    await expect(
      respondToAward(contractorA, tender.id, { decision: 'accept' }),
    ).rejects.toMatchObject({ code: 'not_found' });
    expect((await respondToAward(contractorB, tender.id, { decision: 'accept' })).status).toBe(
      'accepted',
    );
  });
});

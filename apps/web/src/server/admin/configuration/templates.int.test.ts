...
import { computeSlaDueAt, createFinanceRuntime, createQuote, type FinanceActor } from '@simplexd/finance';
...
beforeAll(async () => {
  ...
  await dbs.owner.insert(schema.organization).values({ id: orgId, name: 'Template org', slug: orgId });
  await dbs.owner
    .insert(schema.member)
    .values({ id: `m_${customerId}`, organizationId: orgId, userId: customerId, role: 'owner' });
  await dbs.owner.insert(schema.ledgerAccounts).values(chartOfAccounts).onConflictDoNothing({ target: schema.ledgerAccounts.code });
});
...
describe('quote templates', () => {
  it('lets pricing managers create and edit templates, quote issuers read them, and nobody else', async () => {
    ...
    await expect(
      createQuoteTemplate(pm, { serviceId, name: 'PM template', lines: [{ description: 'x', quantity: '1', unitAmountKobo: '100' }], scopeMarkdown: null, exclusions: null, active: true }),
    ).rejects.toBeInstanceOf(AuthorizationError);
    await expect(
      createQuoteTemplate(ops, { serviceId, name: 'Negative', lines: [{ description: 'x', quantity: '1', unitAmountKobo: '-100' }], scopeMarkdown: null, exclusions: null, active: true }),
    ...
    expect((await listQuoteTemplates(pm, { serviceId: otherServiceId })).map((x) => x.id)).not.toContain(t.id);

    await expect(
      patchQuoteTemplate(ops, t.id, { name: 'Renamed', reason: 'tidy up', expectedUpdatedAt: '2020-01-01T00:00:00.000Z' }),
    ...
    const audit = await dbs.owner.select().from(schema.auditEvents).where(eq(schema.auditEvents.entityId, t.id));
    expect(audit.map((a) => a.action)).toEqual(expect.arrayContaining(['quote_template.created', 'quote_template.updated']));
  });
  ...
  it('pre-fills a quote from a template while the quote keeps its own edited lines', async () => {
    ...
    const [foreign] = await dbs.owner
      ...
      .values({ serviceId: otherServiceId, name: 'Foreign', lines: [{ description: 'y', quantity: '1', unitAmountKobo: '100', amountKobo: '100' }] })
    ...
    const fa: FinanceActor = { actor: ops.identity.actor, ctx: ops.identity.ctx, correlationId: 'test' };

    await expect(
      createQuote(rt, fa, sr!.id, { templateId: foreign!.id, currency: 'NGN', depositBps: 10_000, requiresPayment: true }),
    ).rejects.toMatchObject({ code: 'not_found' });

    // Staff started from the template and changed the second line before saving.
    const edited = [
      { description: tpl!.lines[0]!.description, quantity: '1', unitAmountKobo: tpl!.lines[0]!.unitAmountKobo },
    ...
    expect(quote.versions[0]?.lines.map((l) => l.description)).toEqual(['Document review', 'Site visit (reduced)']);
    ...
    await patchQuoteTemplate(ops, tpl!.id, {
      ...
      expectedUpdatedAt: (await listQuoteTemplates(ops, { serviceId })).find((x) => x.id === tpl!.id)!.updatedAt,
    });
    const [version] = await dbs.owner.select().from(schema.quoteVersions).where(eq(schema.quoteVersions.quoteId, quote.id));
    expect(version?.lines.map((l) => l.description)).toEqual(['Document review', 'Site visit (reduced)']);

    // With no lines, the template's lines are used.
    const fromTemplate = await createQuote(rt, fa, sr!.id, { templateId: tpl!.id, currency: 'NGN', depositBps: 10_000, requiresPayment: true });
...
describe('report templates', () => {
  ...
  it('keeps exactly one active template per kind and records who activated what', async () => {
    ...
    await expect(
      createReportTemplate(inspector, { kind: 'valuation', name: 'x', sections, limitationsMarkdown: 'No guarantee is given.', active: true }),
    ).rejects.toBeInstanceOf(AuthorizationError);
    ...
    const activeNow = async () =>
      dbs.owner
        ...
        .where(and(eq(schema.reportTemplates.kind, 'valuation'), eq(schema.reportTemplates.active, true)));
    expect((await activeNow()).map((r) => r.id)).toEqual([t2.id]);
    expect((await activeReportTemplate(dbs.owner, 'valuation'))?.id).toBe(t2.id);

    const t1Now = (await listReportTemplates(inspector, { kind: 'valuation' })).find((t) => t.id === t1.id)!;
    expect(t1Now.active).toBe(false);
    expect(t1Now.version).toBe(t1.version + 1);
    await expect(
      patchReportTemplate(pm, t1.id, { active: true, expectedVersion: t1.version, reason: 'stale' }),
    ).rejects.toMatchObject({ code: 'version_conflict' });
    await patchReportTemplate(pm, t1.id, { active: true, expectedVersion: t1Now.version, reason: 'switch back' });
    expect((await activeNow()).map((r) => r.id)).toEqual([t1.id]);
    await expect(
      patchReportTemplate(pm, t1.id, { sections: [{ key: 'a', heading: 'A', required: true }, { key: 'a', heading: 'B', required: false }], expectedVersion: t1Now.version + 1, reason: 'dup' }),
  ...
  it('seeds one active template per core report kind and requirements per core service, idempotently', async () => {
    ...
    for (const kind of ['progress', 'inspection', 'virtual_inspection', 'diligence_memo', 'closing_pack', 'search_outcome'] as const) {
...
describe('document requirements', () => {
  it('filters by service, stage and sensitivity for customers and the public', async () => {
    await expect(
      createDocumentRequirement(pm, { serviceId, name: 'x', description: null, stage: null, required: true, sensitive: false, active: true, sortOrder: 0 }),
    ).rejects.toBeInstanceOf(AuthorizationError);
    await expect(
      createDocumentRequirement(ops, { serviceId, name: 'Closed stage', description: null, stage: 'completed', required: true, sensitive: false, active: true, sortOrder: 0 }),
    ).rejects.toMatchObject({ code: 'validation_failed' });

    const title = await createDocumentRequirement(ops, { serviceId, name: `Title copy ${sfx}`, description: 'For searches', stage: 'triage', required: true, sensitive: false, active: true, sortOrder: 10 });
    const id = await createDocumentRequirement(ops, { serviceId, name: `Photo ID ${sfx}`, description: 'Only if the registry asks', stage: 'in_progress', required: false, sensitive: true, active: true, sortOrder: 20 });
    const global = await createDocumentRequirement(ops, { serviceId: null, name: `Photos ${sfx}`, description: null, stage: null, required: false, sensitive: false, active: true, sortOrder: 1 });
    const inactive = await createDocumentRequirement(ops, { serviceId, name: `Old form ${sfx}`, description: null, stage: null, required: true, sensitive: false, active: false, sortOrder: 2 });
    await createDocumentRequirement(ops, { serviceId: otherServiceId, name: `Survey ${sfx}`, description: null, stage: 'triage', required: true, sensitive: false, active: true, sortOrder: 1 });
    ...
    const atTriage = requirementsForStage(all, { serviceId, stage: 'triage', includeSensitive: true });
    ...
    expect(requirementsForStage(all, { serviceId, stage: paused, includeSensitive: true }).now.map((r) => r.id)).toEqual([global.id, title.id, id.id]);

    const listed = await listDocumentRequirements(pm, { serviceId: 'all' });
    expect(listed.every((r) => r.serviceId === null)).toBe(true);
    const patched = await patchDocumentRequirement(ops, id.id, { required: true, expectedVersion: id.version, reason: 'registry now insists' });
    expect(patched.required).toBe(true);
    expect(patched.version).toBe(id.version + 1);
    await expect(
      patchDocumentRequirement(ops, id.id, { required: false, expectedVersion: id.version, reason: 'stale' }),
    ).rejects.toMatchObject({ code: 'version_conflict' });
  });
});
...
describe('SLA policies', () => {
  it('allows one active policy per service and stage and triage prefers the service-specific one', async () => {
    await expect(
      createSlaPolicy(support, { serviceId: null, stage: 'triage', targetHours: 1, businessHoursOnly: true, escalateToRole: null, active: true }),
    ).rejects.toBeInstanceOf(AuthorizationError);
    const specific = await createSlaPolicy(ops, { serviceId, stage: 'triage', targetHours: 8, businessHoursOnly: false, escalateToRole: 'operations_manager', active: true });
    await expect(
      createSlaPolicy(ops, { serviceId, stage: 'triage', targetHours: 9, businessHoursOnly: false, escalateToRole: null, active: true }),
    ).rejects.toMatchObject({ code: 'conflict' });
    const spare = await createSlaPolicy(ops, { serviceId, stage: 'triage', targetHours: 9, businessHoursOnly: false, escalateToRole: null, active: false });
    await expect(
      patchSlaPolicy(ops, spare.id, { active: true, reason: 'swap', expectedUpdatedAt: spare.updatedAt }),
    ).rejects.toMatchObject({ code: 'conflict' });
    // A global triage policy may already exist in the shared database; make sure one is present.
    const globals = (await listSlaPolicies(pm)).filter((p) => p.serviceId === null && p.stage === 'triage' && p.active);
    if (globals.length === 0) {
      await createSlaPolicy(ops, { serviceId: null, stage: 'triage', targetHours: 48, businessHoursOnly: true, escalateToRole: null, active: true });
    }
    ...
    const otherDue = await dbs.owner.transaction((tx) => computeSlaDueAt(tx, otherServiceId, 'triage', now));
    expect(otherDue.policyId).not.toBe(specific.id);

    const off = await patchSlaPolicy(ops, specific.id, { active: false, reason: 'retire', expectedUpdatedAt: specific.updatedAt });
    expect(off.active).toBe(false);
    const on = await patchSlaPolicy(ops, spare.id, { active: true, reason: 'swap', expectedUpdatedAt: spare.updatedAt });
    expect(on.active).toBe(true);
    const after = await dbs.owner.transaction((tx) => computeSlaDueAt(tx, serviceId, 'triage', now));

...
beforeAll(async () => {
  ...
  const [p1, p2, p3] = await owner
    ...
    .values([
      ...
      { organizationId: orgId, name: `P2 ${sfx}`, kind: 'commercial' },
      {
        ...
        name: `P3 ${sfx}`,
        kind: 'land',
        archivedAt: new Date(today.getTime() - 86_400_000),
      },
    ])
  ...
  const lease = (
    await owner
      ...
      .from(schema.leases)
      .where(eq(schema.leases.propertyId, p1!.id))
  )[0]!;
  const invoiceBase = {
    organizationId: orgId,
    leaseId: lease.id,
    isRentOnBehalfOfOwner: true,
    issuedAt: new Date(today.getTime() - 60 * 86_400_000),
  };
    ...
    .values([
      {
        ...
        subtotalKobo: 100_000n,
        totalKobo: 100_000n,
        amountPaidKobo: 30_000n,
  ...
  const [bv] = await owner
    ...
    .values({
      ...
      contingencyKobo: 100_000n,
    })
    .returning({ id: schema.budgetVersions.id });
  await owner
    .update(schema.projects)
    .set({ approvedBudgetVersionId: bv!.id })
    .where(eq(schema.projects.id, pr1!.id));
  ...
  await owner.insert(schema.serviceRequests).values([
    {
      reference: `SRA-${sfx}-1`,
      organizationId: orgId,
      requestedByUserId: users.cust.id,
      serviceId,
      title: 'a',
      status: 'triage',
      slaDueAt: new Date(today.getTime() - 3_600_000),
    },
    {
      reference: `SRA-${sfx}-2`,
      organizationId: orgId,
    ...
    },
    {
      ...
      title: 'c',
      status: 'in_progress',
      slaDueAt: new Date(today.getTime() + 2 * 3_600_000),
    },
    {
      reference: `SRA-${sfx}-4`,
      organizationId: orgId,
  ...
  await postJournal(
    `pa:${sfx}:4`,
    new Date(Date.UTC(today.getUTCFullYear() - 3, 0, 1)),
    orgId,
    '4000',
    9_999n,
  );
...
describe('portfolio analytics', () => {
  it('refuses actors without any analytics permission', async () => {
    ...
    await expect(exportPortfolioSection(support, { section: 'properties' })).rejects.toBeInstanceOf(
      AuthorizationError,
    );
  });
  ...
  it('derives every figure from the fixtures for one organisation', async () => {
    ...
    expect(data.changeOrders).toEqual({
      ...
      byStatus: {
        submitted: { count: 1, deltaKobo: '50000' },
        draft: { count: 1, deltaKobo: '10000' },
      },
    });
    ...
    expect(data.serviceRequests?.byService).toEqual([
      { serviceId, serviceName: `Analytics service ${sfx}`, total: 4, open: 3 },
    ]);
  ...
  it('returns only the sections each role may see, and scopes project managers to their own work', async () => {
    ...
    expect(mine.projects).toMatchObject({
      scope: 'assigned',
      activeCount: 1,
      withApprovedBudget: 1,
    });
    expect(mine.changeOrders).toMatchObject({ scope: 'assigned', openCount: 1 });
    expect(mine.serviceRequests).toMatchObject({
      scope: 'assigned',
      createdInRange: 1,
      byStatus: { inquiry: 1 },
    });
  ...
  it('exports the rows a figure was summed from, audits the export and gates finance sections behind MFA', async () => {
    const revenue = await exportPortfolioSection(
      finance,
      { section: 'revenue', organizationId: orgId },
      today,
    );
    const rows = parseCsv(revenue.csv);
    expect(rows).toHaveLength(2);
    expect(rows.reduce((n, r) => n + BigInt(r.revenue_kobo!), 0n)).toBe(160_000n);
    expect(revenue.filename).toMatch(
      /^simplexd-portfolio-revenue-\d{4}-\d{2}-\d{2}_\d{4}-\d{2}-\d{2}\.csv$/,
    );

    const arrears = await exportPortfolioSection(
      finance,
      { section: 'arrears', organizationId: orgId },
      today,
    );
    ...
    const units = await exportPortfolioSection(
      ops,
      { section: 'occupancy', organizationId: orgId },
      today,
    );
    expect(parseCsv(units.csv).filter((r) => r.occupied_as_of === '1')).toHaveLength(1);

    const projects = await exportPortfolioSection(
      pm,
      { section: 'projects', organizationId: orgId },
      today,
    );
    ...
    await expect(
      exportPortfolioSection(pm, { section: 'revenue', organizationId: orgId }, today),
    ).rejects.toBeInstanceOf(AuthorizationError);
    // Operations managers read finance figures but do not hold finance.export.
    await expect(
      exportPortfolioSection(ops, { section: 'revenue', organizationId: orgId }, today),
    ).rejects.toMatchObject({ decision: { code: 'no_permission' } });
    ...
    expect(
      audits.some(
        (a) =>
          (a.after as { section?: string; organizationId?: string })?.organizationId === orgId &&
          (a.after as { section?: string }).section === 'revenue',
      ),
    ).toBe(true);
  });
  ...
  it('validates the range', async () => {
    await expect(
      portfolioAnalytics(ops, { from: '2026-09-02', to: '2026-09-01' }),
    ).rejects.toMatchObject({ code: 'validation_failed' });
    await expect(
      portfolioAnalytics(ops, { from: '2010-01-01', to: '2026-09-01' }),
    ).rejects.toMatchObject({ code: 'validation_failed' });
    const r = resolveRange(
      { from: '2026-07-10', to: '2026-09-05' },

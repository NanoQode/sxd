...
describe('notes', () => {
  it('keeps internal notes invisible to the customer organisation that owns the entity', async () => {
    ...
    const internal = await createNote(ops, {
      entityType: 'service_request',
      entityId: f.serviceRequestA,
      body: 'Client is difficult',
      visibility: 'internal',
    });
    const customer = await createNote(ops, {
      entityType: 'service_request',
      entityId: f.serviceRequestA,
      body: 'Documents received',
      visibility: 'customer',
    });
    const partner = await createNote(ops, {
      entityType: 'service_request',
      entityId: f.serviceRequestA,
      body: 'Survey brief',
      visibility: 'partner',
    });
    const all = await createNote(ownerA, {
      entityType: 'service_request',
      entityId: f.serviceRequestA,
      body: 'Thanks',
      visibility: 'all',
    ...
    const customerView = await listNotes(ownerA, {
      entityType: 'service_request',
      entityId: f.serviceRequestA,
      limit: 25,
    });
    ...
    const partnerView = await listNotes(partnerIdentity(f), {
      entityType: 'service_request',
      entityId: f.serviceRequestA,
      limit: 25,
    });
    ...
    const staffView = await listNotes(ops, {
      entityType: 'service_request',
      entityId: f.serviceRequestA,
      limit: 25,
    });
  ...
  it('derives write rights from the parent entity', async () => {
    ...
    await expect(
      createNote(customerIdentity(f, 'B'), {
        entityType: 'service_request',
        entityId: f.serviceRequestA,
        body: 'x',
        visibility: 'customer',
    ...
    await expect(
      listNotes(customerIdentity(f, 'B'), {
        entityType: 'service_request',
        entityId: f.serviceRequestA,
        limit: 25,
      }),
    ...
    const partnerNote = await createNote(partnerIdentity(f), {
      entityType: 'service_request',
      entityId: f.serviceRequestA,
      body: 'On site tomorrow',
      visibility: 'partner',
    });
    expect(partnerNote.visibility).toBe('partner');
    await expect(
      createNote(partnerIdentity(f), {
        entityType: 'service_request',
        entityId: f.serviceRequestA,
        body: 'x',
        visibility: 'customer',
    ...
    const page2 = await listNotes(ownerA, {
      entityType: 'property',
      entityId: prop!.id,
      limit: 1,

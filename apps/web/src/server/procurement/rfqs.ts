...
async function loadItems(tx: DbExecutor, rfqId: string) {
  return tx.select().from(schema.rfqItems).where(eq(schema.rfqItems.rfqId, rfqId)).orderBy(asc(schema.rfqItems.sortOrder), asc(schema.rfqItems.id));
}

async function marketName(tx: DbExecutor, marketId: string | null): Promise<string | null> {
  if (!marketId) return null;
  const [m] = await tx.select({ name: schema.markets.name }).from(schema.markets).where(eq(schema.markets.id, marketId));
  return m?.name ?? null;
}

async function assertLinks(tx: DbExecutor, input: { organizationId: string; projectId?: string | null; deliveryMarketId?: string | null }): Promise<void> {
  ...
  if (input.projectId) {
    const [project] = await tx.select({ organizationId: schema.projects.organizationId }).from(schema.projects).where(eq(schema.projects.id, input.projectId));
  ...
  if (input.deliveryMarketId && (await marketName(tx, input.deliveryMarketId)) === null) {
    throw new ApiError('validation_failed', 'delivery market not found', { details: { field: 'deliveryMarketId' } });
  }
}
...
export async function createRfq(identity: RequestIdentity, input: RfqCreate, options: ServiceOptions = {}): Promise<RfqDetail> {
  const userId = requireProcurement(identity);
  assertAllowed(authorizeStaff(identity.actor, 'procurement.manage', { type: 'rfq', organizationId: input.organizationId }));
...
async function loadDraft(tx: DbExecutor, identity: RequestIdentity, rfqId: string): Promise<RfqAccess> {
  ...
  if (access.rfq.status !== 'draft') {
    throw new ApiError('invalid_transition', 'only draft RFQs can be edited', { details: { status: access.rfq.status } });
  }
  return access;
}
...
export async function updateRfq(
  ...
  rfqId: string,
  input: { title?: string; projectId?: string | null; deliveryMarketId?: string | null; deliveryAddress?: Record<string, string> | null; notes?: string | null },
  ...
  return withActor(getDb(), ctxFor(identity, options), async (tx) => {
    ...
    await assertLinks(tx, {
      ...
      deliveryMarketId: input.deliveryMarketId === undefined ? access.rfq.deliveryMarketId : input.deliveryMarketId,
    });
    ...
    if (Object.keys(patch).length > 0) await tx.update(schema.rfqs).set(patch).where(eq(schema.rfqs.id, rfqId));
    await recordAudit(tx, identity, {
      action: 'rfq.updated',
      entityType: 'rfq',
      entityId: rfqId,
...
export async function replaceRfqItems(identity: RequestIdentity, rfqId: string, input: { items: RfqItemInput[] }, options: ServiceOptions = {}): Promise<RfqDetail> {
  requireProcurement(identity);
  return withActor(getDb(), ctxFor(identity, options), async (tx) => {
    const access = await loadDraft(tx, identity, rfqId);
...
async function assertPartnerProfiles(tx: DbExecutor, ids: string[]): Promise<void> {
  ...
  const rows = await tx.select({ userId: schema.partnerProfiles.userId }).from(schema.partnerProfiles).where(inArray(schema.partnerProfiles.userId, ids));
  const known = new Set(rows.map((r) => r.userId));
  const missing = ids.filter((id) => !known.has(id));
  if (missing.length > 0) throw new ApiError('validation_failed', 'every supplier must have a partner profile', { details: { missing } });
}

/** Invitations are draft response rows naming the supplier; that row is what grants the supplier access. */
async function inviteInTx(tx: DbExecutor, rfq: RfqRow, supplierUserIds: string[], currency = 'NGN'): Promise<string[]> {
  ...
  const existing = await tx
    ...
    .where(and(eq(schema.rfqResponses.rfqId, rfq.id), inArray(schema.rfqResponses.supplierUserId, ids)));
  const have = new Set(existing.map((e) => e.supplierUserId));
  const fresh = ids.filter((id) => !have.has(id));
  if (fresh.length > 0) {
    const empty: ResponseLinesJson = { version: 1, deliveryKobo: '0', leadTimeDays: null, lines: [] };
    await tx.insert(schema.rfqResponses).values(fresh.map((supplierUserId) => ({ rfqId: rfq.id, supplierUserId, status: 'draft' as const, lines: empty, currency })));
  }
  return fresh;
}

export async function issueRfq(identity: RequestIdentity, rfqId: string, input: { deadlineAt: string; supplierUserIds: string[] }, options: ServiceOptions = {}): Promise<RfqDetail> {
  const userId = requireProcurement(identity);
  return withActor(getDb(), ctxFor(identity, options), async (tx) => {
    const access = await loadDraft(tx, identity, rfqId);
    const items = await loadItems(tx, rfqId);
    if (items.length === 0) throw new ApiError('validation_failed', 'add at least one item before issuing the RFQ');
    const now = await dbNow(tx);
    const deadline = new Date(input.deadlineAt);
    if (deadline.getTime() <= now.date.getTime()) {
      throw new ApiError('validation_failed', 'the response deadline must be in the future', { details: { serverNow: now.iso } });
    }
    await tx.update(schema.rfqs).set({ status: 'sent', deadlineAt: deadline }).where(eq(schema.rfqs.id, rfqId));
...
async function supplierIds(tx: DbExecutor, rfqId: string): Promise<string[]> {
  const rows = await tx.select({ id: schema.rfqResponses.supplierUserId }).from(schema.rfqResponses).where(eq(schema.rfqResponses.rfqId, rfqId));
  return rows.map((r) => r.id).filter((id): id is string => id !== null);
}

export async function inviteRfqSuppliers(identity: RequestIdentity, rfqId: string, input: { supplierUserIds: string[] }, options: ServiceOptions = {}): Promise<RfqDetail> {
  const userId = requireProcurement(identity);
  return withActor(getDb(), ctxFor(identity, options), async (tx) => {
    const access = await loadRfqAccess(tx, identity, rfqId, { customer: false });
    if (access.role !== 'staff') throw new ApiError('forbidden', 'staff action');
    if (access.rfq.status !== 'draft' && access.rfq.status !== 'sent') {
      throw new ApiError('invalid_transition', 'suppliers can only be invited before the RFQ closes', { details: { status: access.rfq.status } });
    }
    const invited = await inviteInTx(tx, access.rfq, input.supplierUserIds);
    if (invited.length > 0 && access.rfq.status === 'sent') {
      await appendOutbox(tx, {
        ...
        organizationId: access.rfq.organizationId,
        actorUserId: userId,
        payload: { rfqId, deadlineAt: access.rfq.deadlineAt?.toISOString() ?? null, recipientUserIds: invited },
        correlationId: options.correlationId,
      });
    }
    await recordAudit(tx, identity, {
...
export async function closeRfq(identity: RequestIdentity, rfqId: string, options: ServiceOptions = {}): Promise<RfqDetail> {
  requireProcurement(identity);
  return withActor(getDb(), ctxFor(identity, options), async (tx) => {
    const access = await loadRfqAccess(tx, identity, rfqId, { customer: false });
    if (access.role !== 'staff') throw new ApiError('forbidden', 'staff action');
    if (access.rfq.status !== 'sent') throw new ApiError('invalid_transition', 'only an issued RFQ can be closed', { details: { status: access.rfq.status } });
...
export async function cancelRfq(identity: RequestIdentity, rfqId: string, input: { reason: string }, options: ServiceOptions = {}): Promise<RfqDetail> {
  requireProcurement(identity);
  return withActor(getDb(), ctxFor(identity, options), async (tx) => {
    const access = await loadRfqAccess(tx, identity, rfqId, { customer: false });
    if (access.role !== 'staff') throw new ApiError('forbidden', 'staff action');
 ...
 */
export async function submitRfqResponse(identity: RequestIdentity, rfqId: string, input: RfqResponseSubmit, options: ServiceOptions = {}): Promise<RfqResponseDto> {
  const userId = requireProcurement(identity);
  return withActor(getDb(), ctxFor(identity, options), async (tx) => {
    const access = await loadRfqAccess(tx, identity, rfqId, { customer: false });
    const { rfq } = access;
    if (rfq.status !== 'sent') throw new ApiError('invalid_transition', 'the RFQ is not open for responses', { details: { status: rfq.status } });
    const items = (await loadItems(tx, rfqId)).map(toRfqItemDto);
    const itemIds = new Set(items.map((i) => i.id));
    const seen = new Set<string>();
    for (const line of input.lines) {
      if (!itemIds.has(line.itemId)) throw new ApiError('validation_failed', 'line item does not belong to this RFQ', { details: { itemId: line.itemId } });
      if (seen.has(line.itemId)) throw new ApiError('validation_failed', 'duplicate line for an item', { details: { itemId: line.itemId } });
    ...
    if (access.role === 'supplier') {
      ...
      if (rfq.deadlineAt && rfq.deadlineAt.getTime() <= now.date.getTime()) {
        throw new ApiError('deadline_passed', 'the response deadline has passed', { details: { deadlineAt: rfq.deadlineAt.toISOString(), serverNow: now.iso } });
      }
      ...
      const [row] = await tx
        .select()
        .from(schema.rfqResponses)
        .where(and(eq(schema.rfqResponses.rfqId, rfqId), eq(schema.rfqResponses.supplierUserId, input.supplierUserId)));
      target = row ?? null;
    } else if (!input.supplierName) {
      throw new ApiError('validation_failed', 'a manual response needs supplierName or supplierUserId');
    }
    ...
    if (input.supplierFacilityId) {
      const [facility] = await tx.select({ id: schema.supplyFacilities.id }).from(schema.supplyFacilities).where(eq(schema.supplyFacilities.id, input.supplierFacilityId));
      if (!facility) throw new ApiError('validation_failed', 'supplier facility not found', { details: { field: 'supplierFacilityId' } });
    }
    ...
    const values = {
      ...
      supplierFacilityId: input.supplierFacilityId === undefined ? (target?.supplierFacilityId ?? null) : input.supplierFacilityId,
    ...
    if (target) {
      const [row] = await tx.update(schema.rfqResponses).set(values).where(eq(schema.rfqResponses.id, target.id)).returning();
      saved = row!;
    } else {
      const [row] = await tx
    ...
    const [withTotal] = await tx.update(schema.rfqResponses).set({ totalDeliveredKobo: total }).where(eq(schema.rfqResponses.id, saved.id)).returning();
    if (input.submit) {
      await appendOutbox(tx, {
    ...
    await recordAudit(tx, identity, {
      ...
      entityId: saved.id,
      organizationId: rfq.organizationId,
      after: { rfqId, lines: lines.lines.length, deliveryKobo: input.deliveryKobo, totalDeliveredKobo: total?.toString() ?? null, onBehalfOf: input.supplierUserId ?? input.supplierName ?? null },
      correlationId: options.correlationId,
    });
...
export async function withdrawRfqResponse(identity: RequestIdentity, rfqId: string, responseId: string, options: ServiceOptions = {}): Promise<RfqResponseDto> {
  requireProcurement(identity);
  return withActor(getDb(), ctxFor(identity, options), async (tx) => {
    const access = await loadRfqAccess(tx, identity, rfqId, { customer: false });
    const [row] = await tx.select().from(schema.rfqResponses).where(and(eq(schema.rfqResponses.id, responseId), eq(schema.rfqResponses.rfqId, rfqId)));
    if (!row || (access.role === 'supplier' && row.id !== access.ownResponse?.id)) throw new ApiError('not_found', 'response not found');
    if (row.status !== 'draft' && row.status !== 'submitted') throw new ApiError('invalid_transition', `a ${row.status} response cannot be withdrawn`);
    const [updated] = await tx.update(schema.rfqResponses).set({ status: 'withdrawn' }).where(eq(schema.rfqResponses.id, responseId)).returning();
    await recordAudit(tx, identity, {
...
async function detailInTx(tx: Transaction | DbExecutor, identity: RequestIdentity, access: RfqAccess): Promise<RfqDetail> {
  const { rfq } = access;
  ...
  const responses =
    access.role === 'supplier'
      ? access.ownResponse
        ? [access.ownResponse]
        : []
      : await tx.select().from(schema.rfqResponses).where(eq(schema.rfqResponses.rfqId, rfq.id)).orderBy(asc(schema.rfqResponses.createdAt));
  return {
...
export async function getRfq(identity: RequestIdentity, rfqId: string, options: ServiceOptions = {}): Promise<RfqDetail> {
  requireProcurement(identity);
  return withActor(getDb(), ctxFor(identity, options), async (tx) => detailInTx(tx, identity, await loadRfqAccess(tx, identity, rfqId)));
}

/** Staff list every RFQ; customers their organisation's; suppliers the RFQs whose response rows name them. */
export async function listRfqs(identity: RequestIdentity, query: RfqListQuery, options: ServiceOptions = {}): Promise<Page<RfqDto>> {
  const userId = requireProcurement(identity);
  ...
  return withActor(getDb(), ctxFor(identity, options), async (tx) => {
    const supplierRfqIds = supplierOnly
      ? (await tx.select({ rfqId: schema.rfqResponses.rfqId }).from(schema.rfqResponses).where(eq(schema.rfqResponses.supplierUserId, userId))).map((r) => r.rfqId)
      : null;
    ...
    const rows = await tx
      ...
      .where(
        and(
          ...
          cursor ? or(lt(schema.rfqs.createdAt, cursor.createdAt), and(eq(schema.rfqs.createdAt, cursor.createdAt), lt(schema.rfqs.id, cursor.id))) : undefined,
        ),
      )
    ...
    return { items: page.map((r) => toRfqDto(r.rfq, r.marketName)), nextCursor: last ? encodeCursor(last.rfq.createdAt, last.rfq.id) : null };
  });
}

/** Delivered-cost comparison across submitted responses (staff and customers). */
export async function getRfqComparison(identity: RequestIdentity, rfqId: string, options: ServiceOptions = {}): Promise<RfqComparisonDto> {
  requireProcurement(identity);
  return withActor(getDb(), ctxFor(identity, options), async (tx) => {
    const access = await loadRfqAccess(tx, identity, rfqId);
    if (access.role === 'supplier') throw new ApiError('forbidden', 'suppliers do not see competing responses');
    const items = (await loadItems(tx, rfqId)).map(toRfqItemDto);

...
beforeAll(async () => {
  ...
  const [lagos] = await dbs.owner
    .select({ id: schema.markets.id })
    .from(schema.markets)
    .where(eq(schema.markets.slug, 'ng-lagos'));
  lagosId = lagos!.id;
});
...
describe('supplier directory', () => {
  it('lists seeded facilities with their evidence labels and never invents a price', async () => {
    const directory = await listSupplierDirectory(staff, {
      material: 'cement',
    ...
    await expect(
      listSupplierDirectory(customer, { includeArchived: 'false' }),
...
describe('RFQ → responses → comparison → purchase order → delivery → discrepancy', () => {
  it('compares delivered cost honestly and isolates vendors', async () => {
    ...
    await expect(
      submitRfqResponse(vendorA, rfq.id, {
        currency: 'NGN',
        lines: [],
        deliveryKobo: '0',
        submit: true,
    ...
    const issued = await issueRfq(staff, rfq.id, {
      deadlineAt: hoursFromNow(24),
      supplierUserIds: ['prc-vendor-a', 'prc-vendor-b'],
    });
    ...
    const outbox = await dbs.owner
      ...
      .where(eq(schema.outboxEvents.eventType, 'rfq.issued'));
    expect(outbox[0]?.payload).toMatchObject({
      rfqId: rfq.id,
      recipientUserIds: ['prc-vendor-a', 'prc-vendor-b'],
    });
    ...
    await expect(withdrawRfqResponse(vendorA, rfq.id, responseB.id)).rejects.toMatchObject({
      code: 'not_found',
    });
    ...
    expect(entryM).toMatchObject({
      fullyComparable: true,
      totalDeliveredKobo: '157000000',
      rank: 1,
    });
    expect(entryB).toMatchObject({ fullyComparable: false, totalDeliveredKobo: null, rank: null });
    expect(entryB.unknowns).toEqual([
      { itemId: sand!.id, reason: 'unit_conversion_unknown', message: expect.any(String) },
    ]);
    ...
    await expect(
      createPurchaseOrder(staff, { responseId: responseB.id, lineConversions: [] }),
    ...
    const poEvents = await dbs.owner
      .select()
      .from(schema.outboxEvents)
      .where(eq(schema.outboxEvents.eventType, 'purchase_order.issued'));
    expect(poEvents[0]?.payload).toMatchObject({
      purchaseOrderId: po.id,
      recipientUserIds: ['prc-vendor-a'],
    });

    // Vendors see only orders naming them.
    expect((await listPurchaseOrders(vendorA, { limit: 10 })).items.map((p) => p.id)).toEqual([
      po.id,
    ]);
    expect((await listPurchaseOrders(vendorB, { limit: 10 })).items).toEqual([]);
    await expect(acknowledgePurchaseOrder(vendorB, po.id, {})).rejects.toMatchObject({
      code: 'not_found',
    });
    const acknowledged = await acknowledgePurchaseOrder(vendorA, po.id, {
      supplierRef: 'SO-771',
      expectedVersion: issuedPo.version,
    });
    expect(acknowledged.status).toBe('acknowledged');
    expect(
      (await listPurchaseOrders(customer, { limit: 10 })).items.map((p) => p.id).sort(),
    ).toEqual([po.id, poFromB.id].sort());
    ...
    expect(progress.deliveryProgress.lines[0]).toMatchObject({
      lineId: 'L1',
      outstanding: '5',
      status: 'short',
    });
    const discrepancy = await openDiscrepancy(staff, delivery.id, {
      lineId: 'L1',
      kind: 'short_delivery',
      description: '5 m3 of sand missing',
      quantity: '5',
    });
    ...
    expect((await listDeliveries(vendorA, { limit: 10 })).items.map((d) => d.id)).toEqual([
      delivery.id,
    ]);
    expect((await listDeliveries(vendorB, { limit: 10 })).items).toEqual([]);
    await expect(acceptDelivery(staff, delivery.id)).rejects.toMatchObject({
      code: 'invalid_transition',
    });
    await expect(
      transitionDiscrepancy(staff, delivery.id, discrepancy.id, {
        to: 'resolved',
        resolution: 'skip',
      }),
    ).rejects.toMatchObject({ code: 'invalid_transition' });
    await transitionDiscrepancy(staff, delivery.id, discrepancy.id, { to: 'supplier_notified' });
    const credited = await transitionDiscrepancy(staff, delivery.id, discrepancy.id, {
      to: 'credited',
      resolution: 'Supplier credited 5 m3 on the next invoice',
    });
    expect(credited.resolvedAt).not.toBeNull();
    expect((await acceptDelivery(staff, delivery.id)).status).toBe('accepted');
    const disputeEvents = await dbs.owner
      .select()
      .from(schema.outboxEvents)
      .where(eq(schema.outboxEvents.eventType, 'delivery.discrepancy.opened'));

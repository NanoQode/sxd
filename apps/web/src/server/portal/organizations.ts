...
function normalizeRole(role: string): OrganizationMembershipDto['role'] {
  const known: OrganizationMembershipDto['role'][] = [
    'owner',
    'member',
    'adviser',
    'approver',
  ...
  return withActor(getDb(), identity.ctx, async (tx) => {
    ...
    const rows = await tx
      ...
      .leftJoin(
        ...
        eq(schema.organizationProfiles.organizationId, schema.member.organizationId),
      )
...
export async function updateActiveOrganization(
  ...
  assertOrgPermission(identity, 'org.settings.manage', {
    type: 'organization',
    id: orgId,
  ...
  await withActor(getDb(), identity.ctx, async (tx) => {
    ...
    await recordAudit(tx, identity, {
      ...
      before: {
        name: before.name,
        ownershipType: before.profile?.ownershipType,
...
export async function listMembers(identity: RequestIdentity): Promise<OrganizationMemberDto[]> {
  ...
  assertOrgPermission(identity, 'org.read', {
    type: 'organization',
    id: orgId,
    organizationId: orgId,
  });
...
export async function inviteMember(
  ...
  const orgId = requireActiveOrganization(identity);
  assertOrgPermission(identity, 'org.members.invite', {
    type: 'organization',
    id: orgId,
    organizationId: orgId,
  });
  ...
  return withActor(getDb(), identity.ctx, async (tx) => {
    ...
    const existingMember = await tx
      ...
      .innerJoin(schema.user, eq(schema.user.id, schema.member.userId))
      .where(
        and(eq(schema.member.organizationId, orgId), sql`lower(${schema.user.email}) = ${email}`),
      );
    if (existingMember.length > 0)
      throw new ApiError('conflict', 'this person is already a member');
    ...
    if (pending.length > 0)
      throw new ApiError('conflict', 'an invitation for this email is already pending');
    const id = randomUUID();

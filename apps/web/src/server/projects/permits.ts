...
import { assertUpdatedAt, ctxFor, invalidTransition, notFound, toKobo, userIdOf, type ServiceOptions } from './shared';

type PermitRow = typeof schema.permitApplications.$inferSelect;
type EventRow = typeof schema.permitEvents.$inferSelect;

const ELAPSED_TYPES = new Set(['submitted', 'query_raised', 'resubmitted', 'approved', 'rejected', 'withdrawn']);
...
async function toDto(tx: Transaction, p: PermitRow): Promise<PermitDto> {
  ...
  const elapsed = timeline.length > 0 ? permitElapsed({ events: timeline, asOf: new Date().toISOString(), basis: p.statutoryTargetBasis ?? 'elapsed' }) : null;
  const statutory =
    p.statutoryTargetDays !== null && p.statutoryTargetBasis !== null && p.statutorySourceNote
      ? { days: p.statutoryTargetDays, basis: p.statutoryTargetBasis, sourceNote: p.statutorySourceNote }
      : null;
  return {
    id: p.id,
    ...
    notes: p.notes,
    elapsed: elapsed
      ? { basis: elapsed.basis, asOf: elapsed.asOf, applicantDays: elapsed.applicantDays, authorityDays: elapsed.authorityDays, totalDays: elapsed.totalDays, status: elapsed.status }
      : null,
...
async function loadPermit(tx: Transaction, identity: RequestIdentity, id: string, checks: AccessCheck[]) {
  const [p] = await tx.select().from(schema.permitApplications).where(eq(schema.permitApplications.id, id));
  if (!p || !p.projectId) throw notFound('permit application');
  const access = await requireProject(tx, identity, p.projectId, checks);
  return { p, access };
}

export async function createPermit(identity: RequestIdentity, projectId: string, input: PermitCreate, options: ServiceOptions = {}): Promise<PermitDto> {
  userIdOf(identity);
  return withActor(getDb(), ctxFor(identity, options), async (tx) => {
    ...
    await recordAudit(tx, identity, {
      ...
      entityId: row!.id,
      organizationId: access.project.organizationId,
      after: { projectId, jurisdiction: input.jurisdiction, authority: input.authority, permitType: input.permitType, statutoryTarget: input.statutoryTarget ?? null },
...
export async function listPermits(identity: RequestIdentity, projectId: string): Promise<{ items: PermitDto[] }> {
  userIdOf(identity);
  return withActor(getDb(), ctxFor(identity), async (tx) => {
    await requireProject(tx, identity, projectId, PROJECT_READ_CHECKS);
    const rows = await tx.select().from(schema.permitApplications).where(eq(schema.permitApplications.projectId, projectId)).orderBy(desc(schema.permitApplications.createdAt));
...
export async function updatePermit(identity: RequestIdentity, id: string, input: PermitUpdate, options: ServiceOptions = {}): Promise<PermitDto> {
  userIdOf(identity);
  return withActor(getDb(), ctxFor(identity, options), async (tx) => {
    ...
    if (input.applicationReference !== undefined) patch.applicationReference = input.applicationReference;
    if (input.feesKobo !== undefined) patch.feesKobo = input.feesKobo ? toKobo(input.feesKobo) : null;
    ...
    const [updated] = await tx.update(schema.permitApplications).set(patch).where(eq(schema.permitApplications.id, id)).returning();
    await recordAudit(tx, identity, {
      action: 'permit.updated',
      entityType: 'permit_application',
      entityId: id,
      organizationId: access.project.organizationId,
      before: { statutoryTargetDays: p.statutoryTargetDays, applicationReference: p.applicationReference },
      after: { statutoryTargetDays: updated!.statutoryTargetDays, applicationReference: updated!.applicationReference },
      correlationId: options.correlationId,
    });
...
export async function addPermitEvent(identity: RequestIdentity, id: string, input: PermitEventCreate, options: ServiceOptions = {}): Promise<PermitDto> {
  const actorId = userIdOf(identity);
  return withActor(getDb(), ctxFor(identity, options), async (tx) => {
    const { p, access } = await loadPermit(tx, identity, id, [{ staff: 'projects.manage' }]);
    const decision = permitStatusAfterEvent(p.status, input.eventType);
    if (!decision.ok) throw invalidTransition(decision.message, { status: p.status, eventType: input.eventType });
    if (input.eventType === 'note' && !input.note) throw new ApiError('validation_failed', 'a note event needs a note');
    await tx.insert(schema.permitEvents).values({ permitApplicationId: id, eventType: input.eventType, occurredAt: input.occurredAt, note: input.note ?? null, actorUserId: actorId });
    let current = p;
    if (decision.changed) {
      const patch: Partial<typeof schema.permitApplications.$inferInsert> = { status: decision.status };
      if (input.eventType === 'submitted') patch.submittedAt = input.occurredAt;
      if (['approved', 'rejected', 'withdrawn'].includes(input.eventType)) patch.decidedAt = input.occurredAt;
      if (input.eventType === 'completeness_confirmed') patch.completenessDate = input.occurredAt;
      const [updated] = await tx
        .update(schema.permitApplications)
        .set(patch)
        .where(and(eq(schema.permitApplications.id, id), eq(schema.permitApplications.status, p.status)))
        .returning();
      if (!updated) throw new ApiError('version_conflict', 'the application changed while recording the event; reload');
      current = updated;
    } else if (input.eventType === 'completeness_confirmed') {
      const [updated] = await tx.update(schema.permitApplications).set({ completenessDate: input.occurredAt }).where(eq(schema.permitApplications.id, id)).returning();
      current = updated ?? p;
    }
    await recordAudit(tx, identity, {

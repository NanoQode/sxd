...
describe('project delivery flow', () => {
  it('creates a project with an assigned PM and customer contact', async () => {
    ...
    expect(
      await errorCode(transitionProject(pm, projectId, { to: 'on_hold', expectedVersion: 2 })),
  ...
  it('approves a BOQ budget only with both customer and staff approval, with server-computed totals', async () => {
    ...
    expect(
      await errorCode(decideBudgetVersion(memberA, draft.id, { decision: 'approved' })),
    ).toMatch(/^forbidden/);
    ...
    const [project] = await dbs.owner
      .select()
      .from(schema.projects)
      .where(eq(schema.projects.id, projectId));
    expect(project!.approvedBudgetVersionId).toBe(draft.id);
    await addCommitment(pm, projectId, {
      kind: 'commitment',
      description: 'Blockwork contract',
      amountKobo: '90000000',
      currency: 'NGN',
    });
    await addCommitment(pm, projectId, {
  ...
  it('rejects dependency cycles and stores a computed baseline with the critical path', async () => {
    ...
    const s = await replaceSchedule(pm, projectId, {
      tasks: [
        ...
        },
        {
          ...
          name: 'Approvals',
          phase: 'approvals',
          durationDaysLikely: 4,
          calendarBasis: 'calendar_days',
          leadTimeDays: 0,
          accountableParty: 'authority',
          isMilestone: false,
          percentComplete: 0,
        },
        {
      ...
      dependencies: [
        {
          predecessorKey: 'design',
          successorKey: 'foundations',
          type: 'finish_to_start',
          lagDays: 0,
        },
        {
          predecessorKey: 'design',
          successorKey: 'approvals',
          type: 'finish_to_start',
          lagDays: 0,
        },
        {
          predecessorKey: 'foundations',
          successorKey: 'roof',
          type: 'finish_to_start',
          lagDays: 0,
        },
    ...
    await expect(
      dbs.owner
        .update(schema.scheduleBaselines)
        .set({ reason: 'x' })
  ...
  it('keeps inspector progress, customer acceptance and finance authorisation distinct', async () => {
    const m = await createMilestone(pm, projectId, {
      name: 'Foundations complete',
      plannedDate: '2026-10-20',
    });
    // Customer cannot record progress; inspector cannot accept.
    expect(await errorCode(recordMilestoneProgress(ownerA, m.id, { percentComplete: 50 }))).toMatch(
      /^forbidden/,
    );
    expect(await errorCode(decideMilestone(inspector, m.id, { decision: 'accepted' }))).toMatch(
      /^forbidden/,
    );
    ...
    expect(await errorCode(decideMilestone(ownerA, m.id, { decision: 'accepted' }))).toBe(
      'invalid_transition',
    );
    await submitMilestone(pm, m.id);
    // Finance cannot authorise before acceptance; members cannot accept.
    expect(await errorCode(authorizeMilestonePayment(finance, m.id, {}))).toBe(
      'invalid_transition',
    );
    expect(await errorCode(decideMilestone(memberA, m.id, { decision: 'accepted' }))).toMatch(
      /^forbidden/,
    );
    ...
    expect(
      await errorCode(
        authorizeMilestonePayment(staffIdentity(f.finance, 'finance', false), m.id, {}),
      ),
    ).toBe('mfa_required');
  ...
  it('runs a site visit with offline-safe submission and evidence rules', async () => {
    ...
    expect(
      await errorCode(startSiteVisit(staffIdentity(f.inspector2, 'inspector'), visitId, {})),
    ).toBe('forbidden');
    expect(await errorCode(startSiteVisit(pm, visitId, {}))).toBe('forbidden');
    const started = await startSiteVisit(inspector, visitId, {
      startedAt: '2026-10-12T09:05:00.000Z',
    });
    ...
    const sync = await syncSiteVisits(inspector, {
      items: [{ ...submission, siteVisitId: visitId }],
    });
    expect(sync.results[0]!.outcome).toBe('replayed');
    const stolen = await syncSiteVisits(staffIdentity(f.inspector2, 'inspector'), {
      items: [{ ...submission, siteVisitId: visitId }],
    });
    expect(stolen.results[0]!.outcome).toBe('rejected');
    expect(stolen.results[0]!.code).toBe('conflict');
    const visits = await dbs.owner
      .select()
      .from(schema.siteVisits)
      .where(eq(schema.siteVisits.projectId, projectId));
    expect(visits).toHaveLength(1);
    // The customer cannot review; the reviewer must not be the inspector who performed it.
    expect(await errorCode(reviewSiteVisit(inspector, visitId, {}))).toMatch(/^forbidden/);
    expect((await reviewSiteVisit(pm, visitId, { note: 'Looks complete' })).status).toBe(
      'reviewed',
    );
  });

  // Depends on file_objects being readable under row-level security (packages/db migration 0003).
  it('links evidence only for scanned files of the organisation and hides it from customers until approved', async () => {
    // Quarantined and infected files are refused, receivedAt is server time, publication is restricted.
    expect(await errorCode(linkEvidence(inspector, projectId, { fileId: f.fileScanning }))).toBe(
      'file_quarantined',
    );
    expect(await errorCode(linkEvidence(inspector, projectId, { fileId: f.fileInfected }))).toBe(
      'file_rejected',
    );
    expect(await errorCode(linkEvidence(inspector, projectId, { fileId: f.fileOrgB }))).toMatch(
      /not_found|forbidden/,
    );
    const withFiles = await syncSiteVisits(inspector, {
      items: [
        {
          offlineClientId: 'device-1-visit-1',
          siteVisitId: visitId,
          findingsMarkdown: 'replay',
          evidenceFileIds: [f.fileClean, f.fileScanning],
        },
      ],
    });
    expect(withFiles.results[0]!.outcome).toBe('replayed');
    expect(withFiles.results[0]!.evidence.find((e) => e.fileId === f.fileClean)?.outcome).toBe(
      'created',
    );
    expect(withFiles.results[0]!.evidence.find((e) => e.fileId === f.fileScanning)?.outcome).toBe(
      'rejected',
    );
    const again = await syncSiteVisits(inspector, {
      items: [
        {
          offlineClientId: 'device-1-visit-1',
          siteVisitId: visitId,
          findingsMarkdown: 'replay',
          evidenceFileIds: [f.fileClean],
        },
      ],
    });
    expect(again.results[0]!.evidence[0]?.outcome).toBe('replayed');
    ...
    expect(
      (
        await linkEvidence(inspector, projectId, {
          fileId: f.fileClean2,
          offlineClientId: 'device-1-photo-2',
        })
      ).idempotentReplay,
    ...
    expect(
      await errorCode(
        linkEvidence(pm, projectId, { fileId: f.fileClean2, offlineClientId: 'device-1-photo-2' }),
      ),
    ).toBe('conflict');
    ...
    expect(
      await errorCode(
        setEvidencePublication(inspector, projectId, ev.id, { publication: 'approved' }),
      ),
    ).toMatch(/^forbidden/);
  ...
  it('reviews and releases a report through a different named professional, then the customer reads it', async () => {
    const report = await createReport(inspector, projectId, {
      ...
      siteVisitId: visitId,
      initialRevision: {
        bodyMarkdown: 'Foundations complete.',
        scopeLimitations: 'Visual inspection only.',
        attachmentFileIds: [],
      },
    });
    ...
    expect(
      await errorCode(
        submitReport(inspector, report.id, {
          namedReviewerUserId: f.inspector.id,
          expectedVersion: 1,
        }),
      ),
    ...
    expect(
      await errorCode(
        submitReport(inspector, report.id, {
          namedReviewerUserId: f.inspector2.id,
          expectedVersion: 1,
        }),
      ),
    ).toBe('validation_failed');
    const inReview = await submitReport(inspector, report.id, {
    ...
    expect(
      await errorCode(
        addReportRevision(inspector, report.id, {
          bodyMarkdown: 'edit',
          attachmentFileIds: [],
          expectedVersion: 2,
        }),
    ...
    const changes = await reviewReport(ops, report.id, {
      decision: 'changes_requested',
      note: 'Add rebar photos',
      expectedVersion: 2,
    });
    expect(changes.status).toBe('changes_requested');
    const revised = await addReportRevision(inspector, report.id, {
      bodyMarkdown: 'Foundations complete; photos attached.',
      attachmentFileIds: [],
      expectedVersion: 3,
    });
    ...
    expect(approved.status).toBe('approved');
    expect(await errorCode(releaseReport(ops, report.id, { expectedVersion: 5 }))).toBe(
      'version_conflict',
    );
    ...
    expect((await listReports(ownerA, projectId, { limit: 25 })).items.map((r) => r.id)).toEqual([
      report.id,
    ]);
    // Further edits start a new draft version; the released revision stays frozen and visible.
    const draft3 = await addReportRevision(inspector, report.id, {
      bodyMarkdown: 'Version 3 draft',
      attachmentFileIds: [],
      expectedVersion: 7,
    });
    ...
    const outbox = await dbs.owner
      ...
      .where(eq(schema.outboxEvents.eventType, 'project.report.released'));
  ...
  it('numbers defects per project and requires an independent verifier', async () => {
    const d1 = await createDefect(inspector, projectId, {
      title: 'Honeycombing in column C2',
      severity: 'major',
      accountableParty: 'contractor',
    });
    const d2 = await createDefect(pm, projectId, {
      title: 'Missing damp-proof course',
      severity: 'critical',
      accountableParty: 'contractor',
    });
    ...
    expect(await errorCode(transitionDefect(inspector, d1.id, { to: 'verified' }))).toMatch(
      /^forbidden/,
    );
    ...
    expect(await errorCode(transitionDefect(ownerA, d2.id, { to: 'verified' }))).toMatch(
      /^forbidden/,
    );
    ...
    expect(overview.team.map((t) => t.userId)).toEqual(
      expect.arrayContaining([f.pm.id, f.inspector.id, f.ownerA.id]),
    );
  });
});

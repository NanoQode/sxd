...
const identityA = () =>
  customerIdentity({ userId: ids.userA, email: `${ids.userA}@example.test`, organizationId: ids.orgA, role: 'owner' });
const identityB = () =>
  customerIdentity({ userId: ids.userB, email: `${ids.userB}@example.test`, organizationId: ids.orgB, role: 'owner' });
...
describe('service request creation', () => {
  it('creates a request in inquiry with a sequential reference, transition, outbox event and audit entry', async () => {
    const created = await createServiceRequest(
      {
        ...
        intake: { property: 'Plot 12, Lekki Phase 1', title_documents: 'C of O', unknown_key: 'ignored' },
    ...
    expect(created.intake).toEqual({ property: 'Plot 12, Lekki Phase 1', title_documents: 'C of O' });
    ...
    expect(transitions[0]).toMatchObject({ fromStatus: null, toStatus: 'inquiry', actorType: 'customer', actorUserId: ids.userA });

    const outbox = await dbs.owner
      .select()
      .from(schema.outboxEvents)
      .where(and(eq(schema.outboxEvents.aggregateId, created.id), eq(schema.outboxEvents.eventType, 'service_request.transitioned')));
    ...
    const audit = await dbs.owner
      ...
      .where(and(eq(schema.auditEvents.entityId, created.id), eq(schema.auditEvents.action, 'service_request.created')));
    ...
    const results = await Promise.all(
      Array.from({ length: 6 }, (_, i) =>
        createServiceRequest(
          { serviceSlug: bookableSlug, description: `Concurrent request number ${i} for uniqueness.`, intake: {} },
          identityA(),
  ...
  it('refuses a service that is not bookable and points to the interest path', async () => {
    await expect(
      createServiceRequest(
        { serviceSlug: expansionSlug, description: 'Interested in snagging for a handover.', intake: {} },
        identityA(),
  ...
  it('requires an active organisation', async () => {
    const noOrg = customerIdentity({ userId: ids.userA, email: `${ids.userA}@example.test`, organizationId: null, memberships: [] });
    await expect(
      createServiceRequest({ serviceSlug: bookableSlug, description: 'No organisation yet on this account.', intake: {} }, noOrg, {
        correlationId: 'x',
      }),
...
describe('customer transitions', () => {
  it('cancels only with a reason, bumps the version and records the transition', async () => {
    const created = await createServiceRequest(
      { serviceSlug: bookableSlug, description: 'A request that will be cancelled by the customer.', intake: {} },
      identityA(),
      { correlationId: `corr-${sfx}-cancel` },
    );
    await expect(
      applyCustomerTransition(identityA(), created.id, { to: 'cancelled', expectedVersion: created.version }, { correlationId: 'c1' }),
    ).rejects.toSatisfy((err: unknown) => err instanceof ApiError && err.code === 'invalid_transition' && (err.details as { code: string }).code === 'reason_required');
    ...
    await expect(
      applyCustomerTransition(identityA(), created.id, { to: 'paused', reason: 'x', expectedVersion: cancelled.version }, { correlationId: 'c3' }),
    ).rejects.toMatchObject({ code: 'invalid_transition' });
  });

  it('rejects a stale expectedVersion', async () => {
    const created = await createServiceRequest(
      { serviceSlug: bookableSlug, description: 'A request used for the version conflict check.', intake: {} },
      identityA(),
      { correlationId: `corr-${sfx}-ver` },
    );
    await expect(
      applyCustomerTransition(identityA(), created.id, { to: 'cancelled', reason: 'stale', expectedVersion: created.version + 5 }, { correlationId: 'v1' }),
    ).rejects.toMatchObject({ code: 'version_conflict' });
  });

  it('does not allow a customer to move a request into staff-only states', async () => {
    const created = await createServiceRequest(
      { serviceSlug: bookableSlug, description: 'A request used for the actor rule check.', intake: {} },
      identityA(),
      { correlationId: `corr-${sfx}-actor` },
    );
    // inquiry → paused is not a customer transition; the machine refuses it.
    await expect(
      applyCustomerTransition(identityA(), created.id, { to: 'paused', reason: 'wait', expectedVersion: created.version }, { correlationId: 'a1' }),
    ).rejects.toMatchObject({ code: 'invalid_transition' });
  });
});
...
describe('cross-organisation isolation', () => {
  it('hides organisation A requests from organisation B through RLS and the read models', async () => {
    const created = await createServiceRequest(
      { serviceSlug: bookableSlug, description: 'Org A private request that B must never see.', intake: {} },
      identityA(),
      { correlationId: `corr-${sfx}-iso` },
    );
    const rowsForB = await withActor(dbs.app, { userId: ids.userB, organizationId: ids.orgB, staff: false }, (tx) =>
      tx.select({ id: schema.serviceRequests.id }).from(schema.serviceRequests).where(eq(schema.serviceRequests.id, created.id)),
    );
    expect(rowsForB).toHaveLength(0);
    const rowsForA = await withActor(dbs.app, { userId: ids.userA, organizationId: ids.orgA, staff: false }, (tx) =>
      tx.select({ id: schema.serviceRequests.id }).from(schema.serviceRequests).where(eq(schema.serviceRequests.id, created.id)),
    );
    expect(rowsForA).toHaveLength(1);

    await expect(getServiceRequestDetail(identityB(), created.id)).rejects.toMatchObject({ code: 'not_found' });
    await expect(
      applyCustomerTransition(identityB(), created.id, { to: 'cancelled', reason: 'intruder', expectedVersion: created.version }, { correlationId: 'i1' }),
  ...
  it('refuses a stale session whose active organisation the user no longer belongs to', async () => {
    ...
    const impostor = customerIdentity({ userId: ids.userB, email: `${ids.userB}@example.test`, organizationId: ids.orgA, memberships: [{ organizationId: ids.orgB, role: 'owner' }] });
    await expect(
      createServiceRequest({ serviceSlug: bookableSlug, description: 'Should be denied by membership check.', intake: {} }, impostor, { correlationId: 'imp' }),

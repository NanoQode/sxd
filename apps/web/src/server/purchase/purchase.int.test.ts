...
describe('offers and the negotiation log', () => {
  ...
  it('drafts an offer on a shortlisted property (one live offer per request)', async () => {
    const listingId = await insertListing(f, { title: 'Plot 5, Ibeju-Lekki', propertyKind: 'land' });
    const sl = await createShortlist(staff(f.pm, 'project_manager'), f.purchaseA, { name: 'Targets' });
    ...
    expect(
      await code(
        createPurchaseOffer(customerA(f), f.searchA, { shortlistItemId, amountKobo: '1', conditions: [] }),
      ),
    ).toBe('not_found');
    ...
    expect(offer.negotiationLog[0]).toMatchObject({ action: 'drafted', byUserId: f.ownerA, note: 'Opening position' });
    expect(
      await code(
        createPurchaseOffer(customerA(f), f.purchaseA, { shortlistItemId, amountKobo: '100', conditions: [] }),
      ),
    ).toBe('conflict');
    ...
    expect(await code(getPurchaseOffer(staff(f.otherPm, 'project_manager'), offerId))).toBe('forbidden');
  ...
  it('rejects invalid transitions and wrong actors', async () => {
    ...
    expect(
      await code(applyPurchaseOfferAction(staff(f.pm, 'project_manager'), offerId, { action: 'accept', expectedEntries: 1 })),
    ).toBe('invalid_transition');
    expect(
      await code(
        applyPurchaseOfferAction(customerA(f), offerId, { action: 'counter', amountKobo: '1', expectedEntries: 1 }),
      ),
    ).toBe('forbidden');
    // Staff acting for the buyer must cite the customer's instruction.
    expect(
      await code(applyPurchaseOfferAction(staff(f.pm, 'project_manager'), offerId, { action: 'submit', expectedEntries: 1 })),
    ).toBe('validation_failed');
    // An adviser lacks the authority to commit the organisation.
    expect(
      await code(
        applyPurchaseOfferAction(
          { ...customerA(f), actor: { ...customerA(f).actor, memberships: [{ organizationId: f.orgA, role: 'adviser' }] } },
          offerId,
  ...
  it('runs the negotiation with an append-only log and concurrency on the log length', async () => {
    const submitted = await applyPurchaseOfferAction(customerA(f), offerId, { action: 'submit', expectedEntries: 1 });
    ...
    expect(
      await code(applyPurchaseOfferAction(staff(f.pm, 'project_manager'), offerId, { action: 'counter', expectedEntries: 2 })),
    ).toBe('validation_failed');
    ...
    expect(countered).toMatchObject({ status: 'countered', amountKobo: (46_000_000n * 100n).toString(), entries: 3 });
    ...
    const accepted = await applyPurchaseOfferAction(customerA(f), offerId, { action: 'accept', expectedEntries: 4 });
    ...
    const [row] = await f.dbs.owner.select().from(schema.offers).where(eq(schema.offers.id, offerId));
    const stored = parseNegotiationLog(row!.negotiationLog);
    expect(stored.map((e) => e.action)).toEqual(['drafted', 'submitted', 'countered', 'note', 'accepted']);
    expect(() => assertAppendOnly(stored, [...stored.slice(0, 4)])).toThrow(/cannot be removed/);
    expect(() => assertAppendOnly(stored, [{ ...stored[0]!, amountKobo: '1' }, ...stored.slice(1)])).toThrow(
      /cannot be changed/,
    );
    // Terminal: nothing more, not even a withdrawal.
    expect(
      await code(applyPurchaseOfferAction(customerA(f), offerId, { action: 'withdraw', note: 'changed my mind', expectedEntries: 5 })),
    ).toBe('invalid_transition');
    // The accepted terms became tracked conditions.
    const ws = await getPurchaseWorkspace(customerA(f), f.purchaseA);
    expect(ws.conditions.map((c) => c.title).sort()).toEqual(['Subject to registry search', 'Subject to survey']);
...
describe('diligence dependency, conditions, checklist, handover and closing', () => {
  ...
  it('blocks closing until the diligence request is linked and clear', async () => {
    ...
    expect(
      await code(linkDiligence(staff(f.pm, 'project_manager'), f.purchaseA, { diligenceRequestId: f.searchB })),
    ).toBe('validation_failed');
    expect(
      await code(linkDiligence(staff(f.pm, 'project_manager'), f.purchaseA, { diligenceRequestId: f.searchA })),
    ).toBe('validation_failed');
    expect(await code(linkDiligence(customerA(f), f.purchaseA, { diligenceRequestId: f.diligenceA }))).toBe(
      'forbidden',
    );
  ...
  it('clears the dependency once red flags are resolved or waived and the memo is released', async () => {
    await f.dbs.owner
      ...
      .set({ status: 'waived', resolvedAt: new Date(), resolvedBy: f.pm, resolutionNote: 'Re-survey confirmed' })
      .where(eq(schema.engagementItems.id, redFlagId));
    expect(await blockerCodes(staff(f.pm, 'project_manager'))).not.toContain('diligence_red_flags_open');
    expect(await blockerCodes(staff(f.pm, 'project_manager'))).toContain('diligence_memo_not_released');
    ...
    expect(
      await code(waiveDiligence(staff(f.pm, 'project_manager'), f.purchaseA, { reason: 'Customer instructed us to proceed' })),
    ).toBe('forbidden');
    ...
    expect((await getPurchaseWorkspace(customerA(f), f.purchaseA)).diligence.waiverReason).toContain('instructed');
  ...
  it('tracks conditions, closing tasks and customer-acknowledged handover documents', async () => {
    ...
    for (const c of ws.conditions) {
      ...
      expect(
        await code(updatePurchaseItem(staff(f.pm, 'project_manager'), c.id, { status: 'waived', expectedVersion: c.version })),
      ).toBe('invalid_transition');
    }
    ...
    expect(
      await code(createPurchaseItem(customerA(f), f.purchaseA, { kind: 'closing_task', title: 'x', visibility: 'customer', fileIds: [] })),
    ).toBe('forbidden');
    ...
    const fileId = await insertCleanFile(f, { ownerUserId: f.pm, organizationId: f.orgA, entityId: f.purchaseA, name: 'deed.pdf' });
    ...
    expect(
      await code(updatePurchaseItem(staff(f.pm, 'project_manager'), handoverId, { status: 'satisfied', expectedVersion: handover.version })),
    ).toBe('invalid_transition');
    expect(await code(acknowledgeHandover(customerB(f), handoverId, { expectedVersion: handover.version }))).toBe(
      'not_found',
    );
    expect(await blockerCodes(staff(f.pm, 'project_manager'))).toContain('handover_not_acknowledged');
    const acknowledged = await acknowledgeHandover(customerA(f), handoverId, { expectedVersion: handover.version });
    ...
    const [ack] = await f.dbs.owner
      ...
      .where(and(eq(schema.outboxEvents.eventType, 'purchase_handover.acknowledged'), eq(schema.outboxEvents.aggregateId, handoverId)));
    expect(ack).toBeDefined();
    await updatePurchaseItem(staff(f.pm, 'project_manager'), taskId, { status: 'satisfied', expectedVersion: task.version });
  ...
  it('prepares the closing pack only with an agreed fee basis, as a reviewed report', async () => {
    const ws = await getPurchaseWorkspace(staff(f.pm, 'project_manager'), f.purchaseA);
    expect(
      await code(prepareClosingPack(staff(f.pm, 'project_manager'), f.purchaseA, { expectedVersion: ws.serviceRequestVersion })),
    ).toBe('insufficient_evidence');
    const scopeFile = await insertCleanFile(f, { ownerUserId: f.ownerA, organizationId: f.orgA, entityId: f.purchaseA, name: 'signed-scope.pdf' });
    await f.dbs.owner
    ...
    expect(
      await code(prepareClosingPack(customerA(f), f.purchaseA, { expectedVersion: ready.serviceRequestVersion })),
    ).toBe('forbidden');
    expect(
      await code(prepareClosingPack(staff(f.pm, 'project_manager'), f.purchaseA, { expectedVersion: ready.serviceRequestVersion + 5 })),
    ).toBe('version_conflict');
    ...
    expect(pack).toMatchObject({ kind: 'closing_pack', status: 'draft', serviceRequestId: f.purchaseA });
    ...
    expect(staffAfter.closing[0]).toMatchObject({ stage: 'submitted', reportId: pack.id, acceptedOfferId: expect.any(String) });
    expect(staffAfter.closing[0]!.handedOverDocuments.map((d) => d.title)).toEqual(['Deed of assignment']);
    expect(
      await code(prepareClosingPack(staff(f.pm, 'project_manager'), f.purchaseA, { expectedVersion: ready.serviceRequestVersion })),
    ).toBe('conflict');
  });
});

...
import { ApiError, type FilePublicApproval, type FilePublicApprovalResponse } from '@simplexd/contracts';
...
import { ctxFor, isSensitivePurpose, notFound, toFileDto, userIdOf, variantsOf, type ServiceOptions } from './shared';
  ...
  return withActor(getDb(), ctx, async (tx) => {
    ...
    assertAllowed(
      authorizeAny(
        ...
        { type: 'file', id: file.id, organizationId: file.organizationId, createdBy: file.ownerUserId },
      ),
    );
    if (input.approved) {
      ...
      if (!mime.startsWith('image/')) throw new ApiError('validation_failed', 'only images can be approved for public use (originals of documents and video are never public)');
      if (!variantsOf(file).includes('web')) throw new ApiError('conflict', 'the web derivative has not been generated yet; retry shortly', { retryable: true });
    }
    const [updated] = await tx
      ...
      .returning();
    const [existing] = await tx.select().from(schema.mediaAssets).where(eq(schema.mediaAssets.fileId, fileId));
    ...
    await recordAudit(tx, identity, {
      ...
      after: { isPublicApproved: input.approved, mediaAssetId: asset?.id ?? null, rightsConfirmed: asset?.rightsConfirmed ?? null },
      correlationId: options.correlationId,
    });
    return {
      file: toFileDto(updated!),
      mediaAsset: asset
        ? { id: asset.id, altText: asset.altText, caption: asset.caption, rightsConfirmed: asset.rightsConfirmed, approvedForPublic: asset.approvedForPublic }
        : null,
    };
  });
}

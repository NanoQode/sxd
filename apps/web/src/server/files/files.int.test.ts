...
import { LocalDevStorageProvider, sha256HexSync, verifyDevStorageUrl } from '@simplexd/integrations/storage';
import type { UploadIntentCreate } from '@simplexd/contracts';
import type { RequestIdentity } from '@/lib/auth/session';
import { deriveFileObject, scanFileObject, type MediaDeps } from '../../../../worker/src/handlers/media';
...
function identityFor(
  userId: string,
  opts: { staffRoles?: StaffRole[]; memberships?: Array<{ organizationId: string; role: OrgRole }>; isPartner?: boolean } = {},
): RequestIdentity {
  ...
  return {
    ...
    actor: { userId, staffRoles, memberships, activeOrganizationId, isPartner: opts.isPartner ?? false, mfaVerified: true, impersonation: null, flags: {} },
...
const ownerA = () => identityFor(ids.ownerA, { memberships: [{ organizationId: ids.orgA, role: 'owner' }] });
const memberA = () => identityFor(ids.memberA, { memberships: [{ organizationId: ids.orgA, role: 'member' }] });
const ownerB = () => identityFor(ids.ownerB, { memberships: [{ organizationId: ids.orgB, role: 'owner' }] });
...
async function upload(identity: RequestIdentity, bytes: Buffer, input: Partial<UploadIntentCreate> & { fileName: string; declaredMime: string }) {
...
beforeAll(async () => {
  ...
  storage = new LocalDevStorageProvider({ appEnv: 'test', root: storageRoot, appUrl: APP_URL, signingSecret: SIGNING_SECRET });
  setStorageForTests(storage);
  deps = { db: dbs.app, storage, scanner: new DevMalwareScanner({ appEnv: 'test' }), log: pino({ level: 'silent' }) };
  ...
  await owner.insert(schema.user).values(
    [ids.ownerA, ids.memberA, ids.ownerB, ids.ops, ids.superAdmin, ids.inspector, ids.partner].map((id) => ({ id, name: id, email: `${id}@example.test` })),
  );
  ...
  await owner.insert(schema.partnerProfiles).values({ userId: ids.partner, partnerType: 'surveyor', displayName: 'Partner', verificationStatus: 'verified' });
...
describe('upload pipeline', () => {
  it('runs intent → upload → finalise → scan → derivatives → signed download for a clean image', async () => {
    ...
    const intent = await upload(ownerA(), bytes, { fileName: 'site.png', declaredMime: 'image/png', sha256: sha256HexSync(bytes) });
    ...
    expect(pending.storageKey).toBe(`org/${ids.orgA.replace(/_/g, '-')}/org-document/${intent.fileId}.png`);
    ...
    await expect(issueDownload(ownerA(), intent.fileId, {})).rejects.toSatisfy((e) => errorCode(e) === 'file_quarantined');
    ...
    const outbox = await dbs.owner.select().from(schema.outboxEvents).where(eq(schema.outboxEvents.aggregateId, intent.fileId));
    ...
    expect((await storage.headObject({ bucket: 'private', key: clean.storageKey }))?.sizeBytes).toBe(bytes.length);
    const deriveJobs = await dbs.owner.select().from(schema.jobs).where(eq(schema.jobs.type, 'files.derive'));
    expect(deriveJobs.some((j) => (j.payload as { fileId: string }).fileId === intent.fileId)).toBe(true);
    ...
    expect(withVariants.derivatives?.['web']).toBe(`org/${ids.orgA.replace(/_/g, '-')}/org-document/${intent.fileId}.web.webp`);
    expect(withVariants.derivatives?.['thumb']).toContain('.thumb.webp');
    // The original is untouched; derivatives are WebP without EXIF.
    expect((await storage.headObject({ bucket: 'private', key: clean.storageKey }))?.sizeBytes).toBe(bytes.length);
    const thumbHead = await storage.headObject({ bucket: 'derivatives', key: withVariants.derivatives!['thumb']! });
    ...
    const log = await dbs.owner.select().from(schema.fileDownloadLog).where(eq(schema.fileDownloadLog.fileId, intent.fileId));
  ...
  it('enforces signed URL expiry (default 5 minutes, 15 max) and refuses quarantine downloads', async () => {
    ...
    const intent = await upload(ownerA(), bytes, { fileName: 'letter.pdf', declaredMime: 'application/pdf' });
    ...
    expect(verifyDevStorageUrl(issued.url, SIGNING_SECRET, new Date(expiresAt.getTime() - 1000)).ok).toBe(true);
    const late = verifyDevStorageUrl(issued.url, SIGNING_SECRET, new Date(expiresAt.getTime() + 1000));
    ...
    await expect(
      storage.createSignedDownloadUrl({ bucket: 'quarantine', key: (await fileRow(dbs.owner, intent.fileId)).storageKey, fileName: 'x', contentType: 'application/pdf', contentDisposition: 'inline' }),
  ...
  it('marks EICAR uploads infected, keeps them quarantined and refuses downloads', async () => {
    ...
    const intent = await upload(ownerA(), bytes, { fileName: 'notes.csv', declaredMime: 'text/csv' });
    const finalized = await finalizeUpload(ownerA(), intent.fileId, {});
    expect(finalized.outcome).toBe('scanning');
    const scan = await scanFileObject(deps, intent.fileId);
    ...
    await expect(issueDownload(ownerA(), intent.fileId, {})).rejects.toSatisfy((e) => errorCode(e) === 'file_rejected');
    await expect(issueDownload(superAdmin(), intent.fileId, {})).rejects.toSatisfy((e) => errorCode(e) === 'file_rejected');
  ...
  it('leaves the object quarantined when the scanner fails, retries three times, then stops', async () => {
    const bytes = Buffer.from('%PDF-1.4\n%plain\n');
    const intent = await upload(ownerA(), bytes, { fileName: 'report.scan-error.pdf', declaredMime: 'application/pdf' });
    await finalizeUpload(ownerA(), intent.fileId, {});
    ...
    const retries = await dbs.owner
      .select()
      .from(schema.jobs)
      .where(and(eq(schema.jobs.type, 'files.scan'), eq(schema.jobs.dedupeKey, `files.scan:${intent.fileId}:2`)));
    ...
    expect((await scanFileObject(deps, intent.fileId)).status).toBe('skipped');
    await expect(issueDownload(superAdmin(), intent.fileId, {})).rejects.toSatisfy((e) => errorCode(e) === 'file_quarantined');
  });
  ...
  it('rejects markup disguised as images, blocked types up front, and size/checksum mismatches', async () => {
    ...
    const asPng = await upload(ownerA(), html, { fileName: 'photo.png', declaredMime: 'image/png' });
    ...
    await expect(issueDownload(ownerA(), asPng.fileId, {})).rejects.toSatisfy((e) => errorCode(e) === 'file_rejected');
    ...
    const asJpeg = await upload(ownerA(), svg, { fileName: 'logo.jpg', declaredMime: 'image/jpeg' });
    ...
    const asPdf = await upload(ownerA(), jpegish, { fileName: 'scan.pdf', declaredMime: 'application/pdf' });
    ...
    await expect(createUploadIntent(ownerA(), { purpose: 'org_document', fileName: 'logo.svg', declaredMime: 'image/svg+xml', sizeBytes: 10 })).rejects.toSatisfy((e) => errorCode(e) === 'file_rejected');
    await expect(createUploadIntent(ownerA(), { purpose: 'org_document', fileName: 'page.html', declaredMime: 'text/html', sizeBytes: 10 })).rejects.toSatisfy((e) => errorCode(e) === 'file_rejected');
    await expect(createUploadIntent(ownerA(), { purpose: 'org_document', fileName: 'run.png', declaredMime: 'text/javascript', sizeBytes: 10 })).rejects.toSatisfy((e) => errorCode(e) === 'file_rejected');
    await expect(createUploadIntent(ownerA(), { purpose: 'bank_receipt', fileName: 'clip.mp4', declaredMime: 'video/mp4', sizeBytes: 10 })).rejects.toSatisfy((e) => errorCode(e) === 'file_rejected');
    await expect(createUploadIntent(ownerA(), { purpose: 'identity', fileName: 'id.pdf', declaredMime: 'application/pdf', sizeBytes: 30 * 1024 * 1024 })).rejects.toSatisfy((e) => errorCode(e) === 'file_rejected');

    // Declared size is bound to the signed URL; a different declared checksum is rejected at finalisation.
    const pdf = Buffer.from('%PDF-1.4\n%x\n');
    const wrongHash = await upload(ownerA(), pdf, { fileName: 'a.pdf', declaredMime: 'application/pdf', sha256: 'a'.repeat(64) });
    ...
    const sizeIntent = await createUploadIntent(ownerA(), { purpose: 'org_document', fileName: 'b.pdf', declaredMime: 'application/pdf', sizeBytes: 100 });
    ...
    await expect(storage.writeUploadedObject(v.params, pdf)).rejects.toMatchObject({ code: 'invalid_request' });
    await expect(finalizeUpload(ownerA(), sizeIntent.fileId, {})).rejects.toSatisfy((e) => errorCode(e) === 'conflict');
    // Only the uploader may finalise.
    await expect(finalizeUpload(memberA(), sizeIntent.fileId, {})).rejects.toSatisfy((e) => ['forbidden', 'not_found'].includes(errorCode(e) ?? ''));
  });
  ...
  it('plans large uploads as multipart and completes them with part etags', async () => {
    ...
    if (!v.ok) throw new Error(v.reason);
    const { etag } = await storage.writeUploadedPart(v.params, Buffer.from('000000ftyp'.padEnd(12, 'x')));
    await expect(finalizeUpload(ownerA(), intent.fileId, {})).rejects.toSatisfy((e) => errorCode(e) === 'validation_failed');
    await expect(finalizeUpload(ownerA(), intent.fileId, { parts: [{ partNumber: 1, etag: '"wrong"' }] })).rejects.toSatisfy((e) => errorCode(e) === 'validation_failed');
    const finalized = await finalizeUpload(ownerA(), intent.fileId, { parts: [{ partNumber: 1, etag }] });
    expect(finalized.outcome).toBe('scanning');
    const scan = await scanFileObject(deps, intent.fileId);
...
describe('access control', () => {
  async function cleanOrgDocument(identity: RequestIdentity = ownerA(), name = 'contract.pdf') {
    const bytes = Buffer.from(`%PDF-1.4\n%${uniqueSuffix()}\n`);
    const intent = await upload(identity, bytes, { fileName: name, declaredMime: 'application/pdf' });
  ...
  it('lets organisation members view/download, denies immediately once membership is revoked, and isolates organisations', async () => {
    ...
    await expect(issueDownload(ownerB(), fileId, {})).rejects.toSatisfy((e) => errorCode(e) === 'not_found');
    // Revoke the membership in the database; the (stale) session still lists org A.
    await dbs.owner.delete(schema.member).where(and(eq(schema.member.organizationId, ids.orgA), eq(schema.member.userId, ids.memberA)));
    await expect(issueDownload(memberA(), fileId, {})).rejects.toSatisfy((e) => errorCode(e) === 'not_found');
    await expect(getFile(memberA(), fileId)).rejects.toSatisfy((e) => errorCode(e) === 'not_found');
    await dbs.owner.insert(schema.member).values({ id: `m_${ids.memberA}`, organizationId: ids.orgA, userId: ids.memberA, role: 'member' });
  ...
  it('honours explicit grants by level and revocation, for users and organisations', async () => {
    ...
    await expect(createGrant(memberA(), fileId, { userId: ids.ownerB, level: 'view' })).rejects.toSatisfy((e) => errorCode(e) === 'forbidden');
    const view = await createGrant(ownerA(), fileId, { userId: ids.ownerB, level: 'view' });
    expect((await getFile(ownerB(), fileId)).id).toBe(fileId);
    await expect(issueDownload(ownerB(), fileId, {})).rejects.toSatisfy((e) => errorCode(e) === 'forbidden');
    const download = await createGrant(ops(), fileId, { userId: ids.ownerB, level: 'download', expiresAt: new Date(Date.now() + 60_000).toISOString() });
    ...
    const orgGrant = await createGrant(ownerA(), fileId, { organizationId: ids.orgB, level: 'download' });
    expect((await issueDownload(ownerB(), fileId, {})).url).toBeTruthy();
    await dbs.owner.delete(schema.member).where(eq(schema.member.userId, ids.ownerB));
    await expect(issueDownload(ownerB(), fileId, {})).rejects.toSatisfy((e) => errorCode(e) === 'not_found');
    await dbs.owner.insert(schema.member).values({ id: `m_${ids.ownerB}`, organizationId: ids.orgB, userId: ids.ownerB, role: 'owner' });
    await revokeGrant(ownerA(), fileId, orgGrant.id);
    await expect(getFile(ownerB(), fileId)).rejects.toSatisfy((e) => errorCode(e) === 'not_found');
    // Expired grants do not count.
    await dbs.owner.insert(schema.fileAccessGrants).values({ fileId, userId: ids.ownerB, level: 'download', grantedBy: ids.ownerA, expiresAt: new Date(Date.now() - 1000) });
    await expect(getFile(ownerB(), fileId)).rejects.toSatisfy((e) => errorCode(e) === 'not_found');
  });

  it('restricts sensitive purposes to the owner, user grants and staff with files.sensitive.read', async () => {
    const bytes = Buffer.from(`%PDF-1.4\n%id-${uniqueSuffix()}\n`);
    const intent = await upload(ownerA(), bytes, { purpose: 'identity', fileName: 'passport.pdf', declaredMime: 'application/pdf' });
    const row = await fileRow(dbs.owner, intent.fileId);
    ...
    await expect(getFile(memberA(), intent.fileId)).rejects.toSatisfy((e) => errorCode(e) === 'not_found');
    // operations_manager holds files.read_all but not files.sensitive.read.
    await expect(issueDownload(ops(), intent.fileId, {})).rejects.toSatisfy((e) => errorCode(e) === 'not_found');
    expect((await issueDownload(superAdmin(), intent.fileId, {})).url).toBeTruthy();
    await expect(createGrant(ownerA(), intent.fileId, { organizationId: ids.orgB, level: 'view' })).rejects.toSatisfy((e) => errorCode(e) === 'validation_failed');
    ...
    // Never publishable.
    await expect(setPublicApproval(superAdmin(), intent.fileId, { approved: true, altText: 'x', rightsConfirmed: true })).rejects.toSatisfy((e) => errorCode(e) === 'forbidden');
  });

  it('authorises evidence uploads by assignment and lists entity files by access', async () => {
    const pdf = Buffer.from('%PDF-1.4\n%evidence\n');
    const evidence = { purpose: 'evidence' as const, fileName: 'footing.pdf', declaredMime: 'application/pdf', sizeBytes: pdf.length, entityType: 'project' as const, entityId: ids.projectA };
    // Evidence needs an entity; the customer organisation, assigned staff and assigned partners may upload.
    await expect(createUploadIntent(ownerA(), { ...evidence, entityType: undefined, entityId: undefined })).rejects.toSatisfy((e) => errorCode(e) === 'validation_failed');
    await expect(createUploadIntent(ownerB(), evidence)).rejects.toSatisfy((e) => errorCode(e) === 'forbidden');
    await expect(createUploadIntent(inspector(), evidence)).rejects.toSatisfy((e) => errorCode(e) === 'forbidden');
    await expect(createUploadIntent(partner(), evidence)).rejects.toSatisfy((e) => errorCode(e) === 'forbidden');
    await dbs.owner.insert(schema.assignments).values([
      { organizationId: ids.orgA, projectId: ids.projectA, assigneeUserId: ids.inspector, role: 'inspector', status: 'active', assignedBy: ids.ops },
      { organizationId: ids.orgA, projectId: ids.projectA, assigneeUserId: ids.partner, role: 'surveyor', status: 'accepted', assignedBy: ids.ops },
    ]);
    ...
    for (const id of [byInspector.fileId, byPartner.fileId, byCustomer.fileId]) {
      ...
      await finalizeUpload(id === byInspector.fileId ? inspector() : id === byPartner.fileId ? partner() : ownerA(), id, {});
      await scanFileObject(deps, id);
    }
    const forOwner = await listFilesForEntity(ownerA(), { entityType: 'project', entityId: ids.projectA, limit: 25 });
    expect(forOwner.items.map((f) => f.id)).toEqual(expect.arrayContaining([byInspector.fileId, byPartner.fileId, byCustomer.fileId]));
    ...
    const forPartner = await listFilesForEntity(partner(), { entityType: 'project', entityId: ids.projectA, limit: 25 });
    expect(forPartner.items.map((f) => f.id)).toEqual([byPartner.fileId]);
    expect((await listFilesForEntity(ownerB(), { entityType: 'project', entityId: ids.projectA, limit: 25 })).items).toEqual([]);
    const paged = await listFilesForEntity(ops(), { entityType: 'project', entityId: ids.projectA, limit: 2 });
    expect(paged.items).toHaveLength(2);
    expect(paged.nextCursor).not.toBeNull();
    const rest = await listFilesForEntity(ops(), { entityType: 'project', entityId: ids.projectA, limit: 2, cursor: paged.nextCursor! });
    ...
    await expect(getFile(identityFor(ids.ops, { staffRoles: ['content_editor'] }), byCustomer.fileId)).rejects.toSatisfy((e) => errorCode(e) === 'not_found');
  });
  ...
  it('approves only derivatives of clean images for public use, never own uploads or documents', async () => {
    ...
    const intent = await upload(ownerA(), bytes, { fileName: 'facade.png', declaredMime: 'image/png' });
    await finalizeUpload(ownerA(), intent.fileId, {});
    await scanFileObject(deps, intent.fileId);
    await expect(setPublicApproval(ownerA(), intent.fileId, { approved: true, altText: 'Front', rightsConfirmed: true })).rejects.toSatisfy((e) => errorCode(e) === 'forbidden');
    await expect(setPublicApproval(ops(), intent.fileId, { approved: true, altText: 'Front', rightsConfirmed: true })).rejects.toSatisfy((e) => errorCode(e) === 'conflict');
    await deriveFileObject(deps, intent.fileId);
    await expect(setPublicApproval(ops(), intent.fileId, { approved: true, altText: 'Front' })).rejects.toSatisfy((e) => errorCode(e) === 'validation_failed');
    const approved = await setPublicApproval(ops(), intent.fileId, { approved: true, altText: 'Front elevation', rightsConfirmed: true, caption: 'Site A' });
    expect(approved.file.isPublicApproved).toBe(true);
    expect(approved.mediaAsset).toMatchObject({ altText: 'Front elevation', approvedForPublic: true, rightsConfirmed: true });
    const [asset] = await dbs.owner.select().from(schema.mediaAssets).where(eq(schema.mediaAssets.fileId, intent.fileId));
    ...
    const own = await upload(superAdmin(), await pngBytes(), { purpose: 'content_media', fileName: 'banner.png', declaredMime: 'image/png' });
    ...
    await expect(setPublicApproval(superAdmin(), own.fileId, { approved: true, altText: 'Banner', rightsConfirmed: true })).rejects.toSatisfy((e) => errorCode(e) === 'forbidden');
    expect((await setPublicApproval(ops(), own.fileId, { approved: true, altText: 'Banner', rightsConfirmed: true })).file.isPublicApproved).toBe(true);
    const pdfId = await cleanOrgDocument();
    await expect(setPublicApproval(superAdmin(), pdfId, { approved: true, altText: 'Doc', rightsConfirmed: true })).rejects.toSatisfy((e) => errorCode(e) === 'validation_failed');
    // content_media requires content.media.manage.
    await expect(createUploadIntent(ownerA(), { purpose: 'content_media', fileName: 'x.png', declaredMime: 'image/png', sizeBytes: 10 })).rejects.toSatisfy((e) => errorCode(e) === 'forbidden');
  });
});

        ...
        <>
          {outstanding.length > 0 ? (
            <p role="status" className="text-sm text-fg-muted">
              {outstanding.length} invoice{outstanding.length === 1 ? '' : 's'} outstanding. Open an invoice to pay it or declare a transfer.
            </p>
          ) : null}
          <DataTable
            caption="Invoices"
            ...
            columns={[
              ...
              { key: 'outstanding', header: 'Balance', cell: (i) => formatNairaString(i.outstandingKobo), className: 'text-right' },
              { key: 'due', header: 'Due', cell: (i) => (i.dueDate ? formatDateLabel(i.dueDate, zone) : '—') },
              { key: 'issued', header: 'Issued', cell: (i) => (i.issuedAt ? formatDateLabel(i.issuedAt, zone) : '—'), hideOnMobile: true },
            ]}
          />
        </>
      )}

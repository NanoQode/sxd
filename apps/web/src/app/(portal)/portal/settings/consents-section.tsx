...
export function ConsentsSection({
  ...
  async function toggle(next: boolean) {
    ...
    setError(null);
    try {
      await apiFetch('/api/v1/me/consents', {
        method: 'POST',
        body: { purpose: 'marketing_email', granted: next, source: 'portal_settings' },
      });
      setGranted(next);
      toast({
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Every consent decision is recorded with its policy version and source; the history below
          is append-only.
        </CardDescription>
      </CardHeader>
        ...
        <div className="flex items-center gap-3 rounded-md border border-border p-3">
          ...
          <label htmlFor="marketing-consent" className="text-sm">
            ...
            <span className="text-fg-muted">
              Product news and offers. Transactional and security messages are unaffected.
            </span>
          </label>
        </div>
        <DataTable
          ...
          columns={[
            ...
            },
            {
              ...
              cell: (c) => c.policyVersion,
              hideOnMobile: true,
            },
            {
              key: 'source',
              header: 'Source',
              cell: (c) => humanize(c.source),
              hideOnMobile: true,
            },
            {
              key: 'when',
              header: 'Recorded',
              cell: (c) => formatDateTimeLabel(c.recordedAt, timeZone),
            },
          ]}
        />
      </CardContent>

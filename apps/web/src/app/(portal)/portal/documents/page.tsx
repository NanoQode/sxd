...
import { Badge, Card, CardContent, CardDescription, CardHeader, CardTitle, DataTable, EmptyState, PageHeader, formatDateLabel, humanize } from '@simplexd/ui';
...
export default async function DocumentsPage({ searchParams }: { searchParams: Promise<{ folder?: string }> }) {
  ...
  const [folders, { reports }] = await Promise.all([loadDocumentLibrary(identity), listDocuments(identity)]);
  const zone = identity.profile?.timeZone ?? 'Africa/Lagos';
  const total = folders.reduce((n, f) => n + f.items.length, 0);
  const active = folders.find((f) => f.key === folder) ?? folders.find((f) => f.items.length > 0) ?? folders[0];
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Contracts, letters, surveys and anything the team asked for. Attach files to a request, project or property from that record instead.</CardDescription>
        </CardHeader>
        <CardContent>
          <FileUploader purpose="org_document" disabled={!caps.uploadDocuments} disabledReason={capabilityNote(caps, 'Uploading documents')} />
        </CardContent>
      </Card>

      {total === 0 ? (
        <EmptyState title="No files yet" description="Uploads you make and files the team shares with you appear here, grouped by request, project and property." />
      ) : (
            ...
            <ul className="flex gap-2 overflow-x-auto lg:flex-col">
              {folders.map((f) => (
                <li key={f.key} className="shrink-0">
                  ...
                  >
                    {f.label}
                    <Badge tone={f.items.length > 0 ? 'neutral' : 'neutral'}>{f.items.length}</Badge>
                  </Link>
                </li>
              ))}
            </ul>
                    ...
                    columns={[
                      {
                        ...
                        cell: (e) => (
                          <span className="flex flex-col">
                            ...
                            {e.entityLabel ? (
                              e.entityHref ? (
                                <Link href={e.entityHref} className="text-xs text-primary underline">
                                  {e.entityLabel}
                                </Link>
                              ) : (
                      ...
                      { key: 'purpose', header: 'Purpose', cell: (e) => humanize(String(e.file.purpose)), hideOnMobile: true },
                      { key: 'size', header: 'Size', cell: (e) => formatBytes(e.file.sizeBytes), hideOnMobile: true },
                      { key: 'status', header: 'Scan', cell: (e) => <FileStatusBadge fileId={e.file.id} status={e.file.status} reason={e.file.statusReason} /> },
                      { key: 'added', header: 'Added', cell: (e) => formatDateLabel(e.file.createdAt, zone) },
                      {
                        key: 'access',
                        header: 'Access',
                        mobileLabel: 'Access',
                        cell: (e) => <GrantsDisclosure fileId={e.file.id} ownedByMe={e.file.ownerUserId === identity.session?.user.id} />,
                      },
                      {
                        key: 'download',
                        header: <span className="sr-only">Download</span>,
                        mobileLabel: 'Download',
                        cell: (e) => <SignedDownloadButton fileId={e.file.id} fileName={e.file.originalName} status={e.file.status} />,
                      },
                    ]}
                  />
                )}
              ...
              {reports.slice(0, 5).map((r) => (
                <li key={r.id} className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  <span className="text-fg-muted">
                    {humanize(r.kind)} · v{r.releasedVersion ?? '—'} · {r.releasedAt ? formatDateLabel(r.releasedAt, zone) : '—'}
                  </span>
                </li>
              ))}
            </ul>

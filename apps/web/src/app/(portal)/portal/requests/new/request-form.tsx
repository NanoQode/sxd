...
import { Alert, Badge, Button, Field, Input, NativeSelect, Textarea, cn, formatNairaString } from '@simplexd/ui';
...
const INTAKE_LABELS: Record<string, { label: string; hint?: string; multiline?: boolean }> = {
  ...
  drawings_or_boq: { label: 'Drawings or bill of quantities', hint: 'Describe what you already have. File uploads arrive in Wave 2.', multiline: true },
  ...
  fee_basis_agreement: { label: 'Fee basis acknowledgement', hint: 'Purchase representation is charged on an agreed percentage basis with signed scope. Type "understood" to confirm.' },
...
function labelFor(key: string) {
  return INTAKE_LABELS[key] ?? { label: key.replace(/_/g, ' ').replace(/^\w/, (c) => c.toUpperCase()) };
}

const schema = z.object({
  serviceSlug: z.string().min(1, 'Choose a service'),
  title: z.string().trim().max(160).optional(),
  description: z.string().trim().min(10, 'Tell us a little more (at least 10 characters)').max(8000),
...
function priceLabel(p: BookableServiceDto['startingPrice']): string | null {
  ...
  if (p.basis === 'percentage' && p.percentageBps !== null) return `${(p.percentageBps / 100).toFixed(2)}% of purchase price`;
  if (p.amountKobo) {
    const amount = formatNairaString(p.amountKobo);
    return p.basis === 'per_month' ? `${amount} per month` : p.basis === 'from' ? `from ${amount}` : amount;
  }
  return null;
}
...
export function RequestForm({
  ...
  const form = useForm<Values>({
    ...
    defaultValues: {
      serviceSlug: initialServiceSlug && services.some((s) => s.slug === initialServiceSlug) ? initialServiceSlug : '',
      title: '',
      description: '',
      marketId: '',
  ...
  if (interestDone) {
    return (
      <Alert tone="success" title="Interest registered">
        Thank you. {service?.name} is not bookable yet, so we recorded your interest as an inquiry and the operations team
        will contact you at your account email. Reference {interestDone.slice(0, 8)}.{' '}
            ...
            return (
              <label
                ...
                className={cn(
                  ...
                  selected ? 'border-primary bg-primary-soft/40' : 'border-border bg-bg-elevated hover:border-border-strong',
                )}
              >
                <span className="flex items-start justify-between gap-2">
                  <span className="font-medium">
                    <input type="radio" value={s.slug} className="mr-2 accent-[var(--sx-primary)]" {...form.register('serviceSlug')} />
                    {s.name}
                  </span>
                  {s.bookable ? <Badge tone="success">Bookable</Badge> : <Badge tone="warning">Register interest</Badge>}
                </span>
                <span className="text-sm text-fg-muted">{s.shortDescription}</span>
                {price ? <span className="text-xs text-fg-subtle">Indicative price: {price}. Final price by quotation.</span> : null}
                {!s.bookable ? (
                  <span className="text-xs text-fg-subtle">
                    Not staffed for booking yet; we record interest as an inquiry and reply personally.
                  </span>
                ) : null}
              </label>
            );
      ...
      {service ? (
        <>
          <fieldset className="space-y-4">
            ...
            <Field label="Title" hint="Optional; we generate one from the service and market if left blank." error={errors.title?.message}>
              {({ id, describedBy, invalid }) => (
                <Input id={id} aria-describedby={describedBy} aria-invalid={invalid} maxLength={160} {...form.register('title')} />
              )}
            </Field>
            <Field label="Description" required error={errors.description?.message} hint="What do you need and by when? Anything the team should know before triage.">
              {({ id, describedBy, invalid }) => (
                <Textarea id={id} aria-describedby={describedBy} aria-invalid={invalid} rows={5} {...form.register('description')} />
              )}
            </Field>
            <div className="grid gap-4 sm:grid-cols-2">
              <Field label="Market" hint="Published markets from the explorer. Leave blank if unsure.">
                {({ id }) => (
              ...
              <Field label="Budget (whole naira)" hint="Optional. A range is fine in the description." error={errors.budgetNaira?.message}>
                {({ id, describedBy }) => (
                  <Input id={id} type="number" inputMode="numeric" min={0} step={1000} aria-describedby={describedBy} {...form.register('budgetNaira')} />
                )}
              </Field>
              ...
              <Field label="Link a saved scenario" hint={scenarios.length === 0 ? 'You have no unconverted scenarios yet.' : 'Carries your compared markets and assumptions into the request.'}>
                {({ id }) => (
                  <NativeSelect id={id} disabled={scenarios.length === 0} {...form.register('scenarioId')}>
          ...
          {service.bookable && service.intakeKeys.length > 0 ? (
            <fieldset className="space-y-4">
              ...
              <p className="text-sm text-fg-muted">
                These answers come from the {service.name} workflow template. Skip what you do not know; triage will ask.
              </p>
              {service.intakeKeys.map((key) => {
                ...
                return (
                  <Field key={key} label={meta.label} hint={meta.hint}>
                    {({ id, describedBy }) =>
                      meta.multiline ? (
                        <Textarea id={id} aria-describedby={describedBy} rows={3} maxLength={2000} {...form.register(`intake.${key}` as const)} />
                      ) : (
                        <Input id={id} aria-describedby={describedBy} maxLength={2000} {...form.register(`intake.${key}` as const)} />
                      )
                    }
                  </Field>
                );
              ...
              <Alert tone="info" title="Attachments">
                Document and photo uploads (drawings, title documents, evidence) arrive in Wave 2 with resumable, scanned
                uploads. Describe what you have for now; the team will request files securely after triage.
              </Alert>
            </fieldset>
          ) : null}

          <div className="flex flex-wrap items-center gap-3">
            <Button type="submit" loading={form.formState.isSubmitting} loadingLabel={service.bookable ? 'Submitting request' : 'Registering interest'}>
              {service.bookable ? 'Submit request' : 'Register interest'}
            </Button>
            <Link href="/portal/requests" className="sx-touch inline-flex items-center text-sm text-fg-muted underline">
              Cancel
            </Link>
            <span className="text-xs text-fg-subtle">Started {Math.max(1, Math.round((Date.now() - startedAt) / 60000))} min ago; nothing is saved until you submit.</span>
          </div>
        </>
      ) : null}
    </form>

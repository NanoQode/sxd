...
import { Card, CardContent, CardDescription, CardHeader, CardTitle, PageHeader, humanize } from '@simplexd/ui';
...
export default async function ConversationPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const caps = customerCapabilities(identity, conversation.organizationId ?? identity.ctx.organizationId);
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description={`${humanize(conversation.kind)} · ${conversation.participants.filter((p) => !p.leftAt).map((p) => p.name ?? 'Participant').join(', ')}`}
      />
      <Card>
        <CardHeader>
        ...
        <CardContent>
          <ConversationThread conversation={conversation} currentUserId={identity.session!.user.id} zone={zone} canSend={canSend} cannotSendReason={capabilityNote(caps, 'Sending messages')} />
        </CardContent>
      </Card>
    </div>
  );

...
import {
  ...
  ChangeOrderDecision,
  MilestoneDecision,
} from '@/components/portal/decisions';
...
export default async function ProjectDetailPage({
  ...
  return (
    <div className="space-y-6">
      ...
      <SectionTabs
        ...
        tabs={[
          ...
          { value: 'media', label: 'Media' },
          {
            ...
            label: 'Defects',
            badge:
              overview.defects.open > 0 ? (
                <Badge tone="warning">{overview.defects.open}</Badge>
              ) : undefined,
          },
          {
            value: 'decisions',
            ...
            badge:
              overview.pendingApprovals > 0 ? (
                <Badge tone="warning">{overview.pendingApprovals}</Badge>
              ) : undefined,
          },
      ...
      />
      {tab === 'overview' ? (
        <OverviewTab overview={overview} zone={zone} identity={identity} />
      ) : null}
      {tab === 'schedule' ? <ScheduleTab identity={identity} projectId={id} /> : null}
      {tab === 'budget' ? (
        <BudgetTab identity={identity} projectId={id} caps={caps} zone={zone} />
      ) : null}
      ...
      {tab === 'decisions' ? (
        <DecisionsTab identity={identity} projectId={id} caps={caps} zone={zone} />
      ) : null}
      {tab === 'timeline' ? (
...
async function OverviewTab({
  ...
  const notes = await listNotes(identity, {
    entityType: 'project',
    entityId: overview.project.id,
    limit: 50,
  });
  const statusCopy: Record<string, string> = {
    no_approved_budget:
      'No approved budget yet; commitments are tracked but not measured against a baseline.',
      ...
      <div className="space-y-6">
        <div className="grid gap-4 sm:grid-cols-2">
          <Card>
            <CardHeader>
              ...
              <p className="font-display text-2xl font-semibold">
                {v.hasApprovedBudget ? koboToNaira(v.approvedTotalKobo) : 'Not approved'}
              </p>
              <CardDescription>
                {overview.budget.approvedVersion
                  ? `Version ${overview.budget.approvedVersion}, approved ${overview.budget.approvedAt ? formatDateLabel(overview.budget.approvedAt, zone) : ''}`
                  : statusCopy[v.status]}
              </CardDescription>
            </CardHeader>
          </Card>
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                ...
                {v.pendingChangeOrderDeltaKobo !== '0'
                  ? ` · pending change orders ${koboToNaira(v.pendingChangeOrderDeltaKobo)}`
                  : ''}
              </CardDescription>
            </CardHeader>
          </Card>
          <Card>
            <CardHeader>
              ...
              <p className="font-display text-2xl font-semibold">
                {v.varianceKobo !== null ? koboToNaira(v.varianceKobo) : '—'}
              </p>
              <CardDescription>
                {v.forecastFinalCostKobo !== null
                  ? `Forecast final cost ${koboToNaira(v.forecastFinalCostKobo)} (commitment based)`
                  : statusCopy[v.status]}
            ...
            <CardHeader>
              ...
              <p className="font-display text-2xl font-semibold">
                {overview.schedule.forecastCompletionDate ??
                  overview.schedule.targetCompletionDate ??
                  'Unknown'}
              </p>
              <CardDescription>
                {overview.schedule.forecastCompletionDate
                ...
                {overview.schedule.percentComplete !== null
                  ? ` · ${overview.schedule.percentComplete}% complete`
                  : ''}
              </CardDescription>
            </CardHeader>
          </Card>
          ...
          <CardHeader>
            ...
            <CardDescription>
              {overview.milestones.total === 0
                ...
                : `${overview.milestones.total} milestones · ${Object.entries(
                    overview.milestones.byStatus,
                  )
                    .filter(([, n]) => n > 0)
                    .map(([s, n]) => `${n} ${humanize(s).toLowerCase()}`)
                    .join(
                      ', ',
                    )}${overview.milestones.nextPlannedDate ? ` · next planned ${overview.milestones.nextPlannedDate}` : ''}`}
            </CardDescription>
          </CardHeader>
          <CardContent className="flex flex-wrap gap-3 text-sm">
            <Link
              href={`/portal/projects/${overview.project.id}?tab=decisions`}
              className="text-primary underline"
            >
              Milestones awaiting acceptance
            </Link>
            <Link
              href={`/portal/projects/${overview.project.id}?tab=schedule`}
              className="text-primary underline"
            >
              Full schedule
            </Link>
            {overview.latestReleasedReport ? (
              <Link
                href={`/portal/reports/${overview.latestReleasedReport.id}`}
                className="text-primary underline"
              >
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Shared with the project team; staff-only notes are never shown.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <NotesPanel
              ...
              zone={zone}
              readOnly={
                overview.project.status === 'archived' || overview.project.status === 'cancelled'
              }
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="mt-3 text-xs text-fg-muted">
              Generated {formatDateTimeLabel(overview.generatedAt, zone)}
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            ...
            {schedule.range
              ? ` Scenario range ${schedule.range.minBasedCompletionDate} to ${schedule.range.maxBasedCompletionDate} (a range, not a promise).`
              : ''}
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          {schedule.tasks.length === 0 ? (
            <EmptyState
              ...
              description="The project manager builds the task list and dependencies after the budget is agreed."
            />
          ) : (
            <DataTable
              ...
              columns={[
                ...
                },
                {
                  ...
                  header: 'Planned',
                  cell: (t) =>
                    t.plannedStart || t.computed
                      ? `${t.computed?.earlyStart ?? t.plannedStart ?? '?'} → ${t.computed?.earlyFinish ?? t.plannedFinish ?? '?'}`
                      : 'Not computed',
                },
                {
                  key: 'duration',
                  ...
                  cell: (t) =>
                    t.durationDaysLikely === null
                      ? 'Unknown'
                      : `${t.durationDaysLikely}${t.durationDaysMin !== null && t.durationDaysMax !== null ? ` (${t.durationDaysMin}–${t.durationDaysMax})` : ''}`,
                  hideOnMobile: true,
                },
                {
                  key: 'float',
                  header: 'Float',
                  cell: (t) => (t.computed ? `${t.computed.totalFloatDays} d` : '—'),
                  hideOnMobile: true,
                },
                {
                  key: 'progress',
                  header: 'Progress',
                  cell: (t) =>
                    `${t.percentComplete}%${t.computed ? ` · ${humanize(t.computed.status)}` : ''}`,
                },
                {
                  key: 'party',
                  header: 'Accountable',
          ...
          {schedule.baselines.length > 0 ? (
            <p className="text-xs text-fg-muted">
              {schedule.baselines.length} baseline{schedule.baselines.length === 1 ? '' : 's'}{' '}
              recorded; current baseline v{schedule.baseline?.version ?? '—'}
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Approved vs committed vs actual, commitment based. Pending change orders are shown
            separately and never added to the approved figure.
          </CardDescription>
        </CardHeader>
        <CardContent>
            ...
            {[
              ...
              <div key={label}>
                ...
                <dd className="tabular-nums">
                  {value === null || value === undefined ? 'Not available' : koboToNaira(value)}
                </dd>
              </div>
            ))}
            <div>
          ...
          {variance.progressExtrapolation ? (
            <p className="mt-3 text-xs text-fg-muted">
              Progress extrapolation at {variance.progressExtrapolation.percentComplete}%:{' '}
              {koboToNaira(variance.progressExtrapolation.finalCostKobo)} (a scenario, not a
              forecast).
            </p>
          ) : null}
        </CardContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Bills of quantities and their approvals. A version becomes the approved budget only when
            every required approval exists.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          {versions.items.length === 0 ? (
            <EmptyState
              title="No budget version yet"
              description="The team drafts a budget from the accepted quote, an area rate or a bill of quantities."
            />
          ) : (
            versions.items
              .slice()
              ...
              .map((b) => {
                const customerPending = b.approvals.find(
                  (a) => a.approverRole === 'customer' && a.status === 'pending',
                );
                return (
                  <section
                    key={b.id}
                    className="space-y-3 rounded-lg border border-border p-4"
                    aria-labelledby={`budget-${b.id}`}
                  >
                    <div className="flex flex-wrap items-start justify-between gap-2">
                      <div>
                        ...
                        >
                          Version {b.version} <StatusBadge status={b.status} />{' '}
                          <Badge tone="neutral">{humanize(b.source)}</Badge>
                        </h3>
                        <p className="text-sm text-fg-muted">
                          Total {koboToNaira(b.totalKobo)} · contingency{' '}
                          {koboToNaira(b.contingencyKobo)} · created{' '}
                          {formatDateTimeLabel(b.createdAt, zone)}
                          {b.approvedAt
                            ? ` · approved ${formatDateTimeLabel(b.approvedAt, zone)}`
                            : ''}
                        </p>
                      ...
                      {customerPending ? (
                        <BudgetDecision
                          budget={{ id: b.id, version: b.version }}
                          canDecide={caps.approveChangeOrders}
                          cannotDecideReason={capabilityNote(caps, 'Approving a budget')}
                          totalLabel={koboToNaira(b.totalKobo)}
                        />
                      ) : null}
                    </div>
                          ...
                          <DataTable
                            ...
                            columns={[
                              ...
                              },
                              {
                                key: 'amount',
                                header: 'Amount',
                                cell: (i) => koboToNaira(i.amountKobo),
                                className: 'text-right',
                              },
                            ]}
                          />
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Purchase orders, contracts and invoices the team records against the budget.
          </CardDescription>
        </CardHeader>
        <CardContent>
          {commitments.items.length === 0 ? (
            <p className="text-sm text-fg-muted">
              Nothing committed or spent has been recorded yet.
            </p>
          ) : (
            <DataTable
              caption="Commitments"
              ...
              columns={[
                {
                  ...
                  header: 'Kind',
                  cell: (c) => (
                    <Badge tone={c.kind === 'actual' ? 'primary' : 'neutral'}>
                      {humanize(c.kind)}
                    </Badge>
                  ),
                },
                { key: 'desc', header: 'Description', cell: (c) => c.description },
                {
                  key: 'party',
                  header: 'Counterparty',
                  cell: (c) => c.counterparty ?? '—',
                  hideOnMobile: true,
                },
                {
                  key: 'amount',
                  header: 'Amount',
                  cell: (c) => koboToNaira(c.amountKobo),
                  className: 'text-right',
                },
                {
                  key: 'date',
                  header: 'Date',
                  cell: (c) => c.incurredAt ?? formatDateLabel(c.createdAt, zone),
                },
              ]}
            />
          )}
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Only reports a named reviewer released appear here; drafts and internal reviews never do.
        </CardDescription>
      </CardHeader>
      <CardContent>
        {released.length === 0 ? (
          <EmptyState
            ...
            description="Progress and inspection reports appear once reviewed and released."
          />
        ) : (
          <DataTable
            ...
            columns={[
              ...
              },
              {
                key: 'version',
                header: 'Version',
                cell: (r) => `v${r.releasedVersion ?? r.currentVersion}`,
              },
              {
                key: 'reviewer',
                header: 'Reviewed by',
                cell: (r) => r.namedReviewerName ?? '—',
                hideOnMobile: true,
              },
              {
                key: 'released',
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Photos, video and drawings a reviewer approved for you. Thumbnails are derivatives;
      ...
      <CardContent>
        ...
        {page.nextCursor ? (
          <p className="mt-3 text-xs text-fg-muted">Showing the latest 100 items.</p>
        ) : null}
      </CardContent>
    </Card>
  );
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Open, acknowledged, in-progress and disputed defects with who is accountable. Resolved
          defects move to the timeline.
        </CardDescription>
      </CardHeader>
      <CardContent>
        {page.items.length === 0 ? (
          <EmptyState
            title="No unresolved defects"
            description="Defects the inspector raises appear here until they are verified as resolved."
          />
        ) : (
          <DataTable
            ...
            columns={[
              {
                key: 'number',
                header: '#',
                cell: (d) => (d.number !== null ? String(d.number) : '—'),
              },
              {
                key: 'title',
                header: 'Defect',
              ...
              },
              {
                ...
                cell: (d) => (
                  <Badge
                    tone={
                      d.severity === 'critical' || d.severity === 'safety'
                        ? 'danger'
                        : d.severity === 'major'
                          ? 'warning'
                          : 'neutral'
              ...
              { key: 'status', header: 'Status', cell: (d) => <StatusBadge status={d.status} /> },
              {
                key: 'party',
                header: 'Accountable',
                cell: (d) => humanize(d.accountableParty),
                hideOnMobile: true,
              },
              { key: 'due', header: 'Due', cell: (d) => d.dueDate ?? '—' },
              {
                key: 'raised',
                header: 'Raised',
                cell: (d) => formatDateLabel(d.createdAt, zone),
                hideOnMobile: true,
              },
            ]}
          />
...
async function DecisionsTab({
  ...
  const pendingBudgets = budgets.items.filter((b) =>
    b.approvals.some((a) => a.approverRole === 'customer' && a.status === 'pending'),
  );
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Budget or schedule changes proposed by the team. The budget moves only when every
        ...
        <CardContent>
          {changeOrders.items.length === 0 ? (
            <EmptyState
              ...
              description="Budgets only move once you and staff approve a change."
            />
          ) : (
                ...
                return (
                  ...
                  >
                    <div className="min-w-0 space-y-1 text-sm">
                      ...
                      {c.description ? (
                        <p className="whitespace-pre-wrap text-fg-muted">{c.description}</p>
                      ) : null}
                      <p className="text-fg-muted">
                        Budget {c.amountDeltaKobo.startsWith('-') ? '' : '+'}
                        {koboToNaira(c.amountDeltaKobo)} · schedule{' '}
                        {c.scheduleDeltaDays >= 0 ? '+' : ''}
                        {c.scheduleDeltaDays} day(s)
                        {c.submittedAt
                          ? ` · submitted ${formatDateTimeLabel(c.submittedAt, zone)}`
                          : ''}
                        {c.decidedAt ? ` · decided ${formatDateTimeLabel(c.decidedAt, zone)}` : ''}
                      </p>
                      {c.decisionNote ? (
                        <p className="text-fg-muted">Note: {c.decisionNote}</p>
                      ) : null}
                      <ul className="flex flex-wrap gap-2 text-xs">
                        {c.approvals.map((a) => (
                          <li key={a.id}>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            The inspector&apos;s progress estimate never implies acceptance; only your decision
            does.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          {submitted.length === 0 ? (
            <p className="text-sm text-fg-muted">
              No milestone is presented for acceptance right now.
            </p>
          ) : (
            <ul className="space-y-3">
              {submitted.map((m) => (
                <li
                  key={m.id}
                  className="flex flex-col gap-3 rounded-lg border border-warning/40 bg-warning-soft/30 p-4 sm:flex-row sm:items-start sm:justify-between"
                >
                  <div className="text-sm">
                    ...
                    <p className="text-fg-muted">
                      ...
                      {m.inspectorProgressPct !== null
                        ? ` · inspector estimate ${m.inspectorProgressPct}%`
                        : ''}
                    </p>
                  </div>
                  <MilestoneDecision
                    ...
                    canDecide={caps.acceptMilestones}
                    cannotDecideReason={capabilityNote(caps, 'Accepting a milestone')}
                  />
                </li>
              ))}
            </ul>
            ...
            <DataTable
              ...
              columns={[
                ...
                { key: 'planned', header: 'Planned', cell: (m) => m.plannedDate ?? '—' },
                {
                  key: 'forecast',
                  header: 'Forecast',
                  cell: (m) => m.forecastDate ?? '—',
                  hideOnMobile: true,
                },
                {
                  key: 'progress',
                  header: 'Inspector estimate',
                  cell: (m) =>
                    m.inspectorProgressPct !== null ? `${m.inspectorProgressPct}%` : '—',
                  hideOnMobile: true,
                },
                {
                  key: 'accepted',
                  ...
                  cell: (m) =>
                    m.customerAcceptedAt
                      ? formatDateLabel(m.customerAcceptedAt, zone)
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Budget versions waiting for your approval. Line items are on the Budget tab.
          </CardDescription>
        </CardHeader>
        <CardContent>
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Derived from the project&apos;s own records (budgets, change orders, milestones, visits,
          reports, defects), each with the timestamp it carries. The staff audit log is not part of
          the customer view.
        </CardDescription>
      </CardHeader>
      <CardContent>

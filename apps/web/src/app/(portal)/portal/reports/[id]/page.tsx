...
export default async function ReportDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const released =
    report.revisions.find((r) => r.version === report.releasedVersion) ?? report.revisions[0];
  const [evidence, attachments] = await Promise.all([
    listReportEvidence(identity, id).catch(() => ({ items: [] })),
    Promise.all(
      (released?.attachmentFileIds ?? []).map((fileId) =>
        getFile(identity, fileId).catch(() => null),
      ),
  ...
  return (
    <div className="space-y-6">
      ...
      <ReportView
        ...
        projectHref={
          report.projectId
            ? `/portal/projects/${report.projectId}?tab=reports`
            : report.serviceRequestId
              ? `/portal/requests/${report.serviceRequestId}`
              : null
        }
      />
    </div>

...
export default async function OfferDetailPage({
  ...
  const counterpart =
    offer.viewerParty === 'buyer'
      ? (offer.ownerOrganizationName ?? 'the owner')
      : (offer.buyerOrganizationName ?? 'the buyer');
  return (
    <div className="space-y-6">
      <PageHeader
        eyebrow={
          <Link href="/portal/listings?tab=offers" className="underline">
            Offers
          </Link>
        }
      ...
      {offer.nextActions.length > 0 ? (
        <Alert tone="info" title="Your move">
          {offer.status === 'submitted'
            ? 'The buyer’s figure is on the table.'
            : 'The owner’s counter-offer is on the table.'}
          {offer.expiresAt
            ? ` This offer lapses ${formatDateTimeLabel(offer.expiresAt, zone)}.`
            : ''}
        </Alert>
      ) : null}
      ...
      <div className="grid gap-6 lg:grid-cols-[2fr_1fr]">
        ...
        <Card>
          ...
          <CardContent className="space-y-2 text-sm">
            <p>
              Amount on the table: <strong>{formatNairaString(offer.amountKobo)}</strong>
            </p>
            <p>Conditions:</p>
            ...
            {offer.decidedAt ? (
              <p className="text-fg-muted">Decided {formatDateTimeLabel(offer.decidedAt, zone)}.</p>
            ) : null}
            <p className="text-fg-muted">
              Acceptance is not a contract. A sale or lease completes through the land sales/leasing
              engagement with documents.
            </p>
          </CardContent>
        </Card>
      </div>

...
export default async function NewOfferPage({
  ...
  const identity = await requireSignedIn(
    `/portal/listings/offers/new${slug ? `?listing=${encodeURIComponent(slug)}` : ''}`,
  );
    ...
    return (
      <div className="space-y-6">
        ...
        <EmptyState
          ...
          description="Offers can only be made on published listings. Choose one from the public properties page."
          action={
        ...
        <EmptyState
          ...
          description={
            identity.ctx.organizationId
              ? capabilityNote(caps, 'Making offers')
              : 'Create or join an organisation first; offers belong to an organisation.'
          }
          action={
            <LinkButton
              href={identity.ctx.organizationId ? `/properties/${listing.slug}` : '/onboarding'}
              variant="secondary"
            >
              {identity.ctx.organizationId ? 'Back to the listing' : 'Set up an organisation'}
            </LinkButton>
          }
        />
      </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <ListingOfferForm
        ...
        statedPriceKobo={listing.priceKobo}
      />
    </div>
  );
}

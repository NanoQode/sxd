...
export default async function DevPaystackCheckoutPage({
  ...
  return (
    ...
    >
      ...
      {!reference && (
        <p>
          Missing <code>reference</code>. Start a payment from an invoice in the portal.
        </p>
      )}
      {reference && notFoundRef && (
        <p>
          Reference <code>{reference}</code> is not known to this process&apos;s development
          adapter. Create the attempt again from the invoice (the adapter keeps simulated
          transactions in memory per process).
        </p>
      )}
      {reference && known && !notFoundRef && (
        <>
          ...
          >
            ...
            <dd>
              {known.currency}{' '}
              {known.amountKobo === null
                ? '?'
                : (Number(known.amountKobo) / 100).toLocaleString('en-NG', {
          ...
          <form
            method="post"
            action="/dev/paystack-checkout/simulate"
            style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap', marginTop: '1.5rem' }}
          >
            <input type="hidden" name="reference" value={reference} />
            <button type="submit" name="outcome" value="success" style={buttonStyle('#0a7d32')}>
              Simulate success
            </button>
            <button type="submit" name="outcome" value="failed" style={buttonStyle('#b00020')}>
              Simulate failure
            </button>
            <button type="submit" name="outcome" value="abandoned" style={buttonStyle('#555')}>
              Simulate abandon
            </button>
          </form>
          <p style={{ marginTop: '1.5rem', color: '#555', fontSize: '0.9rem' }}>
            After choosing an outcome you are redirected to the callback URL. The callback only asks
            the server to verify; settlement happens only when the adapter reports success with the
            exact reference, amount and currency.
          </p>
        </>
      )}
    </main>

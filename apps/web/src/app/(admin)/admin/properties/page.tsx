...
import { DataTable, EmptyState, PageHeader, StatusBadge, formatArea, formatDateTimeLabel, humanize } from '@simplexd/ui';
...
export default async function PropertiesPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  const status = propertyStatusSchema.safeParse(raw.status).success ? (raw.status as never) : 'active';
  const organizationId = raw.organizationId || undefined;
  const [page, orgs] = await Promise.all([
    organizationId
      ? listProperties(identity, { kind, status, q: raw.q?.trim() || undefined, organizationId, cursor: raw.cursor, limit: 50 })
  ...
  return (
    <div className="space-y-6">
      ...
      <FilterBar>
        <FilterSelect name="organizationId" label="Organisation" value={organizationId} allLabel="All organisations" options={orgs.map((o) => ({ value: o.id, label: o.name }))} />
        <FilterSelect name="kind" label="Kind" value={kind} options={propertyKindSchema.options.map((k) => ({ value: k, label: humanize(k) }))} />
        <FilterSelect name="status" label="Status" value={status === 'active' ? undefined : status} allLabel="Active" options={[{ value: 'archived', label: 'Archived' }]} />
        <FilterInput name="q" label="Search" value={raw.q} placeholder="Name or city" />
      </FilterBar>
      {page.items.length === 0 ? (
        <EmptyState title="No properties match" description="Customers add properties from their portal (Properties → Add). Staff create one on a customer's behalf through the same API with the organisation named." />
      ) : (
        <DataTable
          caption="Properties"
          rows={page.items}
          ...
          rowLabel={(p) => p.name}
          columns={[
            { key: 'name', header: 'Property', cell: (p) => <Link href={`/admin/properties/${p.id}`} className="font-medium text-primary underline">{p.name}</Link> },
            ...
            { key: 'city', header: 'City / state', cell: (p) => [p.address?.city, p.address?.state].filter(Boolean).join(', ') || '—' },
            { key: 'area', header: 'Land area', cell: (p) => (p.landArea ? (p.landArea.m2 ? formatArea(p.landArea.m2) : `${p.landArea.declaredValue} ${p.landArea.declaredUnit}`) : '—'), hideOnMobile: true },

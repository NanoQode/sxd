...
export async function DesignTab({ identity, shell }: { identity: RequestIdentity; shell: ProjectShell }) {
        ...
        {items.map((d) => (
          <li key={d.id} className="rounded-md border border-border p-3">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div>
                <span className="font-medium">{d.title}</span> <span className="text-xs text-fg-muted">v{d.version} · {formatDateTimeLabel(d.updatedAt)}</span>
              </div>
              <span className="flex gap-1">
                <StatusBadge status={d.status === 'signed_off' ? 'accepted' : d.status} label={humanize(d.status)} />
            ...
            <p className="text-xs text-fg-muted">
              ...
              {d.customerSignoffAt ? ` · signed off ${formatDateTimeLabel(d.customerSignoffAt)}` : ''}
            </p>
            {d.comments.length > 0 ? (
              <ul className="mt-2 space-y-1 text-sm">
                {d.comments.map((c) => (
                  <li key={c.id} className="flex flex-wrap items-start justify-between gap-2 rounded-md bg-bg-sunken p-2">
                    <span>
                      <strong>{c.authorName ?? 'Someone'}</strong> <span className="text-xs text-fg-muted">{formatDateTimeLabel(c.createdAt)}</span>
                      <br />
                      {c.body}
                    </span>
                    {c.resolvedAt ? (
                      ...
                      <ApiAction path={`/api/v1/design-options/${d.id}/comments/${c.id}/resolve`} label="Resolve" variant="ghost" body={{}} successMessage="Comment resolved" />
                    ) : null}
                  </li>
                ))}
              </ul>

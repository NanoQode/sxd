...
const NEXT_STATES: Record<string, Array<{ to: string; reason: boolean }>> = {
  planning: [
    { to: 'active', reason: false },
    { to: 'cancelled', reason: true },
  ],
  active: [
    { to: 'on_hold', reason: true },
    { to: 'completed', reason: false },
    { to: 'cancelled', reason: true },
  ],
  on_hold: [
    { to: 'active', reason: false },
      ...
      <PageHeader
        ...
        actions={
          <>
            <StatusBadge
              status={
                p.status === 'active' ? 'in_progress' : p.status === 'planning' ? 'draft' : p.status
              }
            ...
            {permissions.manage
              ? (NEXT_STATES[p.status] ?? []).map((t) => (
                  <ApiAction
                    ...
                    body={{ to: t.to, expectedVersion: p.version }}
                    reasonKey="reason"
                    confirm={{
                      title: `Move project to ${humanize(t.to)}?`,
                      requireReason: t.reason,
                      confirmLabel: humanize(t.to),
                      tone: t.to === 'cancelled' ? 'danger' : 'primary',
                    }}
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
            <StatTile
              ...
              value={
                variance.approvedTotalKobo ? (
                  <Money kobo={variance.approvedTotalKobo} compact />
                ) : (
                  'none'
                )
              }
              href={`/admin/projects/${p.id}?tab=budget`}
              tone={variance.hasApprovedBudget ? 'neutral' : 'warning'}
              hint={
                variance.hasApprovedBudget
                  ? `v${overview.budget.approvedVersion}`
                  : 'No approved version'
              }
            />
            <StatTile
              label="Committed + actual"
              value={<Money kobo={variance.exposureKobo} compact />}
              href={`/admin/projects/${p.id}?tab=budget`}
              tone={
                variance.status === 'over_committed' || variance.status === 'over_spent'
                  ? 'danger'
                  : 'neutral'
              }
              hint={humanize(variance.status)}
            />
            <StatTile
              label="Forecast completion"
              value={
                overview.schedule.forecastCompletionDate
                  ? formatDateLabel(overview.schedule.forecastCompletionDate)
                  : 'unknown'
              }
              href={`/admin/projects/${p.id}?tab=schedule`}
              hint={humanize(overview.schedule.forecastSource)}
            />
            <StatTile
              label="Open defects"
              value={overview.defects.open}
              href={`/admin/projects/${p.id}?tab=defects`}
              tone={overview.defects.open > 0 ? 'warning' : 'neutral'}
            />
            <StatTile
              label="Milestones"
              value={overview.milestones.total}
              href={`/admin/projects/${p.id}?tab=milestones`}
              hint={
                overview.milestones.nextPlannedDate
                  ? `next ${formatDateLabel(overview.milestones.nextPlannedDate)}`
                  : undefined
              }
            />
            <StatTile
              label="Pending change orders"
              value={overview.changeOrders.pending}
              href={`/admin/projects/${p.id}?tab=change-orders`}
              tone={overview.changeOrders.pending > 0 ? 'warning' : 'neutral'}
            />
            <StatTile
            ...
            />
            <StatTile
              label="Latest released report"
              value={
                overview.latestReleasedReport
                  ? `v${overview.latestReleasedReport.releasedVersion}`
                  : 'none'
              }
              href={`/admin/projects/${p.id}?tab=reports`}
              hint={overview.latestReleasedReport?.title}
            />
          </div>
              ...
              <DefinitionList
                items={[
                  ...
                  },
                  {
                    ...
                    value: p.serviceRequestId ? (
                      <Link
                        href={`/admin/service-requests/${p.serviceRequestId}`}
                        className="underline"
                      >
                        Open request
                      </Link>
                  ...
                  },
                  {
                    term: 'Property',
                    value: p.propertyId ? (
                      <Link href={`/admin/properties/${p.propertyId}`} className="underline">
                        Open property
                      </Link>
                    ) : null,
                  },
                  {
                    ...
                    value: overview.team.find((t) => t.source === 'project_manager')?.name ?? null,
                  },
                  {
                    term: 'Customer contact',
                    value: overview.team.find((t) => t.source === 'customer_contact')?.name ?? null,
                  },
                  { term: 'Start', value: p.startDate ? formatDateLabel(p.startDate) : null },
                  {
                    term: 'Target completion',
                    value: p.targetCompletionDate ? formatDateLabel(p.targetCompletionDate) : null,
                  },
                  {
                    term: 'Gross floor area',
                    value: p.grossFloorAreaM2 ? `${p.grossFloorAreaM2} m²` : null,
                  },
                  {
                    term: 'Schedule version',
                    value: `v${overview.schedule.currentScheduleVersion} · ${overview.schedule.taskCount} tasks${overview.schedule.percentComplete !== null ? ` · ${overview.schedule.percentComplete}% complete` : ''}`,
                  },
                ]}
              />
              {p.description ? (
                <p className="whitespace-pre-wrap rounded-md bg-bg-sunken p-3">{p.description}</p>
              ) : null}
              <p className="text-xs text-fg-muted">
                Allowed statuses: {projectStatusSchema.options.map(humanize).join(', ')}.
              </p>
            </Section>
            <Section
              title="Team and assignments"
              description="Assignments give inspectors, surveyors and partners access to this project after acceptance."
            >
              ...
              <AssignmentsPanel
                ...
                assignees={[
                  ...
                  ...shell.partners.map((x) => ({
                    userId: x.userId,
                    name: x.name,
                    kind: 'partner' as const,
                    detail: `${humanize(x.partnerType)} · ${humanize(x.verificationStatus)}`,
                  })),
                ]}

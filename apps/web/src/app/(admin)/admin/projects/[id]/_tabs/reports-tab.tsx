            ...
            fields={[
              {
                ...
                type: 'select',
                required: true,
                options: reportKindSchema.options.map((k) => ({ value: k, label: humanize(k) })),
              },
              { name: 'title', label: 'Title', required: true },
              {
                ...
                label: 'Summary',
                type: 'textarea',
                bodyKey: 'initialRevision.summary',
                emptyAs: 'null',
              },
              {
                name: 'bodyMarkdown',
                ...
                type: 'textarea',
                required: true,
                bodyKey: 'initialRevision.bodyMarkdown',
              },
              {
                name: 'scopeLimitations',
                label: 'Scope and limitations',
                type: 'textarea',
                hint: 'What this report does not cover. Never implies a legal guarantee.',
                bodyKey: 'initialRevision.scopeLimitations',
                emptyAs: 'null',
              },
            ]}
            extraBody={{
              serviceRequestId: p.serviceRequestId,
              'initialRevision.attachmentFileIds': [],
            }}
          />
        ) : null
      }
      ...
      <DataTable
        ...
        columns={[
          ...
          },
          {
            ...
            header: 'Version',
            cell: (r) =>
              `v${r.currentVersion}${r.releasedVersion ? ` (released v${r.releasedVersion})` : ''}`,
          },
          {
            key: 'people',
            header: 'Author / reviewer',
            cell: (r) => `${r.authorName ?? '—'} / ${r.namedReviewerName ?? '—'}`,
            hideOnMobile: true,
          },
          {
            key: 'updated',

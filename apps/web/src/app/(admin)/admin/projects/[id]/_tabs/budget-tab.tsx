...
export async function BudgetTab({
  ...
  return (
    <div className="space-y-6">
      <Section
        title="Variance and forecast"
        description="Commitment-based: approved base + contingency versus committed and actual. Approved change orders move the approved total; pending ones are shown separately."
      >
        <DefinitionList
          items={[
            {
              term: 'Approved base',
              value: v.approvedBaseKobo ? (
                <Money kobo={v.approvedBaseKobo} />
              ) : (
                'no approved budget'
              ),
            },
            ...
            {
              term: 'Approved total',
              value: v.approvedTotalKobo ? <Money kobo={v.approvedTotalKobo} /> : null,
            },
            ...
            {
              term: 'Forecast final cost',
              value: v.forecastFinalCostKobo ? <Money kobo={v.forecastFinalCostKobo} /> : null,
            },
            {
              term: 'Variance',
              value: v.varianceKobo ? (
                <span>
                  <Money kobo={v.varianceKobo} />
                  {v.variancePct !== null ? ` (${v.variancePct.toFixed(1)}%)` : ''}
                </span>
              ) : null,
            },
            {
              term: 'Approved change orders',
              value: <Money kobo={v.approvedChangeOrderDeltaKobo} />,
            },
            {
              term: 'Pending change orders',
              value: <Money kobo={v.pendingChangeOrderDeltaKobo} />,
            },
            {
              term: 'Status',
              value: (
                <Badge
                  tone={
                    v.status === 'within_budget'
                      ? 'success'
                      : v.status === 'no_approved_budget'
                        ? 'neutral'
                        : 'danger'
                  }
                >
        ...
        {v.progressExtrapolation ? (
          <p className="text-xs text-fg-muted">
            Progress extrapolation ({v.progressExtrapolation.percentComplete}% complete):{' '}
            <Money kobo={v.progressExtrapolation.finalCostKobo} /> — a scenario, not a forecast.
          </p>
        ) : null}
      </Section>
      ...
      <Section
        ...
        actions={
          canManage ? (
            <NewBudgetVersion projectId={p.id} hasArea={Boolean(p.grossFloorAreaM2)} />
          ) : null
        }
      >
          ...
          {versions.map((bv) => (
            <div key={bv.id} className="rounded-md border border-border p-3">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div>
                  <span className="font-medium">Version {bv.version}</span>{' '}
                  <StatusBadge
                    status={bv.status === 'approved' ? 'accepted' : bv.status}
                    label={humanize(bv.status)}
                  />{' '}
                  <span className="text-xs text-fg-muted">
                    {humanize(bv.source)} · created {formatDateTimeLabel(bv.createdAt)}
                  </span>
                </div>
                <span className="font-medium">
                  <Money kobo={bv.totalKobo} currency={bv.currency} />{' '}
                  {bv.contingencyKobo !== '0' ? (
                    <span className="text-xs text-fg-muted">
                      + contingency <Money kobo={bv.contingencyKobo} />
                    </span>
                  ) : null}
                </span>
              </div>
              ...
              <div className="mt-2 flex flex-wrap gap-2 text-xs">
                <Badge
                  tone={
                    bv.approvalPolicy.outcome === 'approved'
                      ? 'success'
                      : bv.approvalPolicy.outcome === 'rejected'
                        ? 'danger'
                        : 'warning'
                  }
                >
                  Policy: {humanize(bv.approvalPolicy.outcome)}
                  {bv.approvalPolicy.missing.length > 0
                    ? ` · missing ${bv.approvalPolicy.missing.join(', ')}`
                    : ''}
                </Badge>
                {bv.approvals.map((a) => (
                  <Badge
              ...
              {bv.status === 'draft' &&
              canManage &&
              bv.approvalPolicy.requiresStaffApproval &&
              !bv.approvals.some((a) => a.approverRole === 'staff' && a.status !== 'pending') ? (
                <div className="mt-2 flex flex-wrap gap-2">
                  <ApiAction
                    path={`/api/v1/budgets/${bv.id}/decisions`}
                    label="Approve (staff)"
                    variant="primary"
                    body={{ decision: 'approved' }}
                    reasonKey="note"
                    confirm={{
                      title: `Approve budget version ${bv.version}?`,
                      description:
                        'Becomes the approved budget only once every required approval is present.',
                  ...
                  <ApiAction
                    ...
                    successMessage="Version rejected"
                  />
                </div>
              ) : null}
              {bv.items.length > 0 ||
              (bv.status === 'draft' && bv.source === 'boq' && canManage) ? (
                <details className="mt-2" open={bv.status === 'draft'}>
                  <summary className="cursor-pointer text-sm text-fg-muted">
                    Bill of quantities ({bv.items.length} items)
                  </summary>
                  <BoqEditor
                    budgetId={bv.id}
                    items={bv.items}
                    editable={
                      bv.status === 'draft' &&
                      canManage &&
                      bv.approvals.every((a) => a.status === 'pending')
                    }
                  />
                </details>
              ) : null}
              {bv.inclusions ? (
                <p className="mt-2 text-xs">
                  <strong>Inclusions:</strong> {bv.inclusions}
                </p>
              ) : null}
              {bv.notes ? (
                <p className="text-xs">
                  <strong>Notes:</strong> {bv.notes}
                </p>
              ) : null}
            </div>
          ))}
              ...
              <li key={c.id} className="flex flex-wrap items-center justify-between gap-2 py-2">
                <span>
                  ...
                  <span className="block text-xs text-fg-muted">
                    {c.counterparty ?? ''} {c.reference ? `· ${c.reference}` : ''}{' '}
                    {c.incurredAt ? `· ${c.incurredAt}` : ''}
                  </span>
                </span>

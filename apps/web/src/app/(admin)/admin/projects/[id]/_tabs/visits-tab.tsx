...
export async function VisitsTab({
  identity,
  shell,
}: {
  ...
  const inspectors = [
    ...shell.staff
      .filter(
        (s) =>
          s.roles.includes('inspector') ||
          s.roles.includes('project_manager') ||
          s.roles.includes('operations_manager'),
      )
      .map((s) => ({ value: s.userId, label: `${s.name} (staff)` })),
    ...shell.assignments
      .filter((a) => ['accepted', 'active'].includes(a.status))
      .map((a) => ({
        value: a.assigneeUserId,
        label: `${a.assigneeName ?? a.assigneeUserId} (${humanize(a.role)}, assigned)`,
      })),
              ...
              fields={[
                {
                  ...
                  required: true,
                  options: inspectors,
                  hint: 'Staff inspectors and accepted partner assignments.',
                },
                { name: 'scheduledAt', label: 'Scheduled at', type: 'datetime', required: true },
                {
                  name: 'instructions',
                  label: 'Instructions and checklist notes',
                  type: 'textarea',
                  emptyAs: 'null',
                },
              ]}
        ...
        <ul className="space-y-3">
          {items.map((v) => (
            <li key={v.id} className="rounded-md border border-border p-3">
              <div className="flex flex-wrap items-center justify-between gap-2">
                ...
                <span className="flex flex-wrap gap-2">
                  <StatusBadge
                    ...
                    label={humanize(v.status)}
                  />
                  <Badge tone="neutral">{v.evidenceCount} evidence</Badge>
                  {v.appointmentId ? (
                    <Link
                      href={`/admin/appointments?view=day&date=${(v.scheduledAt ?? '').slice(0, 10)}`}
              ...
              {v.instructions ? (
                <p className="mt-1 whitespace-pre-wrap text-sm">{v.instructions}</p>
              ) : null}
              {v.findingsMarkdown ? (
                <details className="mt-2">
                  <summary className="cursor-pointer text-sm text-fg-muted">
                    Findings{v.weather ? ` · weather: ${v.weather}` : ''}
                  </summary>
                  <p className="mt-1 whitespace-pre-wrap rounded-md bg-bg-sunken p-2 text-sm">
                    {v.findingsMarkdown}
                  </p>
                  {v.accessNote ? (
                    <p className="text-xs text-fg-muted">Access: {v.accessNote}</p>
                  ) : null}
                </details>
              ) : null}
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                ...
                v.inspectorUserId !== me ? (
                  <ApiAction
                    path={`/api/v1/site-visits/${v.id}/review`}
                    label="Mark reviewed"
                    variant="primary"
                    reasonKey="note"
                    confirm={{
                      title: 'Review this visit?',
                      description:
                        'Confirms the findings were reviewed by someone other than the inspector.',
                      confirmLabel: 'Reviewed',
                    }}
                    successMessage="Visit reviewed"
                  />
                ) : null}
                {v.status === 'submitted' && v.inspectorUserId === me ? (
                  <span className="text-xs text-fg-muted">
                ...
                <Link
                  href={`/admin/projects/${p.id}?tab=evidence&siteVisitId=${v.id}`}

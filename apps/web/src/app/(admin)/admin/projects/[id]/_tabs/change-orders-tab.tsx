...
export async function ChangeOrdersTab({
  identity,
  shell,
}: {
  ...
  return (
    <div className="space-y-6">
      <Alert tone="info" title="Approved budgets change only through change orders">
        A change order alters the approved budget and forecast only once every approval the policy
        requires (customer and/or staff) is present. Pending deltas are shown separately in the
        variance.
      </Alert>
      <Section
          ...
          perms.manage ? (
            <FormDialog
              ...
              fields={[
                ...
                {
                  name: 'amountDeltaKobo',
                  label: 'Cost delta (₦, negative for savings)',
                  type: 'naira',
                  required: true,
                  hint: 'Prefix with - for a reduction.',
                },
                {
                ...
                {
                  name: 'requiresStaffApproval',
                  label: 'Requires staff approval',
                  type: 'checkbox',
                  defaultValue: true,
                },
      ...
      >
        ...
        <ul className="space-y-3">
          {items.map((co) => {
            const staffPending = co.approvals.find(
              (a) => a.approverRole === 'staff' && a.status === 'pending',
            );
            ...
            return (
              <li key={co.id} className="rounded-md border border-border p-3">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div>
                    ...
                    <span className="text-xs text-fg-muted">
                      {formatDateTimeLabel(co.createdAt)}
                    </span>
                  </div>
                  <span className="flex flex-wrap items-center gap-2">
                    <Money kobo={co.amountDeltaKobo} /> · {co.scheduleDeltaDays >= 0 ? '+' : ''}
                    {co.scheduleDeltaDays}d
                    <StatusBadge
                      status={
                        co.status === 'approved'
                          ? 'accepted'
                          : co.status === 'submitted' ||
                              co.status === 'customer_review' ||
                              co.status === 'staff_review'
                            ? 'in_review'
                            : co.status
                      }
                      label={humanize(co.status)}
                    />
                  </span>
                </div>
                ...
                <div className="mt-2 flex flex-wrap gap-1 text-xs">
                  <Badge
                    tone={
                      co.approvalPolicy.outcome === 'approved'
                        ? 'success'
                        : co.approvalPolicy.outcome === 'rejected'
                          ? 'danger'
                          : 'warning'
                    }
                  >
                    Policy {humanize(co.approvalPolicy.outcome)}
                    {co.approvalPolicy.missing.length > 0
                      ? ` · missing ${co.approvalPolicy.missing.join(', ')}`
                      : ''}
                  </Badge>
                  {co.approvals.map((a) => (
                    <Badge
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  {co.status === 'draft' && perms.manage ? (
                    <ApiAction
                      path={`/api/v1/change-orders/${co.id}/submit`}
                      label="Submit for approval"
                      variant="primary"
                      body={{ expectedVersion: co.version }}
                      confirm={{
                        title: `Submit CO-${co.number}?`,
                        description: 'Creates the pending approvals required by the policy.',
                        confirmLabel: 'Submit',
                      }}
                  ...
                  {staffPending && perms.changeOrdersApprove ? (
                    <>
                      <ApiAction
                        path={`/api/v1/change-orders/${co.id}/approve`}
                        label="Approve (staff)"
                        variant="primary"
                        body={{
                          approverRole: 'staff',
                        ...
                        reasonKey="note"
                        confirm={{
                          title: `Approve CO-${co.number} as staff?`,
                          description:
                            'The budget changes only once the customer approval (if required) is also present.',
                          confirmLabel: 'Approve',
                        }}
                        successMessage="Staff approval recorded"
                      />
                      <ApiAction
                        path={`/api/v1/change-orders/${co.id}/approve`}
                        label="Reject"
                  ...
                  {staffPending && !perms.changeOrdersApprove ? (
                    <span className="text-xs text-fg-muted">
                      Staff approval needs change_orders.staff_approve.
                    </span>
                  ) : null}
                  {['submitted', 'customer_review', 'staff_review'].includes(co.status) &&
                  isCreator &&
                  co.approvals.every((a) => a.status === 'pending') ? (
                    <ApiAction
                      path={`/api/v1/change-orders/${co.id}/withdraw`}
                      label="Withdraw"
                      variant="ghost"
                      body={{ expectedVersion: co.version }}
                      reasonKey="reason"

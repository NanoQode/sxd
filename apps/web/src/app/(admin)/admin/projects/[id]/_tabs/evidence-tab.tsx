  ...
  return (
    ...
    >
      <DataTable
        ...
        columns={[
            ...
            cell: (e) => (
              <span>
                ...
                <span className="block text-xs text-fg-muted">
                  {humanize(e.kind)} · {e.file?.declaredMime ?? 'unknown type'} ·{' '}
                  {e.file?.status ?? 'unknown scan state'}
                </span>
          ...
          },
          {
            ...
            cell: (e) =>
              e.capturedAt ? (
                formatDateTimeLabel(e.capturedAt)
              ) : (
          ...
          {
            key: 'received',
            header: 'Received (server)',
            cell: (e) => formatDateTimeLabel(e.receivedAt),
            hideOnMobile: true,
          },
          {
            key: 'gps',
            header: 'GPS',
            cell: (e) =>
              e.captureGps ? `${e.captureGps.lat.toFixed(4)}, ${e.captureGps.lon.toFixed(4)}` : '—',
            hideOnMobile: true,
          },
          {
            key: 'links',
            ...
            cell: (e) =>
              [
                e.siteVisitId ? 'visit' : null,
                e.reportId ? 'report' : null,
                e.defectId ? 'defect' : null,
              ]
                .filter(Boolean)
                .join(', ') || '—',
            hideOnMobile: true,
            ...
            cell: (e) => (
              <span className="flex flex-wrap items-center gap-1">
                ...
                {canApprove && e.publication !== 'approved' ? (
                  <ApiAction
                    path={`/api/v1/projects/${p.id}/evidence/${e.id}/publication`}
                    label="Approve for customer"
                    body={{ publication: 'approved' }}
                ...
                {canApprove && e.publication !== 'restricted' ? (
                  <ApiAction
                    ...
                    label="Restrict"
                    variant="ghost"
                    body={{ publication: 'restricted' }}
                    confirm={{
                      title: 'Restrict this evidence?',
                      description: 'The customer loses access to it immediately.',
                      confirmLabel: 'Restrict',
                      tone: 'danger',
                    }}
                    successMessage="Evidence restricted"
                  />
                ) : null}
              </span>
            ),
      ...
      <p className="text-xs text-fg-muted">
        Checksums (SHA-256) are stored with each record; compare with the original when authenticity
        is questioned.
      </p>
    </Section>
  );

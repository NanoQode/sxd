...
const CATEGORY_LABEL: Record<string, string> = {
  core: 'Core',
  expansion: 'Expansion workflows',
  regulated_gated: 'Regulated (review required)',
...
export function FeatureFlagsManager({ items }: { items: FeatureFlagDto[] }) {
  ...
  async function toggle(flag: FeatureFlagDto, enabled: boolean) {
    ...
    try {
      await apiFetch(`/api/v1/admin/feature-flags/${flag.key}`, {
        method: 'PATCH',
        body: { enabled, expectedUpdatedAt: flag.updatedAt },
      });
  ...
  async function confirm(reason: string) {
    if (!pending) return;
    await apiFetch(`/api/v1/admin/feature-flags/${pending.flag.key}`, {
      method: 'PATCH',
      body: {
        enabled: pending.enabled,
        reason,
        confirmKey: pending.flag.key,
        expectedUpdatedAt: pending.flag.updatedAt,
      },
    });
    toast({
      title: `${pending.flag.name} ${pending.enabled ? 'enabled' : 'disabled'}`,
      tone: 'success',
    });
    router.refresh();
  }

  const groups = ['core', 'expansion', 'regulated_gated', 'experimental']
    .map((c) => ({ c, flags: items.filter((f) => f.category === c) }))
    .filter((g) => g.flags.length > 0);

  return (
    <div className="space-y-6">
      {error ? (
        <Alert tone="danger" title="Could not change the flag">
          {error}
        </Alert>
      ) : null}
      {groups.map((g) => (
        <section key={g.c} aria-labelledby={`ff-${g.c}`} className="space-y-2">
          <h2 id={`ff-${g.c}`} className="text-lg font-semibold">
              ...
              >
                <div className="min-w-0">
                  <p className="font-medium">
                    {f.name} <code className="ml-1 font-mono text-xs text-fg-muted">{f.key}</code>
                  </p>
                  {f.description ? <p className="text-sm text-fg-muted">{f.description}</p> : null}
                  {f.reviewNote ? (
                    <p className="mt-1 text-xs text-warning">{f.reviewNote}</p>
                  ) : null}
      ...
      <ActionDialog
        ...
        tone={
          pending?.enabled && pending.flag.category === 'regulated_gated' ? 'danger' : 'primary'
        }
        requireReason
        reasonLabel="Review note (who reviewed, what operating model and providers are in place)"
        confirmText={
          pending?.enabled && pending.flag.category === 'regulated_gated'
            ? pending.flag.key
            : undefined
        }
        onConfirm={confirm}
      >
        {pending?.flag.category === 'regulated_gated' && pending.enabled ? (
          <Alert tone="danger" title="Regulated feature">
            Not an ordinary portal feature. Enabling without a compliant operating model, licensed
            providers and professional review is prohibited.
          </Alert>
        ) : null}
      </ActionDialog>
    </div>

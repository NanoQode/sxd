    ...
    <form onSubmit={onSubmit} noValidate className="space-y-4">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <Field
          label="Slug"
          required
          hint="URL segment, e.g. how-it-works."
      ...
      <Field
        label="Summary"
        hint="Short description used in listings and previews."
      ...
      <Field
        label="Body (Markdown)"
        hint="Scripts, inline event handlers and unknown protocols are stripped when rendered."
      >
        {({ id }) => (
          <Textarea id={id} rows={12} className="font-mono" {...form.register('bodyMarkdown')} />
        )}
      </Field>
      <Field
        label="Fields (JSON)"
        hint={
          CONTENT_FIELD_TEMPLATES[kind]
            ? `Structured data for ${humanize(kind)} pages.`
            : 'Optional structured data for this kind.'
        }

...
export default async function TendersPage({
  ...
  const status = tenderStatusSchema.safeParse(raw.status).success
    ? (raw.status as TenderListQuery['status'])
    : undefined;
  const query = tenderListQuerySchema.parse({
    status,
  ...
  const [loaded, orgs] = await Promise.all([
    attempt(() => listTendersView(identity, query)),
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        actions={
          can(identity, 'tenders.manage') ? <TenderForm organizations={organizations} /> : undefined
        }
      />
      ...
      <FilterBar>
        <FilterSelect
          ...
          label="Status"
          value={status}
          options={tenderStatusSchema.options.map((s) => ({ value: s, label: humanize(s) }))}
        />
        <FilterSelect
          name="organizationId"
          label="Organisation"
      ...
      <div className="flex justify-end">
        <ExportCsvButton
          ...
          columns={[
            ...
            {
              header: 'Effective deadline',
              value: (t) => t.timeline.effectiveSubmissionDeadlineAt,
            },
      ...
      <DataTable
        ...
        columns={[
          ...
          },
          {
            key: 'sealed',
            header: 'Bids',
            cell: (t) => (t.sealed ? <Badge tone="gold">sealed</Badge> : <Badge>open</Badge>),
          },
          {
            key: 'deadline',
            header: 'Deadline',

...
const LEAD_STATUS_TONE: Record<string, 'neutral' | 'primary' | 'success' | 'warning' | 'danger' | 'info'> = {
  new: 'info',
  contacted: 'primary',
  qualified: 'success',
  converted: 'success',
...
export default async function LeadsPage({
  ...
  searchParams: Promise<{ status?: string; assignedToUserId?: string; q?: string; cursor?: string }>;
}) {
  ...
  const [page, assignees] = await Promise.all([listLeads(identity, query), listStaffAssignees(identity)]);
    ...
    <div className="space-y-6">
      ...
      <form method="get" className="grid gap-3 rounded-lg border border-border bg-bg-elevated p-4 sm:grid-cols-[1fr_1fr_2fr_auto] sm:items-end">
        ...
        <label className="text-sm">
          ...
          <input name="q" defaultValue={query.q ?? ''} placeholder="Name or email" className="h-11 w-full rounded-md border border-border-strong bg-bg-elevated px-3 text-sm" />
        </label>
      ...
      {page.items.length === 0 ? (
        <EmptyState title="No leads match" description="Leads arrive from the public consultation form, quote requests and the explorer. Adjust the filters to see more." />
      ) : (
        <DataTable
          caption="Leads"
          rows={page.items}
          ...
          columns={[
            {
              ...
              cell: (l) => (
                <span>
                  <Link href={`/admin/leads/${l.id}`} className="font-medium text-primary underline">
                    {l.contactName}
                  </Link>
                  <br />
            ...
            { key: 'service', header: 'Interest', cell: (l) => l.interestServiceName ?? (l.goal ? humanize(l.goal) : '—') },
            { key: 'status', header: 'Status', cell: (l) => <Badge tone={LEAD_STATUS_TONE[l.status] ?? 'neutral'}>{humanize(l.status)}</Badge> },
            { key: 'assignee', header: 'Assignee', cell: (l) => l.assignedToName ?? <span className="text-fg-muted">Unassigned</span> },
            { key: 'created', header: 'Received', cell: (l) => formatDateTimeLabel(l.createdAt) },
            { key: 'converted', header: 'Converted', cell: (l) => (l.convertedServiceRequestId ? <StatusBadge status="completed" label="Converted" /> : '—'), hideOnMobile: true },
          ]}
        />
      )}
      {page.nextCursor ? (

...
export function StaffRolesManager({
  ...
  async function doGrant(reason: string) {
    if (!grant) return;
    await apiFetch('/api/v1/admin/staff-roles', {
      body: { action: 'grant', email: grant.email.trim(), role: grant.role, reason },
    });
  ...
  async function doRevoke(reason: string) {
    if (!revoke) return;
    await apiFetch('/api/v1/admin/staff-roles', {
      body: { action: 'revoke', userId: revoke.user.id, role: revoke.role, reason },
    });
  ...
  const columns: Column<StaffUserDto>[] = [
        ...
        <div className="flex flex-wrap gap-1">
          {u.roles.map((r) => (
            <span key={r.id} className="inline-flex items-center gap-1">
              <Badge tone={r.role === 'super_admin' ? 'gold' : 'primary'}>
                {r.role.replace(/_/g, ' ')}
              </Badge>
              {canManage && !(r.role === 'super_admin' && u.id === actorId) ? (
                <button
                  type="button"
                  className="sx-touch text-xs text-danger underline"
                  onClick={() => setRevoke({ user: u, role: r.role })}
                >
                  revoke
                </button>
              ) : null}
    ...
    },
    {
      ...
      header: 'MFA',
      cell: (u) => (
        <Badge tone={u.twoFactorEnabled ? 'success' : 'warning'}>
          {u.twoFactorEnabled ? 'Verified' : 'Not enrolled'}
        </Badge>
      ),
    },
    {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-end gap-3 rounded-lg border border-border bg-bg-elevated p-3">
          ...
          >
            {({ id }) => (
              <Input
                ...
                value={grant?.email ?? ''}
                onChange={(e) =>
                  setGrant({ email: e.target.value, role: grant?.role ?? 'data_editor' })
                }
              />
            )}
          </Field>
          <Field label="Role">
            {({ id }) => (
              <NativeSelect
                id={id}
                value={grant?.role ?? 'data_editor'}
                onChange={(e) =>
                  setGrant({ email: grant?.email ?? '', role: e.target.value as StaffRole })
                }
              >
                {STAFF_ROLES.map((r) => (
          ...
          <GrantButton
            grant={grant}
            onClick={() => setGrant((g) => (g ? ({ ...g, confirm: true } as typeof g) : g))}
          />
        </div>
      ) : null}
      <DataTable
        ...
        emptyMessage="No staff roles granted."
      />
      ...
      <ActionDialog
        open={Boolean(grant && (grant as { confirm?: boolean }).confirm)}
        onOpenChange={(o) =>
          !o && setGrant(grant ? { email: grant.email, role: grant.role } : null)
        }
        title={grant ? `Grant ${grant.role.replace(/_/g, ' ')} to ${grant.email}` : ''}
        description={grant ? roleDescriptions[grant.role].scope : undefined}
      ...
      <ActionDialog
        ...
        title={revoke ? `Revoke ${revoke.role.replace(/_/g, ' ')} from ${revoke.user.name}` : ''}
        description="Access is removed immediately; the grant history stays in the audit log."
        confirmLabel="Revoke"
        tone="danger"
        requireReason
        onConfirm={doRevoke}

...
import { Alert, Button, Dialog, DialogContent, DialogFooter, Field, Input, NativeSelect, humanize, useToast } from '@simplexd/ui';
...
export function RecordResponse({
  ...
  const [lines, setLines] = useState<Record<string, LineDraft>>(() => Object.fromEntries(items.map((i) => [i.id, { price: '', unit: i.unit, factor: '', lead: '' }])));
  ...
  const ready = (Boolean(supplierUserId) || supplierName.trim().length >= 2) && deliveryKobo !== null && lineOk;
        ...
        body: {
          ...
          lines: items.map((i) => {
            ...
            return {
              ...
              declaredConversion: differs && l.factor ? { fromUnit: l.unit.trim(), toUnit: i.unit, factor: l.factor, basis: 'supplier_declared' } : null,
  ...
  return (
    <>
      ...
      <Dialog open={open} onOpenChange={(v) => !busy && setOpen(v)}>
        <DialogContent title="Record a supplier response" description="Prices are per the supplier's unit. A different unit needs the supplier's declared conversion (1 supplier unit = factor × RFQ unit)." size="lg">
          <div className="space-y-3">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <Field label="Supplier account">
                {({ id }) => (
                  <NativeSelect id={id} value={supplierUserId} onChange={(e) => setSupplierUserId(e.target.value)}>
              ...
              {supplierUserId ? null : (
                <Field label="Supplier name" required>
                  {({ id }) => <Input id={id} value={supplierName} maxLength={200} onChange={(e) => setSupplierName(e.target.value)} />}
                </Field>
              )}
              <Field label="Delivery charge (₦)" required>
                {({ id }) => <Input id={id} inputMode="decimal" value={delivery} onChange={(e) => setDelivery(e.target.value)} />}
              </Field>
            ...
            {items.map((i) => {
              ...
              const differs = l.unit.trim() !== i.unit;
              return (
                <fieldset key={i.id} className="grid gap-2 rounded-md border border-border p-2 sm:grid-cols-4">
                  <legend className="px-1 text-xs font-medium">
                    {humanize(i.material)} · {i.specification} · {i.quantity} {i.unit}
                  </legend>
                  <Field label="Price per supplier unit (₦)" required>
                    {({ id }) => <Input id={id} inputMode="decimal" value={l.price} onChange={(e) => update(i.id, { price: e.target.value })} />}
                  </Field>
                  <Field label="Supplier unit" required>
                    {({ id }) => <Input id={id} value={l.unit} onChange={(e) => update(i.id, { unit: e.target.value })} />}
                  </Field>
                  <Field label={`1 ${l.unit || 'unit'} = ? ${i.unit}`} hint={differs ? (l.factor ? 'Supplier-declared conversion' : 'Blank: comparison shows this line as unknown') : 'Same unit'}>
                    {({ id, describedBy }) => <Input id={id} aria-describedby={describedBy} inputMode="decimal" disabled={!differs} value={differs ? l.factor : ''} onChange={(e) => update(i.id, { factor: e.target.value })} />}
                  </Field>
                  <Field label="Lead time (days)">
                    {({ id }) => <Input id={id} type="number" min={0} value={l.lead} onChange={(e) => update(i.id, { lead: e.target.value })} />}
                  </Field>
                </fieldset>
              );
            })}

...
import {
  listDeliveriesView,
  listPurchaseOrdersView,
  listRfqsView,
  supplierDirectory,
} from '@/lib/admin/server/procurement';
...
export default async function ProcurementPage({
  ...
  const tab: Tab =
    raw.tab === 'orders' || raw.tab === 'deliveries' || raw.tab === 'suppliers' ? raw.tab : 'rfqs';
  ...
  if (tab === 'rfqs') {
      ...
      <>
        <FilterBar hidden={{ tab: raw.tab }}>
          <FilterSelect
            ...
            label="Status"
            value={status}
            options={rfqStatusSchema.options.map((s) => ({ value: s, label: humanize(s) }))}
          />
          <FilterSelect
            name="organizationId"
            label="Organisation"
          ...
          columns={[
            {
              ...
              cell: (r) => (
                <span>
                  <Link
                    href={`/admin/procurement/rfqs/${r.id}`}
                    className="font-medium text-primary underline"
                  >
                    {r.reference}
                  </Link>
                  <span className="block text-xs text-fg-muted">{r.title}</span>
                </span>
              ),
            },
            {
            ...
            },
            {
              key: 'dl',
              header: 'Deadline',
              cell: (r) => (r.deadlineAt ? formatDateTimeLabel(r.deadlineAt) : '—'),
            },
            {
              key: 'mk',
              header: 'Delivery to',
              cell: (r) =>
                r.deliveryMarketName ??
                (r.deliveryAddress
        ...
        {next(res.value.nextCursor) ? (
          <Link href={next(res.value.nextCursor)!} className="text-sm underline">
            Next page
          </Link>
        ) : null}
      </>
    ...
    const status = purchaseOrderStatusSchema.safeParse(raw.status).success
      ? (raw.status as PurchaseOrderListQuery['status'])
      : undefined;
    const res = await attempt(() =>
      listPurchaseOrdersView(identity, {
        status,
        organizationId: raw.organizationId || undefined,
        cursor,
        limit: 50,
      ...
      <>
        <FilterBar hidden={{ tab: raw.tab }}>
          <FilterSelect
            ...
            label="Status"
            value={status}
            options={purchaseOrderStatusSchema.options.map((s) => ({
              value: s,
              label: humanize(s),
            }))}
          />
          ...
          columns={[
            {
              ...
              cell: (p) => (
                <Link
                  href={`/admin/procurement/purchase-orders/${p.id}`}
                  className="font-medium text-primary underline"
                >
                  {p.number}
                </Link>
            ...
            { key: 'st', header: 'Status', cell: (p) => humanize(p.status) },
            {
              key: 'tot',
              header: 'Total',
              cell: (p) => <Money kobo={p.totalKobo} currency={p.currency} />,
            },
            {
              key: 'exp',
              header: 'Expected',
              cell: (p) => (p.expectedDeliveryAt ? formatDateTimeLabel(p.expectedDeliveryAt) : '—'),
              hideOnMobile: true,
            },
          ]}
        />
    ...
    const status = deliveryStatusSchema.safeParse(raw.status).success
      ? (raw.status as DeliveryDto['status'])
      : undefined;
      ...
      <>
        <FilterBar hidden={{ tab: raw.tab }}>
          <FilterSelect
            ...
            label="Status"
            value={status}
            options={deliveryStatusSchema.options.map((s) => ({ value: s, label: humanize(s) }))}
          />
        </FilterBar>
        <DataTable
          caption="Deliveries"
          ...
          columns={[
            {
              ...
              cell: (d) => (
                <Link
                  href={`/admin/procurement/purchase-orders/${d.purchaseOrderId}`}
                  className="underline"
                >
            ...
            },
            {
              key: 'at',
              header: 'Delivered',
              cell: (d) => (d.deliveredAt ? formatDateTimeLabel(d.deliveredAt) : '—'),
            },
            { key: 'st', header: 'Status', cell: (d) => humanize(d.status) },
            {
              ...
              cell: (d) => {
                const open = d.discrepancies.filter((x) =>
                  ['open', 'supplier_notified'].includes(x.status),
                ).length;
                return open ? (
                  <Badge tone="warning">{open} open</Badge>
                ) : d.discrepancies.length ? (
                  `${d.discrepancies.length} closed`
                ) : (
                  'none'
                );
              },
    ...
    const material = procurementMaterialSchema.safeParse(raw.material).success
      ? raw.material
      : undefined;
      ...
      <>
        ...
        <FilterBar hidden={{ tab: raw.tab }}>
          <FilterSelect
            ...
            label="Material"
            value={material}
            options={procurementMaterialSchema.options.map((m) => ({
              value: m,
              label: humanize(m),
            }))}
          />
          ...
          columns={[
            ...
            { key: 'p', header: 'Price evidence', cell: (s) => humanize(s.priceEvidence) },
            {
              ...
              cell: (s) =>
                s.coverage.length
                  ? s.coverage
                      .map((c) => `${c.marketName ?? 'market'} (${c.relationLabel})`)
                      .join(', ')
                  : 'none recorded',
              hideOnMobile: true,
            },
            {
              key: 'contact',
              header: 'Contact permitted',
              cell: (s) => (s.contactPermission ? 'yes' : 'no'),
              hideOnMobile: true,
            },
          ]}
        />
  ...
  return (
    <div className="space-y-6">
      ...
      <TabNav label="Procurement views">
        ...
        <TabLink href={tabHref('orders')} active={tab === 'orders'}>
          Purchase orders
        </TabLink>
        <TabLink href={tabHref('deliveries')} active={tab === 'deliveries'}>
          Deliveries
        </TabLink>
        <TabLink href={tabHref('suppliers')} active={tab === 'suppliers'}>
          Supplier directory
        </TabLink>
      </TabNav>

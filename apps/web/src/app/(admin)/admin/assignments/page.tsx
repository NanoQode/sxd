...
export default async function AssignmentsPage({
  ...
  const data = await workloadView(identity, {
    status,
  ...
  const entries =
    view === 'calendar'
      ? await calendarEntries(identity, {
          from: weekStart,
          to: weekEnd,
          staffUserId: raw.assignee || undefined,
        })
      : [];
    ...
    <div className="space-y-6">
      ...
      <FilterBar hidden={{ view: raw.view }}>
        <FilterSelect
          ...
          value={raw.assignee}
          allLabel="Everyone"
          options={data.workload.map((w) => ({ value: w.userId, label: `${w.name} (${w.kind})` }))}
        />
        {view === 'list' ? (
          <FilterSelect
            name="status"
            label="Status"
            value={status}
            options={assignmentStatusSchema.options.map((s) => ({ value: s, label: humanize(s) }))}
          />
        ) : null}
        {view === 'calendar' ? (
            ...
            columns={[
              {
                ...
                cell: (w) => (
                  <span>
                    <Link
                      href={q({ view: 'list', assignee: w.userId })}
                    ...
                    <span className="block text-xs text-fg-muted">
                      {w.kind === 'staff'
                        ? w.roles.map(humanize).join(', ')
                        : `${humanize(w.partnerType ?? 'partner')} · ${humanize(w.verificationStatus ?? 'unverified')}`}
                    </span>
                  </span>
                ),
              },
              {
                ...
                header: 'Active',
                cell: (w) => (
                  <Badge tone={w.activeTotal > 5 ? 'warning' : 'neutral'}>{w.activeTotal}</Badge>
                ),
              },
              {
                key: 'proposed',
                header: 'Proposed',
                cell: (w) => w.assignments.proposed ?? 0,
                hideOnMobile: true,
              },
              {
                key: 'completed',
                header: 'Completed',
                cell: (w) => w.assignments.completed ?? 0,
                hideOnMobile: true,
              },
              {
                key: 'declined',
                header: 'Declined / revoked',
                cell: (w) => `${w.assignments.declined ?? 0} / ${w.assignments.revoked ?? 0}`,
                hideOnMobile: true,
              },
            ...
            columns={[
              {
                ...
                cell: (a) => (
                  <Link
                    href={
                      a.serviceRequestId
                        ? `/admin/service-requests/${a.serviceRequestId}`
                        : `/admin/projects/${a.projectId}`
                    }
                    className="underline"
                  >
                    {a.targetLabel}
              ...
              },
              {
                key: 'org',
                header: 'Organisation',
                cell: (a) => a.organizationName,
                hideOnMobile: true,
              },
              {
                key: 'who',
                header: 'Assignee',
                cell: (a) => `${a.assigneeName ?? a.assigneeUserId} · ${humanize(a.role)}`,
              },
              {
                key: 'status',
                header: 'Status',
              ...
              },
              {
                ...
                header: 'Window',
                cell: (a) =>
                  `${a.startsAt ? formatDateTimeLabel(a.startsAt) : '—'} → ${a.endsAt ? formatDateTimeLabel(a.endsAt) : '—'}`,
                hideOnMobile: true,
              },
              {
                key: 'actions',
          ...
          actions={
            <>
              <Link
                href={q({
                  view: 'calendar',
                  week: new Date(weekStart.getTime() - 7 * 86_400_000).toISOString().slice(0, 10),
                })}

...
function ModeBadge({ dto }: { dto: IntegrationConfigDto }) {
  return dto.developmentAdapter ? (
    <Badge tone="warning">development adapter</Badge>
  ) : (
    <Badge tone="primary">{dto.adapter}</Badge>
  );
}
...
/**
 * Form for one configuration version. Mounted with a key per
 * environment/version so switching versions remounts it with fresh
 * defaults instead of resetting state inside an effect.
 */
function VersionForm({
  ...
  descriptor: IntegrationDetailResponse['descriptor'];
  selected: IntegrationConfigDto | null;
  hasActive: boolean;
  canWrite: boolean;
  canRotate: boolean;
  canTest: boolean;
  ...
  onTest: (version: number) => Promise<void>;
  ...
  const onSubmit = form.handleSubmit(async (values) => {
    ...
    await onSave({ adapter, settings, secrets, clearSecrets });
  });
    ...
    <Card>
      <CardHeader>
        <div className="flex flex-wrap items-center justify-between gap-2">
          <CardTitle>
            {selected ? `Version ${selected.version}` : 'No saved configuration'}
          </CardTitle>
          {selected ? (
            <div className="flex flex-wrap items-center gap-2">
              <StatusBadge
                status={selected.status}
                label={statusLabel(selected.status, selected)}
              />
        ...
        {selected?.remedialAction ? (
          <p className="text-sm text-fg-muted">{selected.remedialAction}</p>
        ) : null}
      </CardHeader>
      <CardContent>
        <form onSubmit={onSubmit} noValidate className="space-y-5">
          ...
          <Field
            ...
            required
            hint={
              descriptor.devAdapter
                ? 'Development adapters are labelled and refused in production.'
                : undefined
            }
          >
            {({ id }) => (
                  ...
                  <Field
                    ...
                    hint={
                      <span>
                        {presence?.set ? (
                          <>
                            Set (fingerprint <Mono>{presence.fingerprint}</Mono>, entered{' '}
                            {fmtDate(presence.updatedAt)}). Leave blank to keep it.
                          </>
                        ) : (
                          'Not set.'
                        )}{' '}
                    ...
                    {({ id, describedBy }) => (
                      <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
                        <Input
                          ...
                          disabled={clearing}
                          onChange={(e) =>
                            setSecretInputs((prev) => ({ ...prev, [s.key]: e.target.value }))
                          }
                        />
          ...
          <div className="flex flex-wrap gap-2 border-t border-border pt-4">
            ...
            <Button
              type="button"
              variant="secondary"
              disabled={!selected || !canTest || busy !== null}
              loading={busy === 'test'}
              loadingLabel="Testing"
              onClick={() => selected && onTest(selected.version)}
            >
              Test connection
            </Button>
            <Button
              ...
              disabled={
                ...
                !canWrite ||
                busy !== null ||
                selected.lastCheckOk !== true
              }
              loading={busy === 'activate'}
              loadingLabel="Activating"
              onClick={() => selected && onActivate(selected.version)}
              title={
                selected && selected.lastCheckOk !== true
            ...
            {selected && !selected.isActive && selected.lastCheckOk !== true && canWrite ? (
              <Button type="button" variant="ghost" disabled={busy !== null} onClick={onForce}>
            ...
            <Button
              type="button"
              variant="danger"
              disabled={!hasActive || !canWrite || busy !== null}
              onClick={onDisable}
            >
              Disable
            </Button>
            <Button
              type="button"
              variant="secondary"
              disabled={!selected || !canRotate || busy !== null}
              onClick={onRotate}
            >
              Rotate secret…
            </Button>
          ...
          <p className="text-xs text-fg-muted">
            Save writes a new version and never activates it. Test runs the real provider check and
            records the outcome. Activate switches the runtime to a version that passed its test.
          </p>
        </form>
      </CardContent>
    </Card>
...
export function ProviderPanel({
  ...
  const selected: IntegrationConfigDto | null =
    envData.versions.find((v) => v.version === selectedVersion) ??
    envData.active ??
    envData.versions[0] ??
    null;
  ...
  async function saveVersion(body: SavePayload) {
    const saved = await run('save', () =>
      apiFetch<IntegrationConfigDto>(base, { method: 'PUT', body: { environment, ...body } }),
    );
    if (saved) {
      toast({
        title: `Version ${saved.version} saved`,
        description: 'Saved, not tested. Run the connection test next.',
        tone: 'success',
      });
  ...
  async function onTest(version: number) {
    const result = await run('test', () =>
      apiFetch<IntegrationCheckResult>(`${base}/test`, {
        method: 'POST',
  ...
  async function activate(version: number, force: boolean, reason?: string) {
    const dto = await apiFetch<IntegrationConfigDto>(`${base}/activate`, {
      method: 'POST',
      body: { environment, version, force, reason },
    });
    toast({
      title: `Version ${dto.version} is now active`,
      description: statusLabel(dto.status, dto),
      tone: dto.status === 'connected' ? 'success' : 'info',
    });
  ...
  async function onDisable(reason: string) {
    await apiFetch<IntegrationConfigDto>(`${base}/disable`, {
      method: 'POST',
      body: { environment, reason },
    });
  ...
  async function onRotate(reason: string) {
    ...
    const res = await apiFetch<{
      ...
      method: 'POST',
      body: { environment, field: rotateField, value: rotateValue, reason },
    });
    setRotateValue('');
    toast({
  ...
  const versionColumns: Column<IntegrationConfigDto>[] = [
    ...
    },
    {
      key: 'status',
      header: 'State',
      cell: (v) => <StatusBadge status={v.status} label={statusLabel(v.status, v)} />,
    },
      ...
      cell: (v) =>
        v.lastCheckAt ? (
          <span className="text-xs">
            {v.lastCheckOk ? 'passed' : 'failed'} · {fmtDate(v.lastCheckAt)}
          </span>
        ) : (
  ...
  const logColumns: Column<IntegrationLogDto>[] = [
    {
      key: 'at',
      header: 'When',
      cell: (l) => <span className="text-xs">{fmtDate(l.createdAt)}</span>,
    },
    {
      key: 'env',
      header: 'Env',
      cell: (l) => (
        <Badge tone={l.environment === 'live' ? 'gold' : 'neutral'}>{l.environment}</Badge>
      ),
    },
    {
      key: 'level',
      header: 'Level',
      cell: (l) => (
        <Badge tone={l.level === 'error' ? 'danger' : l.level === 'warn' ? 'warning' : 'neutral'}>
          {l.level}
        </Badge>
      ),
    },
    { key: 'event', header: 'Event', cell: (l) => <Mono>{l.event}</Mono> },
    {
      key: 'message',
      header: 'Message',
      cell: (l) => <span className="text-sm">{l.message ?? '—'}</span>,
    },
  ];
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center gap-3">
        <Field label="Environment" className="min-w-48">
          {({ id }) => (
            <NativeSelect
              ...
              value={environment}
              onChange={(e) => {
                setEnvironment(e.target.value as IntegrationEnvironment);
        ...
        <p className="text-sm text-fg-muted">
          {isDefaultEnv
            ? `The runtime reads the ${environment} environment (APP_ENV=${detail.appEnv}).`
            : `The runtime currently reads the ${detail.defaultEnvironment} environment; ${environment} settings are kept separate.`}
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-[2fr_1fr]">
        <div className="space-y-6">
          <VersionForm
            key={`${environment}:${selected?.id ?? 'none'}`}
            descriptor={descriptor}
            selected={selected}
            hasActive={Boolean(envData.active)}
            canWrite={canWrite}
            canRotate={canRotate}
            canTest={permissions.test}
            ...
            onForce={() => setForceDialog(true)}
            onDisable={() => setDisableDialog(true)}
            onRotate={() => setRotateDialog(true)}
          />

          {check ? (
            <Alert
              tone={check.ok ? 'success' : 'danger'}
              title={`${check.ok ? 'Test passed' : 'Test failed'} · version ${check.config.version} · ${check.mode === 'development' ? 'development adapter (not a real provider)' : check.adapter}`}
            >
              <p>{check.message}</p>
              <p className="mt-1 text-xs text-fg-muted">
                Checked {fmtDate(check.checkedAt)}
                {check.environmentDetected
                  ? ` · environment detected: ${check.environmentDetected}`
                  : ''}
              </p>
          ...
          <Card>
            <CardHeader>
              <CardTitle>Versions · {environment}</CardTitle>
            </CardHeader>
            <CardContent>
              <DataTable
                columns={versionColumns}
                rows={envData.versions}
                rowKey={(v) => v.id}
                rowLabel={(v) => `version ${v.version}`}
                caption={`${descriptor.name} ${environment} versions`}
                emptyMessage="Nothing saved for this environment yet."
              />
            </CardContent>
          </Card>
        ...
        <div className="space-y-6">
          <Card>
            ...
            <CardContent className="space-y-3 text-sm">
              <p>
                Guide: <Mono>{descriptor.docsPath}</Mono> (repository). The index at{' '}
                <Mono>docs/providers/README.md</Mono> lists what to supply, where to enter it, how
                to test, activate and rotate.
              </p>
              ...
              {descriptor.provider === 'google_workspace' ? (
                <Alert tone="info" title="Organiser grant is separate">
                  A client ID and secret do not connect a calendar. After activating, an organiser
                  must complete the OAuth grant at{' '}
                  <Link href="/api/v1/calendar/connect" className="underline">
                    /api/v1/calendar/connect
                  </Link>
                  ; token health is shown there.
                </Alert>
              ) : null}
              {selected ? (
                <dl className="grid grid-cols-[max-content_1fr] gap-x-3 gap-y-1 text-xs">
                  <dt className="text-fg-muted">Last success</dt>
                  <dd>{fmtDate(selected.lastSuccessAt)}</dd>
                  <dt className="text-fg-muted">Credential rotated</dt>
                  <dd>{fmtDate(selected.credentialRotatedAt)}</dd>
                  <dt className="text-fg-muted">Activated</dt>
                  <dd>{fmtDate(selected.activatedAt)}</dd>
      ...
      <ActionDialog
        ...
        requireReason
        onConfirm={(reason) =>
          selected ? activate(selected.version, true, reason) : Promise.resolve()
        }
      />
      <ActionDialog
        open={disableDialog}
        ...
        <Field label="Secret" required>
          {({ id }) => (
            <NativeSelect
              id={id}
              value={rotateField}
              onChange={(e) => setRotateField(e.target.value)}
            >
              {descriptor.secrets.map((s) => (

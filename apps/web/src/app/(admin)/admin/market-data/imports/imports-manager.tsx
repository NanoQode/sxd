...
export function ImportsManager({
  ...
  async function runPreview() {
    ...
    try {
      ...
      const res = await apiFetch<ImportDto>('/api/v1/admin/market-imports/preview', {
        body: { fileName: file.name, format, content },
      });
      setPreview(res);
      router.refresh();
    } catch (err) {
  ...
  async function apply(reason: string) {
    if (!applying) return;
    const res = await apiFetch<ImportDto>(`/api/v1/admin/market-imports/${applying.id}/apply`, {
      method: 'POST',
      body: { reason: reason || undefined },
    });
    toast({
      title: 'Import applied',
      description: `${res.conflicts.length} conflict(s) reported`,
      tone: 'success',
    });
    setPreview(res);
    router.refresh();
  ...
  const columns: Column<ImportDto>[] = [
      ...
      cell: (i) => (
        <span className="font-medium">
          ...
          <span className="block text-xs text-fg-muted">
            {i.format} · {i.uploadedByName ?? i.uploadedBy ?? '—'}
          </span>
        </span>
      ),
    },
    {
      ...
      cell: (i) => (
        <Badge
          tone={i.status === 'applied' ? 'success' : i.status === 'failed' ? 'danger' : 'neutral'}
        >
          {i.status}
        </Badge>
      ),
    ...
    {
      key: 'errors',
      header: 'Errors / conflicts',
      cell: (i) => `${i.rowErrors.length} / ${i.conflicts.length}`,
    },
    {
      key: 'created',
      header: 'Previewed',
      cell: (i) => <span className="text-xs">{fmtDate(i.createdAt)}</span>,
    },
    {
      key: 'applied',
      header: 'Applied',
      cell: (i) => <span className="text-xs">{fmtDate(i.appliedAt)}</span>,
    },
    {
      key: 'actions',
      header: 'Actions',
      cell: (i) => (
        <div className="flex gap-1">
          ...
          {i.status === 'previewed' && i.rowErrors.length === 0 ? (
            <Button size="sm" onClick={() => setApplying(i)}>
              Apply
            </Button>
          ) : null}
        </div>
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle>Preview a file</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          {error ? (
            <Alert tone="danger" title="Preview failed">
              {error}
            </Alert>
          ) : null}
          ...
          <Button onClick={runPreview} disabled={!file} loading={busy} loadingLabel="Previewing">
            Preview (dry run)
          </Button>
          {format === 'observations_csv' ? (
            <details className="text-sm">
              ...
              <p className="mt-1 text-fg-muted">
                Header names match the observation contract. <code>sourceSlug</code>,{' '}
                <code>marketSlug</code> and <code>stateName</code> resolve ids; a <code>slug</code>{' '}
                column makes re-imports skip existing rows.
              </p>
              <code className="mt-1 block break-all rounded bg-bg-sunken p-2 font-mono text-xs">
                {csvColumns.join(',')}
              </code>
            </details>
          ) : null}
        ...
        <Card>
          <CardHeader>
            <div className="flex flex-wrap items-center justify-between gap-2">
              ...
              {preview.status === 'previewed' && preview.rowErrors.length === 0 ? (
                <Button onClick={() => setApplying(preview)}>Apply this import</Button>
              ) : null}
            </div>
          </CardHeader>
          <CardContent className="space-y-4 text-sm">
            {preview.rowErrors.length > 0 ? (
              <Alert
                tone="danger"
                title={`${preview.rowErrors.length} row-level error(s); fix the file and preview again`}
              >
            ...
            {preview.conflicts.length > 0 ? (
              <Alert
                tone="warning"
                title={`${preview.conflicts.length} conflict(s): human-edited records are not overwritten`}
              >
                <ul className="mt-1 max-h-48 list-disc overflow-y-auto pl-5">

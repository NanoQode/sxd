...
import { Alert, Badge, Button, Card, CardContent, CardHeader, CardTitle, PageHeader, StatusBadge } from '@simplexd/ui';
...
export default async function MarketDetailPage({
  ...
  const [observations, suppliers, coverage, neighborhoods, flags, tasks, revisions, sources, staff, marketOptions, audit] =
    await Promise.all([
        ...
        <CardContent>
          <DefinitionList
            items={[
              ...
              { term: 'Aliases', value: market.aliases.length > 0 ? market.aliases.join(', ') : null },
              { term: 'State / zone', value: `${market.stateName} (${market.geopoliticalZone})` },
              { term: 'Parent market', value: market.parent ? <Link className="text-primary underline" href={`/admin/market-data/markets/${market.parent.id}`}>{market.parent.name}</Link> : null },
              { term: 'Overlap note', value: market.overlapNote },
              { term: 'Reference point', value: `${market.location.lon}, ${market.location.lat} (lon, lat)${market.coordinateAccuracy ? ` · ${market.coordinateAccuracy}` : ''}` },
              ...
              { term: 'Imported', value: market.importedAt ? `${fmtDate(market.importedAt)}` : 'Created by hand' },
              { term: 'Human edited', value: fmtDate(market.humanEditedAt) },
              { term: 'Linked records', value: `${market.linkedProjects} projects · ${market.linkedProperties} properties · ${market.linkedServiceRequests} requests` },
            ]}
          />
          {market.profileMarkdown ? (
            <div className="sx-prose mt-6 border-t border-border pt-4 text-sm" dangerouslySetInnerHTML={{ __html: renderMarkdown(market.profileMarkdown) }} />
          ) : (
            <p className="mt-6 border-t border-border pt-4 text-sm text-fg-muted">No profile text yet.</p>
          )}
        </CardContent>
      </Card>
        ...
        <Card>
          ...
          <CardContent className="space-y-3 text-sm">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Badge tone={market.serviceAvailability === 'available' ? 'success' : 'neutral'}>{humanize(market.serviceAvailability)}</Badge>
            </div>
            <DefinitionList
              items={[
                ...
                { term: 'Merged into', value: market.mergedInto ? <Link className="text-primary underline" href={`/admin/market-data/markets/${market.mergedInto.id}`}>{market.mergedInto.name}</Link> : null },
              ]}
            />
            <p className="text-xs text-fg-muted">
              Map publication and service availability are separate decisions; a visible marker does not
              claim a staffed operation.
            </p>
            <ProfileActions
              market={{ id: market.id, name: market.name, version: market.version, publicationState: market.publicationState, location: market.location, mergedIntoMarketId: market.mergedIntoMarketId, coordinateSourceId: market.coordinateSourceId, coordinateAccuracy: market.coordinateAccuracy, linkedProjects: market.linkedProjects }}
              canEdit={canEdit}
  ...
  const observationsTab = (
    <div className="space-y-6">
      ...
      <ObservationTable items={observations.local} actorId={actorId} canPublish={canPublish} canEdit={canEdit} caption="Local observations" showMarket={false} emptyMessage="No local observations. Statewide context is never cloned into city rows; record local comparables instead." />
      <h2 className="text-lg font-semibold">Statewide context ({market.stateName})</h2>
      <Alert tone="info">Statewide figures are shown as regional context and are never rank-eligible for this market.</Alert>
      <ObservationTable items={observations.statewide} actorId={actorId} canPublish={canPublish} canEdit={canEdit} caption="Statewide context observations" emptyMessage="No statewide observations for this state." />
    </div>
  );
    ...
    <div className="space-y-6">
      ...
      {market.mergedIntoMarketId ? (
        <Alert tone="warning" title="This market was merged">
          ...
          <Link href={`/admin/market-data/markets/${market.mergedIntoMarketId}`} className="font-medium text-primary underline">
      ...
      <MarketTabs
        ...
        tabs={[
          ...
          { key: 'observations', label: `Observations (${observations.local.length})`, content: observationsTab },
          { key: 'suppliers', label: `Suppliers (${suppliers.leads.length})`, content: <SuppliersTab leads={suppliers.leads} quotes={suppliers.quotes} /> },
          { key: 'coverage', label: 'Coverage', content: <CoverageEditor marketId={market.id} rows={coverage} canEdit={canEdit} /> },
          { key: 'neighborhoods', label: `Neighborhoods (${neighborhoods.length})`, content: <NeighborhoodsTab marketId={market.id} items={neighborhoods} sources={sourceOptions} canEdit={canEdit} canPublish={canPublish} /> },
          { key: 'flags', label: `Flags (${flags.filter((f) => f.active).length})`, content: <FlagsTab marketId={market.id} items={flags} sources={sourceOptions} neighborhoods={neighborhoods.map((n) => ({ id: n.id, name: n.name }))} canEdit={canEdit} canPublish={canPublish} /> },
          { key: 'research', label: `Research (${tasks.filter((t) => t.status !== 'done').length})`, content: <ResearchTasksTab marketId={market.id} items={tasks} staff={staff} canEdit={canEdit} /> },
          { key: 'revisions', label: `Revisions (${revisions.length})`, content: canHistory ? <RevisionsTab marketId={market.id} currentVersion={market.version} items={revisions} canEdit={market.publicationState === 'published' ? canPublish : canEdit} /> : <Alert tone="info">You do not have permission to read revision history.</Alert> },
          { key: 'audit', label: 'Audit', content: canAudit ? <AuditTable items={audit} caption={`Audit entries for ${market.name}`} /> : <Alert tone="info">You do not have permission to read the audit log.</Alert> },
        ]}
      />
    </div>
  );

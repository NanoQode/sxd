...
export function FlagsTab({
  ...
  const [form, setForm] = useState({
    flagType: 'data_dispute' as (typeof FLAG_TYPES)[number],
    note: '',
    neighborhoodId: '',
    sourceId: '',
    validFrom: '',
  ...
  async function applyToggle(reason: string) {
    if (!toggle) return;
    await apiFetch(`/api/v1/admin/markets/${marketId}/flags/${toggle.id}`, {
      method: 'PATCH',
      body: { active: !toggle.active, reason, expectedUpdatedAt: toggle.updatedAt },
    });
  ...
  const columns: Column<MarketFlagDto>[] = [
    {
      ...
      header: 'Flag',
      cell: (f) => (
        <Badge tone={f.active ? (APPROVER_ONLY.has(f.flagType) ? 'danger' : 'warning') : 'neutral'}>
          {humanize(f.flagType)}
        </Badge>
      ),
    },
    ...
    { key: 'active', header: 'Active', cell: (f) => (f.active ? 'Yes' : 'No') },
    {
      ...
      header: 'Created',
      hideOnMobile: true,
      cell: (f) => <span className="text-xs text-fg-muted">{fmtDate(f.createdAt)}</span>,
    },
    {
      key: 'actions',
      header: 'Actions',
      cell: (f) =>
        canChange(f.flagType) ? (
          <Button size="sm" variant="secondary" onClick={() => setToggle(f)}>
  ...
  return (
    <div className="space-y-4">
      <Alert tone="info" title="Flags never certify safety">
        Exclusions, title stops and site restrictions block ranking and need the approver.
        Advisories are shown to customers as dated, attributed context. Unknown flood or title
        status is never shown as low risk.
      </Alert>
      ...
      <DataTable
        ...
        rows={items}
        rowKey={(f) => f.id}
        rowLabel={(f) => humanize(f.flagType)}
        caption="Market flags"
      ...
      <Dialog open={open} onOpenChange={(o) => !busy && setOpen(o)}>
        <DialogContent title="Add flag">
          <div className="space-y-4">
            {error ? (
              <Alert tone="danger" title="Could not create the flag">
                {error}
              </Alert>
            ) : null}
            <Field
              ...
              required
              hint={
                APPROVER_ONLY.has(form.flagType)
                  ? 'Blocking flag: requires the approver permission.'
                  : undefined
              }
            >
              {({ id }) => (
                <NativeSelect
                  ...
                  onChange={(e) =>
                    setForm((f) => ({
                      ...f,
                      flagType: e.target.value as (typeof FLAG_TYPES)[number],
                    }))
                  }
                >
      ...
      <ActionDialog
        ...
        title={
          toggle
            ? `${toggle.active ? 'Deactivate' : 'Reactivate'} ${humanize(toggle.flagType)}`
            : ''
        }

...
export function SourcesManager({ items, canEdit }: { items: SourceDto[]; canEdit: boolean }) {
  ...
  async function save() {
    ...
    try {
      const body = {
        ...
        url: form.url || null,
        dataUrl: form.dataUrl || null,
        licenseNote: form.licenseNote || null,
        licenseRights: form.licenseRights,
        useNote: form.useNote || null,
      ...
      if (editing === 'new') {
        ...
        await apiFetch(`/api/v1/admin/sources/${editing.id}`, {
          method: 'PATCH',
          body: { ...body, reason: form.reason, expectedUpdatedAt: editing.updatedAt },
        });
  ...
  const filtered = items.filter(
    (s) =>
      !q || `${s.title} ${s.slug} ${s.publisher ?? ''}`.toLowerCase().includes(q.toLowerCase()),
  );
  const columns: Column<SourceDto>[] = [
    ...
    },
    {
      ...
      cell: (s) => (
        <Badge
          tone={
            s.licenseRights === 'first_party' || s.licenseRights === 'licensed_commercial'
              ? 'success'
              : s.licenseRights === 'unknown'
                ? 'warning'
                : 'neutral'
          }
        >
          {humanize(s.licenseRights)}
        </Badge>
      ),
    },
    {
  ...
  return (
    <div className="space-y-4">
        ...
        <Field label="Filter" className="w-full sm:max-w-xs">
          {({ id }) => (
            <Input
              ...
              placeholder="Title, slug or publisher"
            />
          )}
        </Field>
      ...
      <Dialog open={editing !== null} onOpenChange={(o) => !o && !busy && setEditing(null)}>
        <DialogContent
          title={editing === 'new' ? 'Register a source' : `Edit ${editing?.title ?? ''}`}
          size="lg"
        >
          <div className="space-y-4">
            {error ? (
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <Field label="License rights">
                {({ id }) => (
                  <NativeSelect
                    id={id}
                    value={form.licenseRights}
                    onChange={(e) =>
                      setForm((f) => ({
                        ...f,
                        licenseRights: e.target.value as FormState['licenseRights'],
                      }))
                    }
                  >
                    {RIGHTS.map((r) => (
            ...
            <Field
              label="Permitted use"
              hint="What may be done with the data (e.g. attributed factual reference, no bulk ingestion)."
            >
              {({ id }) => (
                <Textarea
                  id={id}
                  className="min-h-20"
                  value={form.useNote}
                  onChange={(e) => setForm((f) => ({ ...f, useNote: e.target.value }))}
                />
              )}
            </Field>
            {editing !== 'new' ? (
            ...
            <DialogFooter>
              ...
              <Button
                ...
                loading={busy}
                loadingLabel="Saving"
                disabled={form.title.trim().length < 2 || (editing === 'new' && !form.slug.trim())}
              >
                Save
              </Button>
            </DialogFooter>

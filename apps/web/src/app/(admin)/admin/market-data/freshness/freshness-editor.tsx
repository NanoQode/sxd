...
export function FreshnessEditor({
  items,
  canManage,
}: {
  items: FreshnessPolicyDto[];
  canManage: boolean;
}) {
  ...
  async function save(reason: string) {
    ...
    if (!/^[a-z][a-z0-9_]{1,60}$/.test(type))
      throw new Error('Data type must be snake_case (e.g. rent_observation).');
    await apiFetch(`/api/v1/admin/freshness-policies/${type}`, {
      method: 'PUT',
      body: {
        maxAgeDays: maxAge.trim() === '' ? null : Number(maxAge),
        respectSourceValidity: respect,
        note: note || null,
        reason,
        expectedUpdatedAt: editing !== 'new' ? editing?.updatedAt : undefined,
      },
    });
  ...
  const columns: Column<FreshnessPolicyDto>[] = [
    {
      ...
      cell: (p) => (
        <span className="font-medium">
          {humanize(p.dataType)}
          <span className="block font-mono text-xs text-fg-muted">{p.dataType}</span>
        </span>
      ),
    },
    {
      key: 'age',
      header: 'Max age',
      cell: (p) => (p.maxAgeDays === null ? 'Source validity only' : `${p.maxAgeDays} days`),
    },
    {
      key: 'respect',
      header: 'Respect source validity',
      cell: (p) => (p.respectSourceValidity ? 'Yes' : 'No'),
    },
    {
      key: 'note',
      header: 'Note',
      cell: (p) => <span className="text-xs text-fg-muted">{p.note ?? '—'}</span>,
    },
    {
      key: 'updated',
      header: 'Updated',
    ...
    <div className="space-y-4">
      {canManage ? (
        <div className="flex justify-end">
          <Button onClick={() => open('new')}>Add data type</Button>
        </div>
      ) : null}
      <DataTable
        columns={columns}
        rows={items}
        rowKey={(p) => p.id}
        rowLabel={(p) => p.dataType}
        caption="Freshness policies"
      />
      <ActionDialog
        open={editing !== null}
        onOpenChange={(o) => !o && setEditing(null)}
        title={editing === 'new' ? 'Add freshness policy' : `Edit ${editing?.dataType ?? ''}`}
        confirmLabel="Save"
        requireReason
        onConfirm={save}
      >
        {editing === 'new' ? (
          <Field label="Data type (snake_case)" required>
            {({ id }) => (
              <Input id={id} value={dataType} onChange={(e) => setDataType(e.target.value)} />
            )}
          </Field>
        ) : null}
        <Field
          label="Maximum age in days"
          hint="Leave empty when only the source's own validity applies (official risk layers)."
        >
          {({ id }) => (
            <Input
              id={id}

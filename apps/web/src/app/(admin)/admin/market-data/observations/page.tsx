...
export default async function ObservationsPage({
  ...
  const parsed = observationListQuerySchema.safeParse(hasFilters ? raw : { ...raw, pendingOnly: 'true' });
  const query = parsed.success ? parsed.data : observationListQuerySchema.parse({ pendingOnly: 'true' });
  const ctx = adminContext(identity);
  const [result, metrics, markets] = await Promise.all([listObservations(ctx, query), listObservationMetrics(ctx), listMarketOptions(ctx)]);
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        ...
        actions={canEdit ? <Link href="/admin/market-data/observations/new"><Button>Record observation</Button></Link> : undefined}
      />
      <ObservationsQueue
        result={result}
        query={query}

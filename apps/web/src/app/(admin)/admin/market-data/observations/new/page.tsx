...
export default async function NewObservationPage({ searchParams }: { searchParams: Promise<{ marketId?: string }> }) {
  const identity = await requireStaffPage('market_data.edit');
  const { marketId } = await searchParams;
  const ctx = adminContext(identity);
  const [sources, markets, states] = await Promise.all([listSources(ctx, { limit: 500 }), listMarketOptions(ctx), listStates(ctx)]);
  return (
    <div className="space-y-6">
      ...
      />
      <ObservationForm sources={sources.map((s) => ({ id: s.id, title: s.title, licenseRights: s.licenseRights }))} markets={markets} states={states} defaultMarketId={marketId ?? ''} />
    </div>
  );
}

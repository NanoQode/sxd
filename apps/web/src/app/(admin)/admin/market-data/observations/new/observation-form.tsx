...
const date = z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'YYYY-MM-DD').or(z.literal(''));
...
const schema = z
  ...
  .superRefine((v, ctx) => {
    ...
    if (['median', 'mean', 'min', 'max', 'count', 'quote', 'single_observation'].includes(v.statistic) && num(v.value) === null)
      ctx.addIssue({ code: 'custom', path: ['value'], message: 'This statistic needs a numeric value' });
    if (v.statistic === 'range' && (num(v.valueLow) === null || num(v.valueHigh) === null))
      ctx.addIssue({ code: 'custom', path: ['valueLow'], message: 'A range needs low and high values' });
    if (v.statistic === 'categorical' && !v.valueText.trim())
      ctx.addIssue({ code: 'custom', path: ['valueText'], message: 'A categorical observation needs a text value' });
    if (v.geographyLevel === 'state_or_fct' && !v.stateId) ctx.addIssue({ code: 'custom', path: ['stateId'], message: 'Choose the state' });
    ...
    if (v.slug && !/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(v.slug)) ctx.addIssue({ code: 'custom', path: ['slug'], message: 'Lowercase letters, digits and hyphens' });
  });
...
export function ObservationForm({
    ...
    try {
      const created = await apiFetch<{ id: string }>('/api/v1/admin/observations', {
        body: {
          ...
          marketId: ['city', 'neighborhood', 'site'].includes(v.geographyLevel) ? v.marketId || null : null,
          ...
          periodCompleteAtRetrieval: v.periodCompleteAtRetrieval === 'unknown' ? null : v.periodCompleteAtRetrieval === 'yes',
  ...
  const errors = Object.entries(formState.errors).map(([k, e]) => ({ id: `of-${k}`, message: e?.message ?? k }));
  const f = (name: keyof Values) => ({ htmlFor: `of-${name}`, error: formState.errors[name]?.message });

  return (
    <form onSubmit={onSubmit} noValidate className="space-y-6">
      {serverError ? <Alert tone="danger" title="Could not record the observation">{serverError}</Alert> : null}
      ...
      <fieldset className="grid grid-cols-1 gap-4 rounded-lg border border-border p-4 md:grid-cols-2">
        ...
        <Field label="Source" required {...f('sourceId')} hint={source ? `Rights: ${humanize(source.licenseRights)}` : 'Register sources first if missing.'}>
          {({ id, invalid, describedBy }) => (
            <NativeSelect id={id} aria-invalid={invalid} aria-describedby={describedBy} {...register('sourceId')}>
        ...
        <Field label="Retrieved on" required {...f('retrievedAt')} hint="When you read it, not when it was measured.">
          {({ id, invalid, describedBy }) => <Input id={id} type="date" aria-invalid={invalid} aria-describedby={describedBy} {...register('retrievedAt')} />}
        </Field>
        ...
        <Field label="Collection method" {...f('collectionMethod')}>
          {({ id }) => (
            <NativeSelect id={id} {...register('collectionMethod')}>
              {['published_report_read', 'first_party_survey', 'supplier_quotation', 'official_publication', 'licensed_dataset', 'field_visit', 'other'].map((m) => (
        ...
        <Field label="Sample size" {...f('sampleSize')} hint="Listings or records behind the figure, if stated.">
          {({ id }) => <Input id={id} type="number" min={0} {...register('sampleSize')} />}
        </Field>
        <Field label="Stable slug (optional)" {...f('slug')} hint="Lets a CSV re-import skip this row.">
      ...
      <fieldset className="grid grid-cols-1 gap-4 rounded-lg border border-border p-4 md:grid-cols-3">
        ...
        <Field label="Metric key" required {...f('metric')} hint="e.g. annual_rent_median, sale_price_median, cement_50kg_delivered">
          {({ id, invalid, describedBy }) => <Input id={id} aria-invalid={invalid} aria-describedby={describedBy} {...register('metric')} />}
        </Field>
        ...
        <Field label="Property cohort" required {...f('propertyCohort')} hint="e.g. 3-bedroom flat, mixed advertised stock">
          {({ id, invalid, describedBy }) => <Input id={id} aria-invalid={invalid} aria-describedby={describedBy} {...register('propertyCohort')} />}
        </Field>
        {statistic === 'range' ? (
          <>
            <Field label="Low" required {...f('valueLow')}>
              {({ id, invalid }) => <Input id={id} type="number" step="any" aria-invalid={invalid} {...register('valueLow')} />}
            </Field>
            <Field label="High" required {...f('valueHigh')}>
              {({ id, invalid }) => <Input id={id} type="number" step="any" aria-invalid={invalid} {...register('valueHigh')} />}
            </Field>
          </>
        ) : statistic === 'categorical' ? (
          <Field label="Categorical value" required {...f('valueText')}>
            {({ id, invalid }) => <Input id={id} aria-invalid={invalid} {...register('valueText')} />}
          </Field>
        ) : (
          <Field label="Value" required {...f('value')}>
            {({ id, invalid }) => <Input id={id} type="number" step="any" aria-invalid={invalid} {...register('value')} />}
          </Field>
        )}
        <Field label="Unit" required {...f('unit')} hint="e.g. NGN/year, NGN/m2, days, bags">
          {({ id, invalid, describedBy }) => <Input id={id} aria-invalid={invalid} aria-describedby={describedBy} {...register('unit')} />}
        </Field>
        <Field label="Numeric representation" required {...f('numericRepresentation')} hint="Whole naira is not kobo.">
          {({ id }) => (
            <NativeSelect id={id} {...register('numericRepresentation')}>
              {NUMERIC_REPRESENTATIONS.map((n) => (
                <option key={n} value={n}>{humanize(n)}</option>
              ))}
            </NativeSelect>
          )}
        </Field>
        ...
        <Field label="Valid until" {...f('validUntil')} hint="Source-declared validity (e.g. quote expiry).">
      ...
      <fieldset className="grid grid-cols-1 gap-4 rounded-lg border border-border p-4 md:grid-cols-3">
        ...
        <Field label="Level" required {...f('geographyLevel')} hint="Statewide figures are context; never clone them into a city.">
          {({ id }) => (
        ...
        <Field label="Source geography label" required {...f('geographyLabel')} hint="As the source names it, e.g. Oyo State, Ibadan.">
          {({ id, invalid, describedBy }) => <Input id={id} aria-invalid={invalid} aria-describedby={describedBy} {...register('geographyLabel')} />}
        </Field>
      </fieldset>

      <Field label="Editorial note (interpretation v1)" htmlFor="of-editorialNote" hint="Context for reviewers: stock mix, caveats, why it matters.">
        {({ id }) => <Textarea id={id} {...register('editorialNote')} />}
      </Field>
      <div className="flex gap-2">
        <Button type="submit" loading={formState.isSubmitting} loadingLabel="Submitting">Submit for review</Button>

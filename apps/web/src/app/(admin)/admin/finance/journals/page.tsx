...
export default async function JournalsPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  const identity = await requireStaffPage('finance.read');
  const raw = await searchParams;
  const page = Math.max(1, Number(raw.page) || 1);
  const [result, orgs] = await Promise.all([
    listJournals(identity, { organizationId: raw.organizationId || undefined, sourceType: raw.sourceType?.trim() || undefined, page, pageSize: PAGE_SIZE }),
  ...
  return (
    <div className="space-y-6">
      ...
      <FilterBar>
        ...
        <FilterInput name="sourceType" label="Source type" value={raw.sourceType} placeholder="e.g. allocation, refund" />
      </FilterBar>
          ...
          <li key={j.id} className="rounded-lg border border-border bg-bg-elevated p-3 text-sm">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <span className="font-medium">
                <Mono>{j.businessEventRef}</Mono> {j.description ? <span className="text-fg-muted">· {j.description}</span> : null}
              </span>
              <span className="flex flex-wrap items-center gap-2 text-xs text-fg-muted">
                ...
                <Badge tone={j.balanced ? 'success' : 'danger'}>{j.balanced ? 'balanced' : 'NOT balanced'}</Badge>
              </span>
            </div>
              ...
              <table className="w-full min-w-[480px] text-left text-xs">
                ...
                <thead>
                  <tr className="text-fg-muted">
                    <th scope="col" className="py-1 pr-2 font-medium">Account</th>
                    <th scope="col" className="py-1 pr-2 text-right font-medium">Debit</th>
                    <th scope="col" className="py-1 pr-2 text-right font-medium">Credit</th>
                    <th scope="col" className="py-1 font-medium">Memo</th>
                  </tr>
                </thead>
                <tbody>
                  {j.lines.map((l) => (
                    <tr key={l.lineNo} className="border-t border-border">
                      ...
                      <td className="py-1 pr-2 text-right">{l.debitKobo !== '0' ? <Money kobo={l.debitKobo} /> : ''}</td>
                      <td className="py-1 pr-2 text-right">{l.creditKobo !== '0' ? <Money kobo={l.creditKobo} /> : ''}</td>

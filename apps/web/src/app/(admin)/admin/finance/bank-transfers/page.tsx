...
export default async function BankTransfersPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <FilterBar>
        <FilterSelect name="status" label="Status" value={status} allLabel="Awaiting review" options={bankReceiptStatusSchema.options.map((s) => ({ value: s, label: humanize(s) }))} />
      </FilterBar>
      <DataTable
        ...
        columns={[
          { key: 'inv', header: 'Invoice', cell: (r) => <Link href={`/admin/finance/invoices/${r.invoiceId}`} className="font-medium underline">{r.invoiceNumber ?? 'invoice'}</Link> },
          { key: 'org', header: 'Organisation', cell: (r) => r.organizationName },
          { key: 'amt', header: 'Declared', cell: (r) => <Money kobo={r.declaredAmountKobo} /> },
          { key: 'bal', header: 'Invoice balance', cell: (r) => <Money kobo={r.invoiceBalanceKobo} />, hideOnMobile: true },
          { key: 'ref', header: 'Bank ref / paid on', cell: (r) => `${r.bankReference ?? '—'} · ${r.declaredPaidAt ?? '—'}`, hideOnMobile: true },
          { key: 'file', header: 'Slip', cell: (r) => (r.uploadedFileId ? <a href={`/api/v1/files/${r.uploadedFileId}/download`} className="underline">download</a> : 'none'), hideOnMobile: true },
          { key: 'by', header: 'Declared', cell: (r) => `${r.submittedByName ?? '—'} · ${formatDateTimeLabel(r.createdAt)}`, hideOnMobile: true },
          { key: 'st', header: 'Status', cell: (r) => <StatusBadge status={r.status === 'confirmed' ? 'successful' : r.status === 'rejected' ? 'rejected' : 'pending'} label={humanize(r.status)} /> },
          {
            key: 'act',
            header: 'Review',
            cell: (r) =>
              ['submitted', 'under_review'].includes(r.status) ? (
                <BankReceiptActions receiptId={r.id} declaredAmountKobo={r.declaredAmountKobo} invoiceBalanceKobo={r.invoiceBalanceKobo} invoiceNumber={r.invoiceNumber} canReconcile={canReconcile} />
              ) : (

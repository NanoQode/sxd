...
export default async function MessagesPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  const [inbox, staff] = await Promise.all([
    staffInbox(identity, { status, kind, mine: raw.mine === '1', q: raw.q?.trim() || undefined, page, pageSize: PAGE_SIZE }),
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        actions={<NewInternalConversation staff={staff.map((s) => ({ userId: s.userId, name: s.name }))} me={identity.session!.user.id} />}
      />
      <SavedViewsBar tableKey="messages" />
      <FilterBar>
        <FilterSelect name="status" label="Status" value={raw.status} allLabel="Open" options={[{ value: 'closed', label: 'Closed' }, { value: 'all', label: 'All' }]} />
        <FilterSelect name="kind" label="Kind" value={kind} options={conversationKindSchema.options.map((k) => ({ value: k, label: humanize(k) }))} />
        <FilterInput name="q" label="Subject" value={raw.q} placeholder="Search subjects" />
        {inbox.readAll ? <FilterCheckbox name="mine" label="Only conversations I am in" checked={raw.mine === '1'} /> : null}
      </FilterBar>
      <DataTable
        ...
        columns={[
          {
            ...
            cell: (c) => (
              <span>
                <Link href={`/admin/messages/${c.id}`} className={c.unreadCount > 0 ? 'font-semibold text-primary underline' : 'text-primary underline'}>
                  {c.subject}
                </Link>{' '}
                {c.unreadCount > 0 ? <Badge tone="primary">{c.unreadCount} unread</Badge> : null}
                {c.lastMessagePreview ? <span className="block max-w-prose truncate text-xs text-fg-muted">{c.lastMessagePreview}</span> : null}
              </span>
            ),
          },
          { key: 'kind', header: 'Kind', cell: (c) => humanize(c.kind) },
          { key: 'org', header: 'Organisation', cell: (c) => (c.organizationId ? <Link href={`/admin/customers/${c.organizationId}`} className="underline">{c.organizationName ?? 'organisation'}</Link> : 'internal'), hideOnMobile: true },
          { key: 'who', header: 'Participants', cell: (c) => <span className="text-xs">{c.participantNames.join(', ')}{!c.amParticipant ? <Badge className="ml-1">reading only</Badge> : null}</span>, hideOnMobile: true },
          { key: 'last', header: 'Last message', cell: (c) => (c.lastMessageAt ? formatDateTimeLabel(c.lastMessageAt) : '—') },
          { key: 'n', header: 'Messages', cell: (c) => c.messageCount, hideOnMobile: true },
          { key: 'st', header: 'State', cell: (c) => (c.closedAt ? <Badge>closed</Badge> : <Badge tone="success">open</Badge>) },
        ]}
      />

...
export function AuditLog({
  ...
  const [loaded, setLoaded] = useState<{
    ...
    more: AuditEventDto[];
    cursor: string | null;
  }>({ base: initial, more: [], cursor: initial.nextCursor });
  const current =
    loaded.base === initial ? loaded : { base: initial, more: [], cursor: initial.nextCursor };
  ...
  async function loadMore() {
    ...
    try {
      ...
      for (const [k, v] of Object.entries(query))
        if (v !== undefined && k !== 'cursor' && k !== 'limit') qs.set(k, String(v));
      qs.set('cursor', cursor);
      const page = await apiFetch<{ items: AuditEventDto[]; nextCursor: string | null }>(
  ...
  const toIso = (v: string, end: boolean) =>
    v ? new Date(`${v}T${end ? '23:59:59' : '00:00:00'}Z`).toISOString() : null;
  ...
  return (
    <div className="space-y-4">
        ...
        <Field label="Actor user id">
          {({ id }) => (
            <Input
              ...
              onBlur={(e) => setFilters({ actorUserId: e.target.value || null })}
            />
          )}
        </Field>
        ...
        <Field label="Entity id">
          {({ id }) => (
            <Input
              ...
              onBlur={(e) => setFilters({ entityId: e.target.value || null })}
            />
          )}
        </Field>
        <Field label="Action">
        ...
        <Field label="From">
          {({ id }) => (
            <Input
              ...
              onChange={(e) => setFilters({ from: toIso(e.target.value, false) })}
            />
          )}
        </Field>
        <Field label="To">
          {({ id }) => (
            <Input
              ...
              onChange={(e) => setFilters({ to: toIso(e.target.value, true) })}
            />
          )}
        </Field>
      </div>
      ...
      {cursor ? (
        <Button variant="secondary" onClick={loadMore} loading={busy} loadingLabel="Loading">
          Load more
        </Button>
      ) : (
        <p className="text-xs text-fg-muted">End of results.</p>
      )}
    </div>
  );
}

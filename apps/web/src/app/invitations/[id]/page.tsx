...
export default async function InvitationPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const invitation = /^[A-Za-z0-9_-]{1,64}$/.test(id)
    ? await getInvitationView(id, identity)
    : null;
  ...
  const emailMatches = Boolean(
    invitation && sessionEmail && invitation.email.toLowerCase() === sessionEmail.toLowerCase(),
  );
          ...
          {!invitation ? (
            <CardContent className="pt-5">
              <Alert tone="danger" title="Invitation not found">
                This link is not a valid invitation. Ask the person who invited you to send a new
                one.
              </Alert>
            </CardContent>
          ) : (
              ...
              <CardContent className="space-y-4">
                {invitation.status === 'expired' ? (
                  <Alert tone="warning" title="This invitation has expired">
                    It expired on {formatDateTimeLabel(invitation.expiresAt)}. Ask an owner of{' '}
                    {invitation.organizationName} to invite you again.
                  </Alert>
                ) : invitation.status === 'canceled' ? (
                  <Alert tone="warning" title="This invitation was revoked">
                    An owner of {invitation.organizationName} withdrew it. Contact them if you still
                    need access.
                  </Alert>
                  ...
                  <div className="space-y-3">
                    <p className="text-sm text-fg-muted">
                      Sign in with the invited email address, or create an account with it, to
                      accept. The invitation expires on {formatDateTimeLabel(invitation.expiresAt)}.
                    </p>
                    <div className="flex flex-col gap-2 sm:flex-row">
                      <Link
                        href={`/sign-in?next=${encodeURIComponent(nextPath)}`}
                        className="sx-touch inline-flex flex-1 items-center justify-center rounded-md bg-primary px-4 text-sm font-medium text-fg-on-primary"
                      >
                        Sign in
                      </Link>
                      <Link
                        href={`/sign-up?next=${encodeURIComponent(nextPath)}`}
                        className="sx-touch inline-flex flex-1 items-center justify-center rounded-md border border-border-strong px-4 text-sm font-medium"
                      >
                        Create account
                      </Link>
                    </div>
                  ...
                  <Alert tone="warning" title="Signed in with a different email">
                    ...
                    {sessionEmail}. Sign out and use the invited address, or ask for an invitation
                    to this account.
                  </Alert>
                ) : (
                  <AcceptInvitation

import { z } from 'zod';
import { getQuote } from '@simplexd/finance';
...
/** GET /api/v1/quotes/:id — quote with all versions (staff, or the owning organisation). */
export const GET = route<{ params: Promise<{ id: string }> }>(async (req, ctx) => {
  ...
  return json(await getQuote(rt, fa, id), { correlationId: ctx.correlationId });
});

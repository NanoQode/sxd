...
export const GET = route(async (req, ctx) => {
  ...
  return json(await listAssets(identity, parseQuery(req, assetListQuerySchema)), { correlationId: ctx.correlationId });
});
...
export const POST = route(async (req, ctx) => {
  ...
  return json(await createAsset(identity, body, { correlationId: ctx.correlationId }), { status: 201, correlationId: ctx.correlationId });
});

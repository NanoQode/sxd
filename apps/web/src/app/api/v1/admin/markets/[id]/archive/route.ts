...
/** POST /api/v1/admin/markets/:id/archive — markets are archived, never deleted. */
export const POST = route<{ params: Promise<{ id: string }> }>(async (req, ctx) => {
  ...
  return json(await transitionMarket(admin, id, 'archive', body), { correlationId: ctx.correlationId });
});

...
/** POST /api/v1/admin/markets/:id/unpublish (market_data.publish, MFA). */
export const POST = route<{ params: Promise<{ id: string }> }>(async (req, ctx) => {
  ...
  return json(await transitionMarket(admin, id, 'unpublish', body), { correlationId: ctx.correlationId });
});

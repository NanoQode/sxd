...
export const GET = route<{ params: Promise<{ id: string }> }>(async (_req, ctx) => {
  ...
  return json(await listMilestones(identity, id), { status: 200, correlationId: ctx.correlationId });
});
...
export const POST = route<{ params: Promise<{ id: string }> }>(async (req, ctx) => {
  ...
  return json(await createMilestone(identity, id, body, { correlationId: ctx.correlationId }), { status: 201, correlationId: ctx.correlationId });
});

...
export const GET = route(async (req, ctx) => {
  ...
  return json(await listOwnerStatements(identity, parseQuery(req, ownerStatementListQuerySchema)), {
...
export const POST = route(async (req, ctx) => {
  ...
  return json(await generateOwnerStatement(identity, body, { correlationId: ctx.correlationId }), {
    status: 201,
    correlationId: ctx.correlationId,
  });
});

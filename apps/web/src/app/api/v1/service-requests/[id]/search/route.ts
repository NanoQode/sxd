...
/** GET /api/v1/service-requests/:id/search — shortlists, comparison and viewings of the request. */
export const GET = route<{ params: Promise<{ id: string }> }>(async (_req, ctx) => {
  const identity = await identityOrThrow();
  const { id } = await params(ctx, idParams);
  return json(await getSearchWorkspace(identity, id), { correlationId: ctx.correlationId });
});

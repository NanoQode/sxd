...
import { z } from 'zod';
import { correlationIdHeader, publicListingMediaQuerySchema, slugSchema, uuidSchema } from '@simplexd/contracts';
 ...
 */
export const GET = route<{ params: Promise<{ slug: string; fileId: string }> }>(async (req, ctx) => {
  ...
  await enforceRateLimit(`listing-media:ip:${hashIp(clientIp(req))}`, { windowSeconds: 60, max: 120 });

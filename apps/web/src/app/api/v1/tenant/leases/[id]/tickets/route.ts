import { z } from 'zod';
import { ApiError, uuidSchema, workOrderCreateSchema } from '@simplexd/contracts';
import { getIdentity } from '@/lib/auth/session';
import { json, params, parseJson, route } from '@/lib/api/respond';
import { createMyTicket, listMyTickets } from '@/server/rentals/tenant';
...
export const GET = route<{ params: Promise<{ id: string }> }>(async (_req, ctx) => {
  ...
  return json({ items: await listMyTickets(identity, id) }, { correlationId: ctx.correlationId });
});

...
export const GET = route<{ params: Promise<{ id: string }> }>(async (_req, ctx) => {
  ...
  return json(await getChangeOrder(identity, id), { status: 200, correlationId: ctx.correlationId });
});
...
export const PATCH = route<{ params: Promise<{ id: string }> }>(async (req, ctx) => {
  ...
  return json(await updateChangeOrder(identity, id, body, { correlationId: ctx.correlationId }), { status: 200, correlationId: ctx.correlationId });
});

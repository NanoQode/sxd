import { z } from 'zod';
import { getBankTransferReceipt } from '@simplexd/finance';
...
/** GET /api/v1/bank-transfer-receipts/:id */
export const GET = route<{ params: Promise<{ id: string }> }>(async (req, ctx) => {
  ...
  return json(await getBankTransferReceipt(rt, fa, id), { correlationId: ctx.correlationId });
});

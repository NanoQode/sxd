...
export const GET = route<{ params: Promise<{ id: string }> }>(async (_req, ctx) => {
  ...
  return json({ items: await listTenderInvitations(identity, id, { correlationId: ctx.correlationId }) }, { correlationId: ctx.correlationId });
});
...
export const POST = route<{ params: Promise<{ id: string }> }>(async (req, ctx) => {
  ...
  return json({ items: await inviteTenderPartners(identity, id, body, { correlationId: ctx.correlationId }) }, { correlationId: ctx.correlationId });
});

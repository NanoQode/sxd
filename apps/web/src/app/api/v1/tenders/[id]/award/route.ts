...
export const GET = route<{ params: Promise<{ id: string }> }>(async (_req, ctx) => {
  ...
  return json(await getAward(identity, id, { correlationId: ctx.correlationId }), { correlationId: ctx.correlationId });
});
...
export const POST = route<{ params: Promise<{ id: string }> }>(async (req, ctx) => {
  ...
  return json(await decideAward(identity, id, body, { correlationId: ctx.correlationId }), { status: 201, correlationId: ctx.correlationId });
});

...
export const GET = route<{ params: Promise<{ id: string }> }>(async (_req, ctx) => {
  ...
  return json({ items: await listTenderQuestions(identity, id, { correlationId: ctx.correlationId }) }, { correlationId: ctx.correlationId });
});
...
export const POST = route<{ params: Promise<{ id: string }> }>(async (req, ctx) => {
  ...
  return json(await askTenderQuestion(identity, id, body, { correlationId: ctx.correlationId }), { status: 201, correlationId: ctx.correlationId });
});

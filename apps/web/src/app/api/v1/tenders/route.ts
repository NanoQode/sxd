...
export const GET = route(async (req, ctx) => {
  ...
  return json(await listTenders(identity, query, { correlationId: ctx.correlationId }), { correlationId: ctx.correlationId });
});

export const POST = route(async (req, ctx) => {
  const identity = await getIdentity();
  const body = await parseJson(req, tenderCreateSchema);
  return json(await createTender(identity, body, { correlationId: ctx.correlationId }), { status: 201, correlationId: ctx.correlationId });
});

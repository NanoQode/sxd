...
import { getRfqComparison } from '@/server/procurement/rfqs';
...
export const GET = route<{ params: Promise<{ id: string }> }>(async (_req, ctx) => {
  ...
  return json(await getRfqComparison(identity, id, { correlationId: ctx.correlationId }), { correlationId: ctx.correlationId });
});

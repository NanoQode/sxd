...
export const GET = route(async (req, ctx) => {
  ...
  return json({ items: await stayCalendar(identity, parseQuery(req, stayCalendarQuerySchema)) }, { correlationId: ctx.correlationId });
});
...
export const POST = route(async (req, ctx) => {
  ...
  return json(await createStayBooking(identity, body, { correlationId: ctx.correlationId }), { status: 201, correlationId: ctx.correlationId });
});

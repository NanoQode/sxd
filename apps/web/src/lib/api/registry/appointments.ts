...
export const detailRoute = ensure({
  ...
  responses: { 200: { description: 'Appointment', body: appointmentDtoSchema }, 404: { description: 'Not found' } },
});
...
export const rescheduleRoute = ensure({
  ...
  responses: { 200: { description: 'Appointment', body: appointmentDtoSchema }, 409: { description: 'Hold invalid or notice too short' } },
});
...
export const cancelRoute = ensure({
  ...
  responses: { 200: { description: 'Appointment', body: appointmentDtoSchema }, 409: { description: 'Invalid transition' } },
});
...
export const icsRoute = ensure({
  ...
  description: 'text/calendar fallback that works while Google sync is pending; includes the Meet link only once confirmed.',
  tags: ['appointments'],
...
export const calendarConnectRoute = ensure({
  ...
  description: 'Server-side authorisation code flow with PKCE and HMAC-signed state; requires appointments.manage_all.',
  tags: ['calendar', 'staff'],
  operationId: 'calendar.connect',
  auth: 'staff',
  request: { query: z.object({ calendarId: z.string().optional(), redirect: z.enum(['0', '1']).optional() }) },
  responses: { 200: { description: 'Authorization URL', body: connectStartResponseSchema }, 302: { description: 'Redirect to Google' } },
});
...
export const calendarCallbackRoute = ensure({
  ...
  description: 'Verifies state and PKCE, exchanges the code, stores encrypted tokens and redirects to the admin page.',
  tags: ['calendar', 'staff'],
...
export const calendarPushRoute = ensure({
  ...
  description: 'Header-only notification; the channel token is checked against the stored hash and an authenticated incremental sync is queued.',
  tags: ['calendar', 'webhooks'],
  operationId: 'calendar.push',
  auth: 'webhook',
  responses: { 200: { description: 'Accepted' }, 403: { description: 'Token mismatch' }, 404: { description: 'Unknown channel' } },
});
...
export const calendarStatusRoute = ensure({
  ...
  description: 'Adapter, redirect URI, organiser grants (connected / degraded / expired / disconnected with remedies) and sync health counts.',
  tags: ['calendar', 'admin'],
...
export const calendarTestBookingRoute = ensure({
  ...
  description: 'Creates a 15-minute event with a Meet request, reads it back and deletes it; reports the real provider result. Requires appointments.test_booking.',
  tags: ['calendar', 'admin'],

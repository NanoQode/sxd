...
describe('explorer api client', () => {
  it('parses a valid market list and sends the query string', async () => {
    const fetchSpy = mockFetch(() =>
      jsonResponse({
        ...
        total: 1,
        policy: { activeRankingPolicyVersion: null, defaultFinancialRankingEnabled: false, coverageThreshold: 0.7 },
  ...
  it('surfaces the error envelope with code, correlation id and retryability', async () => {
    mockFetch(() =>
      jsonResponse(
        { error: { code: 'provider_unavailable', message: 'Database unavailable', correlationId: 'corr-123', retryable: true } },
        { status: 503 },
      ),
    );
    ...
    expect(described).toMatchObject({ code: 'provider_unavailable', correlationId: 'corr-123', retryable: true, status: 503 });
  });
  ...
  it('accepts both array and {items} shapes for states', async () => {
    ...
    expect((await getStates())[0]).toMatchObject({ name: 'Lagos', isFederalCapital: false, marketCount: 0 });
    mockFetch(() => jsonResponse({ items: [{ id: 's2', name: 'Kano', geopoliticalZone: 'NW', marketCount: 1 }] }));
  ...
  it('normalises calculator responses of unknown key style', async () => {
    mockFetch(() => jsonResponse({ development_cost: { ok: false, reason: 'Missing required inputs: land.', missing: ['land'] } }));
    const result = await runCalculators({
      assumptions: { base: {} } as never,
      objective: 'long_term_rent',
      sensitivity: { vacancy: [], rents: [], costs: [], interest: [], completionDelayMonths: [] },
    });
    expect(result.developmentCost).toEqual({ ok: false, reason: 'Missing required inputs: land.', missing: ['land'] });
  });
  ...
  it('returns a document link when the report is not JSON, and a normalised report when it is', async () => {
    ...
    expect(await getScenarioReport('abc')).toEqual({ kind: 'document', url: '/api/v1/scenarios/abc/report', contentType: 'text/html' });
    mockFetch(() => jsonResponse({ report: { title: 'Comparison', generatedAt: '2026-09-23T00:00:00Z', policyVersion: 2, disclaimers: ['Not advice'] } }));
    const report = await getScenarioReport('abc');
    expect(report).toMatchObject({ kind: 'json', title: 'Comparison', policyVersion: 2, disclaimers: ['Not advice'] });
  });

...
export async function listPartners(
  ...
  requireAnyStaff(identity, ['access.partners.verify', 'service_requests.assign', 'tenders.manage', 'projects.manage']);
  return staffTx(identity, async (tx) => {
    ...
    const filtered = rows.filter((r) => {
      if (filters.verificationStatus && r.p.verificationStatus !== filters.verificationStatus) return false;
    ...
    const assignmentCounts = ids.length
      ? await tx
          .select({ userId: schema.assignments.assigneeUserId, status: schema.assignments.status, n: count() })
    ...
    return filtered.map((r) => {
      ...
      return {
        ...
        bids: {
          submitted: (bc.submitted ?? 0) + (bc.evaluated ?? 0) + (bc.awarded ?? 0) + (bc.unsuccessful ?? 0),

    ...
    <div className="sm:col-span-2 space-y-2">
      {fields.length === 0 ? (
        <p className="text-xs text-fg-muted">
          Add at least one unit group (e.g. 4 × 3-bed flats at ₦3,000,000 per year).
        </p>
      ) : null}
      {fields.map((field, index) => (
        <div
          key={field.id}
          className="grid gap-2 rounded-md border border-border p-2 sm:grid-cols-[1fr_auto_1fr_auto]"
        >
          <Field label="Unit label" error={errorAt(errors, `units.${index}.label`)}>
            {({ id }) => (
              <Input id={id} placeholder="3-bed flats" {...register(`units.${index}.label`)} />
            )}
          </Field>
          <Field label="Count" error={errorAt(errors, `units.${index}.count`)}>
            {({ id, invalid }) => (
              <Input
                ...
                aria-invalid={invalid || undefined}
                className="sm:w-24"
                {...register(`units.${index}.count`)}
              />
            )}
          </Field>
          <Field
            label="Annual rent per unit (₦)"
            error={errorAt(errors, `units.${index}.annualRentPerUnitNaira`)}
          >
            {({ id, invalid }) => (
              <Input
                id={id}
                inputMode="decimal"
                aria-invalid={invalid || undefined}
                {...register(`units.${index}.annualRentPerUnitNaira`)}
              />
            )}
          </Field>
          <div className="flex items-end">
            <Button
              size="icon"
              variant="ghost"
              aria-label={`Remove unit group ${index + 1}`}
      ...
      <Button
        variant="secondary"
        onClick={() => append({ label: '', count: '1', annualRentPerUnitNaira: '' })}
      >
    ...
    <fieldset className="rounded-lg border border-border p-3">
      ...
      {enabled ? (
        <div className="mt-2 grid gap-3 sm:grid-cols-2">
          ...
          <NumberField
            name={`${set}.completionDelayMonths`}
            label="Completion delay"
            unit="months"
          />
          <NumberField
            name={`${set}.annualDebtServiceNaira`}
          ...
          {units.map((group, index) => (
            <NumberField
              key={index}
              name={`${set}.unitRents.${index}`}
              label={`Annual rent per unit: ${group?.label || `group ${index + 1}`}`}
              unit="₦"
            />
          ))}
        </div>
...
export function ScenarioBuilder() {
  ...
  const showNpv =
    assumptions.base.discountRate !== null && assumptions.base.exitValueNaira !== null;

  return (
    <section
      aria-labelledby="scenario-builder-heading"
      className="rounded-lg border border-gold/60 bg-bg-elevated p-4"
    ...
    >
      <div className="mb-3">
        ...
        <p className="text-sm text-fg-muted">
          {COPY.scenarioDisclaimer} Whole naira, square metres, months and percentages.
        </p>
      </div>
        ...
        <FormProvider {...form}>
          ...
          >
            <SectionCard
              ...
              description="total = land + acquisition + build + fees + approvals + utilities/external works + contingency + financing during build"
            >
              <NumberField name="landCostNaira" label="Land cost" unit="₦" />
              <NumberField name="acquisitionCostsNaira" label="Acquisition costs" unit="₦" />
              <NumberField
                name="grossFloorAreaM2"
                label="Gross floor area"
                unit="m²"
                hint="Actual surveyed area; a plot is not a fixed size."
              />
              <NumberField name="buildRateNairaPerM2" label="Approved build rate" unit="₦ per m²" />
              <NumberField
                name="boqTotalNaira"
                label="Priced BOQ total"
                unit="₦"
                hint="When supplied the BOQ is the basis; it is never added to the area-rate estimate."
              />
              <NumberField name="professionalFeesNaira" label="Professional fees" unit="₦" />
              <NumberField name="approvalsNaira" label="Approvals" unit="₦" />
              <NumberField
                name="utilitiesAndExternalWorksNaira"
                label="Utilities and external works"
                unit="₦"
              />
              <NumberField name="contingencyPercent" label="Contingency" unit="% of build + fees" />
              <NumberField
                name="financingDuringBuildNaira"
                label="Financing during build"
                unit="₦"
              />
            </SectionCard>
            ...
            <SectionCard
              title="Rental income"
              description="scheduled rent = Σ units × annual rent; effective income = scheduled × (1 − vacancy) × (1 − collection loss) + other income"
            >
              <UnitGroups />
            ...
            <SectionCard
              title="Operating expenses and tax"
              description="Management fee is either a percentage of effective income or a fixed amount, never both."
            >
              <NumberField
                name="managementFeePercent"
                label="Management fee"
                unit="% of effective income"
              />
              <NumberField
                name="managementFeeFixedNaira"
              ...
              <NumberField
                name="unrecoverableChargesNaira"
                label="Unrecoverable charges"
                unit="₦ per year"
              />
              <Field
                label="Tax assumption"
                hint="Supplied and reviewed by the business; the calculator never chooses a rate."
              >
                {({ id, describedBy }) => (
                  <NativeSelect
                    id={id}
                    aria-describedby={describedBy}
                    {...form.register('taxKind')}
                  >
              ...
              {taxKind === 'fraction_of_noi' ? (
                <NumberField name="taxPercent" label="Tax on NOI" unit="%" />
              ) : null}
              {taxKind === 'fixed' ? (
                <NumberField name="taxFixedNaira" label="Fixed annual tax" unit="₦" />
              ) : null}
              <NumberField
                name="capexReserveNaira"
                label="Capex reserve"
                unit="₦ per year"
                hint="Reported separately from NOI."
              />
            </SectionCard>

            <SectionCard title="Debt, equity and schedule">
              <NumberField
                name="annualDebtServiceNaira"
                label="Annual debt service"
                unit="₦"
                hint="Principal and interest; not an NOI expense."
              />
              <NumberField
                name="equityNaira"
                label="Equity invested"
                unit="₦"
                hint="Needed for cash-on-cash."
              />
              <NumberField name="constructionMonths" label="Construction duration" unit="months" />
              <NumberField
                name="completionDelayMonths"
                label="Completion delay"
                unit="months"
                hint="Delays postpone the rental start."
              />
            </SectionCard>

            <fieldset className="rounded-lg border border-border p-3">
              <legend className="px-1 text-sm font-semibold">
                Short stay (optional, separate cohort)
              </legend>
              <label className="sx-touch flex items-center gap-2 text-sm">
                <input
                  type="checkbox"
                  className="h-5 w-5 accent-[var(--sx-primary)]"
                  {...form.register('shortStayEnabled')}
                />
              ...
              {shortStayEnabled ? (
                <div className="mt-2 grid gap-3 sm:grid-cols-2">
                  <NumberField
                    name="shortStay.availableNightsPerYear"
                    label="Available nights per year"
                    hint="Explicit; never assumed to be 365."
                  />
                  ...
                  <NumberField
                    name="shortStay.platformChargePercent"
                    label="Platform charge"
                    unit="%"
                  />
                  <NumberField
                    name="shortStay.cleaningCostPerStayNaira"
                    label="Cleaning per stay"
                    unit="₦"
                  />
                  <NumberField
                    name="shortStay.averageLengthOfStayNights"
                    label="Average length of stay"
                    unit="nights"
                  />
                  <NumberField
                    name="shortStay.operatingCostsNaira"
            ...
            <SectionCard
              title="Exit and discounting (optional)"
              description="NPV and IRR need an explicit discount rate, exit value and selling costs; nothing is defaulted."
            >
            ...
            <Field
              label="Notes"
              hint="Assumption sources, e.g. a quantity surveyor's estimate reference."
            >
              {({ id, describedBy }) => (
                <Textarea id={id} aria-describedby={describedBy} {...form.register('notes')} />
              )}
            </Field>
          </form>
        </FormProvider>

...
- **Paying rent online.** Invoices addressed to the tenant show a **Pay** button on `/tenant/balances`. It starts the hosted checkout (`POST /api/v1/invoices/{id}/payment-attempts`, allowed for the invoice's addressee even though rent invoices belong to the owner organisation). The checkout callback verifies the payment with the provider and returns to `/tenant/balances`, which shows the verified result; a receipt appears on `/tenant/receipts`. Returning from checkout is never treated as payment on its own.
- **Ticket photos.** The new-request form accepts up to five photos (`maintenance_photo` uploads, images only, 25 MB each). They are scanned, then attached to the ticket; attaching shares them with the owner organisation, staff with file access and the assigned contractor. Photos from the reporter describe the problem and never count as evidence that the work was done. If a photo fails to attach, the request is still sent and the tenant is told.

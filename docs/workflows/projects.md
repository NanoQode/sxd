...
- **Unscheduled visit**: an inspector with an accepted/active field assignment (inspector, surveyor, valuer, architect, quantity surveyor, contractor) or staff with `site_visits.perform` may start a visit nobody scheduled: `POST /projects/{id}/site-visits/unscheduled` (reason required, idempotent on `offlineClientId`) or a sync item carrying `unscheduledReason`. The visit starts `in_progress` with no `scheduledAt` (the DTO carries `unscheduled: true` and `unscheduledReason`; the reason is stored behind a marker in `instructions`, the one column a submission never overwrites), the project manager is notified (`project.site_visit.unscheduled_started`), and capture, evidence and offline rules are those of any in-progress visit. Staff with `projects.manage` or `reports.review` may reject it (`POST /site-visits/{id}/reject`, reason required, never the inspector) while it is in progress or submitted, i.e. until its findings are reviewed; the inspector is told (`project.site_visit.rejected`). Scheduled visits are cancelled, not rejected.
